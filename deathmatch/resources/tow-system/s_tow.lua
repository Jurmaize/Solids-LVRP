            ...
            if (owner > 0) then
                if (faction > 3 or faction < 0) then
                    if (source == towSphere2) then
                        ...
                        outputChatBox("((Lūdzu neaizmirstat veikt /park un /handbrake iekš mūsu auto placi!))", thePlayer, 255, 194, 14)
                    else
                ...
                if (owner > 0) then
                    if (faction > 3 or faction < 0) then
                        if (source == towSphere2) then
                            ...
                            outputChatBox("((Lūdzu neaizmirstat veikt /park un /handbrake iekš mūsu auto placi!))", player, 255, 194, 14)
                        else
                            if rank >= 5 then
                                if (faction ~= 4) then
                                    if (impounded == 0) then
                                        ...
                                        setVehicleEngineState(veh, false)
                                        outputChatBox("(( BMX/Mocis veiksmīgi konfiscēts. ))", player, 50, 205, 50)
                                        outputChatBox("((Lūdzu neaizmirstat veikt /park un /handbrake iekš mūsu auto placi!))", player, 255, 194, 14)
                                        isin = false
...
function payRelease(vehid)
    if exports.global:takeMoney(source, 1000) then
        ...
        outputChatBox("Auto veiksmīgi izpirkts, tas sagaida Jūs ēkas priekšpusē. (( Lūdzu neaizmirstat veikt /park un /handbrake, savādāk tas atjaunosies konfiscētuvē! ))", source, 255, 194, 14)
    else
        ...
        if not vehid then
            ...
            if not vehID then
                ...
                if getElementData(vehID, "Impounded") and getElementData(vehID, "Impounded") ~= 0 then
                    ...
                    outputChatBox("Auto #" .. vehid .. " nav šobrīd konfiscēts.", thePlayer, 255, 0, 0)
                end
            end
        end
    end
