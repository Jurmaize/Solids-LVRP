	...
	if (isElement(element)) and (getElementType(element)=="vehicle") and (matchingDimension) then
		...
		if (thePlayer) then
			...
			if (vehicleHealth >= 1000) then
				outputChatBox("Laipni lūgts garāžā! Tavs " .. exports.global:getVehicleName(element) .. " ir labā stāvoklī. Ja tev nākotnē ar to ir kādas problēmas, tu zini, kur braukt!", thePlayer, 255, 194, 14)
			else
				...
				if playerMoney == 0 and not factionPlayer[factionVehicle] and not doesPayForRepair(factionPlayer) and not PAYING_FACTIONS[factionVehicle] then
					outputChatBox("Piedod, bet tev nav pietiekami naudas, lai mēs strādātu uz tava auto.", thePlayer, 255, 0, 0)
				else
					outputChatBox("Laipni lūgts garāžā! Uzgaidi, kamēr mēs apskatām tavu " .. exports.global:getVehicleName(element) .. ".", thePlayer, 255, 194, 14)
	...
	if (isElementWithinColShape(thePlayer, shape)) then
		...
		if not factionPlayer[factionVehicle] or not doesPayForRepair(factionPlayer) or not PAYING_FACTIONS[factionVehicle] then
			...
			if insured > 0 then
				...
				outputChatBox("BILL: Tavai apdrošināšanas kompānijai tika aizsūtīts rēķins $".. exports.global:formatMoney(estimatedCosts) .." apmērā, lūdzu uzgaidi, kamēr salabojam auto.", thePlayer, 0, 255, 0)
			...
			if exports.global:takeMoney(theTeam, estimatedCosts, true) then
				completefix = true
				setElementFrozen(vehicle, true)
				outputChatBox("Rēķins tiks aizsūtīts darba devējam, uzgaidi, kamēr sataisām Jūsu auto.", thePlayer, 0, 255, 0)
		...
		if (completefix) then
			setTimer(function()
				...
				outputChatBox("Tavs auto tika salabots!", thePlayer, 0, 255, 0)
		...
		outputChatBox("Tu aizmirsi uzgaidīt, kamēr taisām tavu auto!", thePlayer, 255, 0, 0)
	end
end
...
function acceptedInvoice(paymentType)
	...
	if paymentType == PAY_BY_CARD then 
		if not exports.bank:takeBankMoney(client, getElementData(client, "paynspray:bill")) then return outputChatBox("Jums nav pietiekami naudas bankā, lai samaksātu par darbu.", client, 255, 100, 100) end
	...
	elseif paymentType == PAY_BY_CASH then
		if not exports.global:takeMoney(client, getElementData(client, "paynspray:bill"), true) then return outputChatBox("Jums nav pietiekami skaidras naudas, lai samaksātu par darbu.", client, 255, 100, 100) end
	end

	exports.global:giveMoney(faction, getElementData(client, "paynspray:bill"), true)
	outputChatBox("Jūs samaksājāt rēķinu, lūdzu uzgaidiet, kamēr labojam auto.", client, 0, 255, 0)
	...
	setTimer(function(player)
		...
		outputChatBox("Jūsu automašīna ir salabota!", player, 0, 255, 0)
