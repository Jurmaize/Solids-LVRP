...
function respawnPlayer(thePlayer, victimDropItem)
	if (isElement(thePlayer)) then
		...
		exports.global:takeMoney( getTeamFromName("Los Santos Valdība"), math.ceil((1-tax)*cost) )
		...
		outputChatBox("Jūs izārstēja Los Santos Glābšanas Dienests.", thePlayer, 255, 255, 0)
	...
	if action == 1 then --pay by cash
		if exports.global:takeMoney(client, toPay) then
			...
			if exports.global:giveMoney(theTeam, amount) then
				...
				outputChatBox("Tu samaksāji medicīnas rēķinu ($"..tostring(exports.global:formatMoney(amount))..").", client, 0, 250, 0)
				outputChatBox(name.." samaksāja medicīnas rēķinu ($"..tostring(exports.global:formatMoney(amount))..").", doctor, 0, 250, 0)
			end
		else
			outputChatBox("Jums uz rokas nav pietiekami naudas, lai nomaksātu šo rēķinu. Tiks izņemts no Jūsu bankas konta.", client, 255, 0, 0)
			payMedicalBill(client, doctor, faction, amount, insurance, 2)

			local name = exports.global:getPlayerName(client):gsub("_", " ")
			outputChatBox(name.." nepietiek naudas, lai nomaksātu medicīnas rēķinu.", doctor, 255, 0, 0)
		end
		...
		if exports.bank:hasBankMoney(client, toPay) then
			if exports.bank:takeBankMoney(client, toPay) then
				...
				if exports.global:giveMoney(theTeam, amount) then
					...
					outputChatBox("Tu samaksāji savu medicīnas rēķinu ($"..tostring(exports.global:formatMoney(amount))..").", client, 0, 250, 0)
					outputChatBox(name.." samaksāja savu medicīnas rēķinu ($"..tostring(exports.global:formatMoney(amount))..").", doctor, 0, 250, 0)
				end
			else
				outputChatBox("Transakcijas kļūda notika. Rēķins nesamaksāts.", client, 255, 0, 0)
				local name = exports.global:getPlayerName(client):gsub("_", " ")
				outputChatBox(name.." nevarēja samaksāt rēķinu dēļ transkacijas kļūmes.", doctor, 255, 0, 0)
			end
		else
			outputChatBox("Jums nepeitiek nauda, lai samaksātu rēķinu.", client, 255, 0, 0)
			local name = exports.global:getPlayerName(client):gsub("_", " ")
			outputChatBox(name.." nepietiek nauda rēķinam.", doctor, 255, 0, 0)
		end
	elseif action == 3 then --refuse to pay
		local name = exports.global:getPlayerName(client):gsub("_", " ")
		outputChatBox(name.." atsakas maksāt medicīnas rēķinu.", doctor, 255, 0, 0)
	end
end
