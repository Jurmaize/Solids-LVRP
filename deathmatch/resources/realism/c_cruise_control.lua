...
function deactivateCruiseControl()
    ...
    setPedControlState(localPlayer, "accelerate",false)
    ccEnabled = false
    exports.hud:sendBottomNotification(localPlayer, "Kruīza kontrole", "Kruīza kontrole izslēgta.")
end
				...
				if (ccEnabled) then
					...
					if targetSpeed > 10 then
						if (getVehicleType(theVehicle) == "Automobile" or getVehicleType(theVehicle) == "Bike" or getVehicleType(theVehicle) =="Boat" or getVehicleType(theVehicle) == "Train" or getVehicleType(theVehicle) == "Plane" or getVehicleType(theVehicle) == "Helicopter") or specialVehicles[getElementModel(theVehicle)] then
							exports.hud:sendBottomNotification(localPlayer, "Krūiza kontrole ieslēgta", "Lieto - un +, lai mainītu ātrumu.")
							activateCruiseControl()
						end
					else
                        exports.hud:sendBottomNotification(localPlayer, "Krūiza kontrole", "Kruīza kontroli var lietot, lai noturētu vienādu ātrumu, nejau lai uzsāktu braukšanu!")
					end
				end
            else
                exports.hud:sendBottomNotification(localPlayer, "Krūiza kontrole", "Dzinējs ir izslēgts!")
			end
		end
	end
end
