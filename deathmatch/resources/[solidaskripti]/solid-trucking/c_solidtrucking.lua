...
function onTruckEnter(vehicle, seat)
	if (trucks[getElementModel(vehicle)] and getVehicleController(vehicle) == localPlayer) and seat == 0 then
		outputChatBox("Tu esi smagajā! Raksti /starttrucking, lai uzsāktu darbu, kā tālbraucējs!")
		outputChatBox("Jebkurā laikā vari rakstīt /stoptruckingroute, lai pārtrauktu maršrutu!")
	...
	if drivenVehicle and getVehicleController(drivenVehicle) == localPlayer then
		...
		if (trucks[drivenVehicleID]) then
			if getTrailerCP or getTrailerblip then
				outputChatBox("Tu esi jau uzsācis maršrutu, tāpēc jaunu nevari uzsākt!")
			elseif deliverLoadCP or deliverLoadBlip then
				outputChatBox("Tu jau esi uzsācis maršrutu! Pabeidz šo pirms ņem jaunu!")
			else
				createTruckingWindow()
			end
		else
			outputChatBox("Tev ir jābūt tālbraucēju auto, lai uzsāktu darbu!")
		end
	else
		outputChatBox("Tu neesi tālbraucēju smagajā, vai vismaz to nevadi!")
	end
end
...
function createTruckingWindow()
	if isGUImade == false then
		...
		truckingWindow = dgsCreateWindow(x, y, width, height, "Tālbraucēju izvēlne", true)
		...
		dgsGridListAddColumn(truckingRouteWindow, "Kravas punkts", 0.2, nil, "left")
		dgsGridListAddColumn(truckingRouteWindow, "Galapunkts", 0.2, nil, "left")
		dgsGridListAddColumn(truckingRouteWindow, "Līdz kravai", 0.13, nil, "left")
		dgsGridListAddColumn(truckingRouteWindow, "No kravas -> līdz galapunktam", 0.23, nil, "left")
		dgsGridListAddColumn(truckingRouteWindow, "Alga", 0.1, nil, "left")
		dgsGridListAddColumn(truckingRouteWindow, "Nelegāls?", 0.1, nil, "left")
...
function getSelectedRoute()
	...
	if #selectedRouteInfo == 7 then
		...
		getTrailerBlip = exports.customblips:createCustomBlip(deliveryPoints[tonumber(selectedRouteID)][3], deliveryPoints[tonumber(selectedRouteID)][4], 20, 20, truckingBlipPath, 9999)
		exports.hud:sendBottomNotification(localPlayer, "Tālbraucējs", "Kravas saņemšanas punkts tika atzīmēts kartē ar tālbraucēju auto!")
		dgsCloseWindow(truckingWindow)
		showCursor(false)
		isGUImade = false
	else
		outputChatBox("Tu neesi izvēlējies maršrutu! Vispirms izvēlies vienu tad meiģini vēlreiz!")
	end
end
	...
	if thePlayer == getLocalPlayer() then
		...
		if drivenVehicle and getVehicleController(drivenVehicle) == localPlayer and getPedOccupiedVehicleSeat(localPlayer) == 0 then
			...
			if (trucks[drivenVehicleID]) then
				...
				deliverLoadBlip = exports.customblips:createCustomBlip(deliveryPoints[tonumber(selectedRouteID)][7], deliveryPoints[tonumber(selectedRouteID)][8], 20, 20, truckingBlipPath, 9999)
				exports.hud:sendBottomNotification(localPlayer, "Tālbraucējs", "Kravas nodošanas punkts tika atzīmēts kartē ar tālbraucēju auto!")
				outputChatBox("[Tālbraucējs] Ja krava netiks aizvesta "..tostring(timeTillTrailerGetsDeleted/1000/60) .. " minūtēs, tā tiks automātiski dzēsta!")
			else
				outputChatBox("Tev ir jaatrodas smagajā auto, lai saņemtu kravu!")
			end
		else
			outputChatBox("Tev ir jaatrodas smagajā auto, lai saņemtu kravu!")
		end
	...
	if drivenVehicle and getVehicleController(drivenVehicle) == localPlayer and getPedOccupiedVehicleSeat(localPlayer) == 0 then
		...
		if (trucks[drivenVehicleID]) then
			if getVehicleTowedByVehicle(drivenVehicle) == trailer then
				...
				deliverLoadCP = nil
				deliverLoadBlip = nil
				outputChatBox("Veiksmīgi aizvedāt kravu! Lai vestu jaunu, rakstiet /starttrucking.")
				exports.hud:sendBottomNotification(localPlayer, "Tālbraucējs", "Veiksmīgi aizvedāt kravu!")
			else
				outputChatBox("Jums nav pievienots traileris vai arī šis nav jūsu traileris.")
			end
		else
			outputChatBox("Tu nevadi tālbraucēju auto.")
		end
	else
		outputChatBox("Tev ir jaabut tālbraucēju auto, lai nodotu kravu!")
	end
end
		...
		if heavyTruck then
			if trucks[getElementModel(heavyTruck)] then
				...
				if getElementData(heavyTruck, "solidtrucking:illegal") == true then
					...
					outputChatBox("Pēc trailera pārmeklēšansas neko nelegālu neatrastu!")
				end
			else
				outputChatBox("Tuvākais transports nav tālbraucēju auto. Pameiģini aiziet pie kabīnes.")
			end
		end
	else
