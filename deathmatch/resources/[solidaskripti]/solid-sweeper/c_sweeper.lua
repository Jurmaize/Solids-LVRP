...
function enterSweeper(vehicle, seat)
    ...
    if seat == 0 and getElementModel(vehicle) == sweeperID then
        if not sweeperMarker then
            outputChatBox("Raksti /startsweeping, lai uzsāktu darbu!")
        end
    end
end
...
addEventHandler("onClientResourceStart", root,
    function (startedRes)
        exports.customblips:createCustomBlip(blipx,blipy, 20, 20, customBlipPath, 9999) -- Customblip resource function! Remove and change to normal one if you don't have the resource!
    end
);
...
function startSweeping()
    ...
    if sweeperMarker then
        exports.hud:sendBottomNotification(localPlayer, "Ielas tīrītājs", "Tu jau esi uzsācis darbu!")
    else
        if vehicle and getVehicleController(vehicle) == localPlayer and getElementModel(vehicle) == sweeperID then
            ...
            marker_number = 0
            exports.hud:sendBottomNotification(localPlayer, "Ielas tīrītājs", "Seko marķieriem, lai tīrītu ielas!")
        else
            exports.hud:sendBottomNotification(localPlayer, "Ielas tīrītājs", "Tu nevari tīrīt ielas bez tīrīšanas auto!")
        end
    end
end
...
function updateSweeperCheckpointCheck(thePlayer)
    if thePlayer == localPlayer then
        ...
        if vehicle and getElementModel(vehicle) == sweeperID then
            ...
            exports.hud:sendBottomNotification(localPlayer, "Ielas tīrītājs", "Tev ir jaatrodas iekš ielas tīrītāja auto, lai tīrītu ielas!")
        end
    end
end
...
function checkEndOfLine()
    if nextsweeperMarker then
        ...
        triggerServerEvent("paySweeper", localPlayer, line, 0)
        exports.hud:sendBottomNotification(localPlayer, "Ielas tīrītājs", "Iela ir tīra! Tu vari uzsākt jaunu maršrutu rakstot /startsweeping")
        local braucamaiscels = nil
    end
end