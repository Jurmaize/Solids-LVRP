...
function validateCredentials(username,password,checksave)
	if not (username == "") then
		if not (password == "") then
			...
			triggerClientEvent(client,"set_warning_text",client,"Login","Lūdzu ievadiet savu paroli!")
		end
	else
		triggerClientEvent(client,"set_warning_text",client,"Login","Lūdzu ievadiet lietotāju!")
	end
	return false
end
			...
			if result and client then
				if #result > 0 then
					...
					if exports.bans:checkAccountBan(accountData["id"]) then
						triggerClientEvent(client,"set_warning_text",client,"Login","Konts ir nobanots. Apelāciju rakstīt iekš https://discord.gg/S5PDPdDeac")
					...
					triggerClientEvent(client,"set_authen_text",client,"Login","Parole pieņemta! Notiek pārbaude..")
					-- Check if old method
					if not string.find(accountData["password"], "$", 1, true) then -- Plain search, not regex
						...
						local new_pass = passwordHash(password, "bcrypt") -- 12 work factor // https://github.com/django/django/blob/master/django/contrib/auth/hashers.py#L404
						...
						if not verified then
							triggerClientEvent(client,"set_warning_text",client,"Login","Nepareiza parole priekš šī konta nosaukuma '".. username .."'!")
							return false
						end
					end
					...
					for _, thePlayer in ipairs(exports.pool:getPoolElementsByType("player")) do
						...
						if (playerAccountID) then
							if (playerAccountID == tonumber(accountData["id"])) and (thePlayer ~= client) then
								kickPlayer(thePlayer, thePlayer, "Kāds cits ir jūsu kontā.")
								triggerClientEvent(client,"set_authen_text",client,"Login","Konts jau šobrīd tiek lietots. Izmetam otru lietotāju..")
								break
							end
						end
					end
					...
					if not exports.serialwhitelist:check(client) then
						triggerClientEvent(client,"set_warning_text",client,"Login","Tu neesi atļauts pievienoties serverim no šī datora. Raksti augstākajai administrācijai.")
				...
				triggerClientEvent(client,"set_warning_text",client,"Login","Neiznāca savienoties ar serveri. Datubāzes problēma!")
				dbFree(qh)
			end
		...
		if result then
			if #result == 0 then
				...
				if (mysql:num_rows(Q2) > 0) and usernameExisted["id"] ~= "1" then
					triggerClientEvent(client,"set_warning_text",client,"Register","Vairāki konti nav atļauti tu gurķi (Eksistē: "..tostring(usernameExisted["username"])..")")
					return false
				end
				mysql:free_result(Q2)

				--START CREATING ACCOUNT.
				local encryptedPW = passwordHash(password, "bcrypt",{}) -- 12 work factor // https://github.com/django/django/blob/master/django/contrib/auth/hashers.py#L404
				...
				if userid then
					...
					triggerClientEvent(client,"set_warning_text",client,"Register","Neiznāca izveidot jaunu kontu.")
					return false
				end
			else
				triggerClientEvent(client,"set_warning_text",client,"Register","Lietotāj vārds vai epasts jau eksistē!")
			end
		else
