	...
	if (characterList) then
		...
		for _, v in ipairs(characterList) do
			...
			if thePed and isElement( thePed ) then
				...
				outputChatBox("[ACCOUNT] Error occurred while spawning character '"..v[2].."'. Reporto: https://discord.gg/S5PDPdDeac :" )
...
--[[
	...
	dxDrawText("GC:", 8+mta_posxOffset, 65+mta_posyOffset, 74, 107+mta_posyOffset, tocolor(255, 255, 255, 255), 1.00, "default-bold", "left", "top", false, false, true, false, false)
	dxDrawText("Stundas:", 170+mta_posxOffset, 65+mta_posyOffset, 242, 106+mta_posyOffset, tocolor(255, 255, 255, 255), 1.00, "default-bold", "left", "top", false, false, true, false, false)
	...
	dxDrawText("Konta izveides datums:", 8+mta_posxOffset, 88+mta_posyOffset, 137, 124+mta_posyOffset, tocolor(255, 255, 255, 255), 1.00, "default-bold", "left", "top", false, false, true, false, false)
	...
	dxDrawText("Spēlēts:", 8+mta_posxOffset, 106+mta_posyOffset, 74, 144+mta_posyOffset, tocolor(255, 255, 255, 255), 1.00, "default-bold", "left", "top", false, false, true, false, false)
	...
	dxDrawText("Epasts:", 8+mta_posxOffset, 125+mta_posyOffset, 113, 169+mta_posyOffset, tocolor(255, 255, 255, 255), 1.00, "default-bold", "left", "top", false, false, true, false, false)
    ...
    dxDrawText("Konta vēsture", 54+mta_posxOffset, 150+mta_posyOffset, width-54+mta_posxOffset, 183+mta_posyOffset, tocolor(255, 255, 255, 255), 1.00, "arial", "center", "top", false, false, true, false, false) -- History
    ...
    dxDrawText("Atsvaidzināt personāžus", 3+mta_posxOffset, 172+mta_posyOffset, buttonWidth+mta_posxOffset, 204+mta_posyOffset,  cooldown and cooldown>=getTickCount()-5000 and tocolor(255, 255, 255, 100) or tocolor(255, 255, 255, 255), 1.00, "arial", "center", "top", false, false, true, false, false)
...
function linkForumAccount()
	...
	forumLink.window[1] = guiCreateWindow(749, 273, 318, 223, "Flog N/A", false)
	guiWindowSetSizable(forumLink.window[1], false)
	exports.global:centerWindow(forumLink.window[1])

	forumLink.button[1] = guiCreateButton(165, 171, 143, 42, "Atcelt", false, forumLink.window[1])
	guiSetProperty(forumLink.button[1], "NormalTextColour", "FFAAAAAA")
	forumLink.button[2] = guiCreateButton(9, 171, 146, 42, "Pieslēgties", false, forumLink.window[1])
	guiSetProperty(forumLink.button[2], "NormalTextColour", "FFAAAAAA")
	forumLink.label[1] = guiCreateLabel(11, 23, 287, 17, "N/A", false, forumLink.window[1])
	guiSetFont(forumLink.label[1], "default-bold-small")
	guiLabelSetHorizontalAlign(forumLink.label[1], "center", false)
	forumLink.label[2] = guiCreateLabel(12, 40, 287, 17, "Credent N/A", false, forumLink.window[1])
	guiLabelSetHorizontalAlign(forumLink.label[2], "center", false)
	forumLink.label[3] = guiCreateLabel(19, 71, 113, 15, "UserNA:", false, forumLink.window[1])
	forumLink.edit[1] = guiCreateEdit(20, 86, 278, 23, "", false, forumLink.window[1])
	forumLink.label[4] = guiCreateLabel(19, 118, 113, 15, "PassNA:", false, forumLink.window[1])
...
function Characters_characterSelectionVisisble()
	...
	local text1= guiCreateLabel (0,0,1,1, "Izrakstīties", true, bLogout)
	...
	newCharacterButton_text = guiCreateLabel (0,0,1,1, "Izveidot jaunu personāžu!", true, newCharacterButton)
	...
	local greetings = {
		...
		"Čav",
		"Yo yo yo",
		"Sveiks mans mazais dirsā pisēj",
	}
...
function characterMouseOver()
	...
	if isCursorShowing( ) then
		...
		if ped and isElement(ped) then
			...
			local text = "■ Rase: "..tRace.."\n■ Dzimums: "..(getElementData(ped, "account:charselect:gender") == 0 and "Male" or "Female").."\n■ Statuss: "..(getElementData(ped, "account:charselect:cked") > 0 and "#FF0000Dead" or "#00FF00Alive").."\n#FFFFFF■ Vecums: "..getElementData(ped, "account:charselect:age").."\n■ Garums: "..getElementData(ped, "account:charselect:height").."cm\n■ Svars: "..getElementData(ped, "account:charselect:weight").."kg\n■ Nospēlēts: "..getElementData(ped, "account:charselect:hoursplayed").."h"
			local text2 = "■ Rase: "..tRace.."\n■ Dzimums: "..(getElementData(ped, "account:charselect:gender") == 0 and "Male" or "Female").."\n■ Statuss: "..(getElementData(ped, "account:charselect:cked") > 0 and "Dead" or "Alive").."\n■ Vecums: "..getElementData(ped, "account:charselect:age").."\n■ Garums: "..getElementData(ped, "account:charselect:height").."cm\n■ Svars: "..getElementData(ped, "account:charselect:weight").."kg\n■ Nospēlēts: "..getElementData(ped, "account:charselect:hoursplayed").."h"
			...
			text = string.upper("Uzspied, lai spēlētu kā: "..name)
...
function characters_onSpawn(fixedName, adminLevel, gmLevel, location)
	...
	outputChatBox("Nospied F1 priekš palīdzības.", 255, 194, 14)
	outputChatBox("Tu vari apskatīt izvēlni nospiežot F10 vai /home.", 255, 194, 15)
