-- // Chaos
local maxFineAllowed = 50000 --Added maximum fine variable when checking arrest -Solid 24/11/2021
...
function PrisonGUIF(result)
        ...
        PrisonGUI.window[1] = guiCreateWindow(x, y, w, h, "Cietuma sistēma", false)
        ...
        PrisonGUI.column[1] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Unikāls ID", 0.1)
        PrisonGUI.column[7] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Kamera", 0.05)
        PrisonGUI.column[2] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Vārds", 0.2)
        PrisonGUI.column[3] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Arestēšanas datums", 0.15)
        PrisonGUI.column[4] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Dienas", 0.1)
        PrisonGUI.column[8] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Stundas", 0.1)
        PrisonGUI.column[10] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Pieslēgušās minūtes", 0.08)
        PrisonGUI.column[5] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Pēdējo reiz atjaunots", 0.1)
        PrisonGUI.column[9] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Sods", 0.1)
        PrisonGUI.column[6] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Pārkāpumi", 0.5)

        PrisonGUI.button[1] = guiCreateButton(13, 241, 100, 73, "Aizvērt", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[1], "NormalTextColour", "FFAAAAAA")
        PrisonGUI.button[2] = guiCreateButton(129, 243, 119, 30, "Izlaist cietumnieku", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[2], "NormalTextColour", "FFAAAAAA")
        PrisonGUI.button[3] = guiCreateButton(130, 280, 119, 30, "Pievienot cietumnieku", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[3], "NormalTextColour", "FFAAAAAA")
        PrisonGUI.button[4] = guiCreateButton(263, 243, 119, 30, "Rediģēt cietumnieku", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[4], "NormalTextColour", "FFAAAAAA")
        PrisonGUI.button[5] = guiCreateButton(588, 242, 134, 30, "Atvērt vienus vārtus", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[5], "NormalTextColour", "FFAAAAAA")
        PrisonGUI.button[6] = guiCreateButton(588, 282, 134, 30, "Atvērt visus vārtus", false, PrisonGUI.window[1])
        ...
        if pd_offline_jail or exports.integration:isPlayerTrialAdmin(getLocalPlayer()) then
            PrisonGUI.button[7] = guiCreateButton(263, 280, 119, 30, "Pievienot izgajušu spēlētāju", false, PrisonGUI.window[1])
        ...
        addEventHandler("onClientGUIClick", PrisonGUI.button[2], function()
            ...
            if r>=0 and c>=0 then
                ...
                if not isCloseTo(getLocalPlayer(), targetPlayer) then
                    outputChatBox("Tev ir jābūt tuvumā šim cietumniekam, lai viņu izņemtu.", 255, 0, 0)
                    return
                end
                triggerServerEvent("removePrisoner", resourceRoot, removeid, true)
            else
                outputChatBox("Vispirms veiciet atlasi.", 255, 0, 0)
            end
            end, false)
        ...
        addEventHandler("onClientGUIClick", PrisonGUI.button[4], function()
            ...
            if r>=0 and c>=0 then
                local targetPlayer = getPlayerFromName(guiGridListGetItemText(PrisonGUI.gridlist[1], r, 3))
                if not isCloseTo(getLocalPlayer(), targetPlayer) then
                    outputChatBox("Tev ir jābūt tuvumā šim cietumniekam, lai viņu rediģētu.", 255, 0, 0)
                    return
                end
        ...
        addEventHandler("onClientGUIClick", PrisonGUI.button[6], function() -- Toggle all gates
                if getElementDimension(getLocalPlayer(  )) == gateDim then
                    ...
                    gui["_root"] = guiCreateWindow(left, top, windowWidth, windowHeight, "Atvērt visus vārtus", false)
                    guiWindowSetSizable(gui["_root"], false)

                    gui["label"] = guiCreateLabel(10, 35, 231, 56, "Tu esi pārliecināts, ka vēlies atvērt\n visus vārtus cietumā?", false, gui["_root"])
                    guiLabelSetHorizontalAlign(gui["label"], "center", false)
                    guiLabelSetVerticalAlign(gui["label"], "center")

                    gui["pushButton"] = guiCreateButton(30, 95, 81, 31, "Jā", false, gui["_root"])
                    ...
                    gui["pushButton_2"] = guiCreateButton(150, 95, 81, 31, "Nē", false, gui["_root"])
                        addEventHandler("onClientGUIClick", gui["pushButton_2"], function()
                            destroyElement(gui["_root"])
                            local gui = nil
                        end, false)
                else
                    outputChatBox("Tev ir jābūt tajā pašā interjerā/dimensijā, kur ir vārti.", 255, 0, 0)
                end
            end, false)
...
function gridlistDoubleClick()
    ...
    if r>=0 and c>=0 then
        local targetPlayer = getPlayerFromName(guiGridListGetItemText(PrisonGUI.gridlist[1], r, 3))
        if not isCloseTo(getLocalPlayer(), targetPlayer) then
            outputChatBox("Tev ir jābūt šī cietumnieka tuvumā, lai viņu rediģētu.", 255, 0, 0)
            return
        end
        local ID = guiGridListGetItemText ( PrisonGUI.gridlist[1], r, 1 )
        local name = guiGridListGetItemText ( PrisonGUI.gridlist[1], r, 3 )
    ...
    elseif r==-1 or c==-1 then
        outputChatBox("Veiciet atlasti vispirms.", 255, 0, 0)
    end
end
...
addEventHandler( "PrisonGUI:Refresh", localPlayer, function(result)
        ...
        PrisonGUI.column[8] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Stundas", 0.1)
        PrisonGUI.column[10] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Pieslēgušās minūtes", 0.08)
        PrisonGUI.column[5] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Pēdējo reiz rediģēts", 0.1)
        PrisonGUI.column[9] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Sods", 0.1)
        PrisonGUI.column[6] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Pārkāpumi", 0.5)
        ...
        outputChatBox("Saraksts atjaunots.")
    end
, false)
...
function addPrisonerGUI(source, name, cell, days, hours, charges, row, fines, jailID)
    ...
    if arrestLocation then
        ...
        newPrisonerGUI.window[1] = guiCreateWindow(x, y, w, h, "Pievienot cietumnieku", false)
        guiWindowSetSizable(newPrisonerGUI.window[1], false)

        newPrisonerGUI.button[1] = guiCreateButton(13, 241, 123, 28, "Atcelt", false, newPrisonerGUI.window[1])
        guiSetProperty(newPrisonerGUI.button[1], "NormalTextColour", "FFAAAAAA")
        newPrisonerGUI.button[2] = guiCreateButton(170, 241, 123, 28, "Pievienot citumnieku", false, newPrisonerGUI.window[1])
        ...
        if source==PrisonGUI.button[3] then
            newPrisonerGUI.label[1] = guiCreateLabel(13, 22, 67, 15, "Vārds/ID:", false, newPrisonerGUI.window[1])
            guiSetFont(newPrisonerGUI.label[1], "default-bold-small")
            newPrisonerGUI.label[4] = guiCreateLabel(226, 25, 72, 17, "Netika atrasts", false, newPrisonerGUI.window[1])
        ...
        elseif source==PrisonGUI.button[7] then
            newPrisonerGUI.label[1] = guiCreateLabel(15, 22, 67, 15, "Precīzs vāŗds:", false, newPrisonerGUI.window[1])
            guiSetFont(newPrisonerGUI.label[1], "default-bold-small")

            guiSetEnabled(newPrisonerGUI.edit[5], false)
        end

        newPrisonerGUI.label[2] = guiCreateLabel(12, 147, 77, 15, "Pārkāpumi:", false, newPrisonerGUI.window[1])
        ...
        newPrisonerGUI.label[3] = guiCreateLabel(14, 48, 30, 15, "Kamera:", false, newPrisonerGUI.window[1])
        ...
        newPrisonerGUI.label[8] = guiCreateLabel(120, 48, 30, 15, "Sods:", false, newPrisonerGUI.window[1])
        guiSetFont(newPrisonerGUI.label[8], "default-bold-small")
        guiEditSetMaxLength(newPrisonerGUI.edit[5], 6)

        newPrisonerGUI.label[5] = guiCreateLabel(13, 96, 51, 15, "Dienas:", false, newPrisonerGUI.window[1])
        guiSetFont(newPrisonerGUI.label[5], "default-bold-small")
        newPrisonerGUI.label[6] = guiCreateLabel(7, 77, 301, 15, "============Arestēšanas laiks==========", false, newPrisonerGUI.window[1])
        guiLabelSetHorizontalAlign(newPrisonerGUI.label[6], "center", false)
        newPrisonerGUI.label[7] = guiCreateLabel(148, 96, 51, 15, "Stundas:", false, newPrisonerGUI.window[1])
        ...
        if name then
            ...
            guiSetText(newPrisonerGUI.window[1], "Atjaunot cietumnieku") -- All about the cosmedics baby
            guiSetText(newPrisonerGUI.button[2], "Atjaunot cietumnieku")
        ...
        addEventHandler("onClientGUIClick", newPrisonerGUI.button[2], function()
            if overLimit(guiGetText(newPrisonerGUI.edit[3]), guiGetText(newPrisonerGUI.edit[4])) then -- Returns true if over hour limit
                outputChatBox("Tu centies kādu arestēt virs "..tonumber(hourLimit).." stundu limita.", 255, 0, 0) return end

            local item = guiComboBoxGetSelected(newPrisonerGUI.edit[2])
            if item==-1 then
                outputChatBox("Vispirms izvēlies kameru!.", 255, 0, 0) return end
            ...
            if string.len(guiGetText(newPrisonerGUI.edit[1])) > 0 then
                ...
                if source==PrisonGUI.button[3] then -- Online add
                    if tonumber(guiGetText(newPrisonerGUI.edit[5])) <= maxFineAllowed then
                        if tonumber(guiGetText(newPrisonerGUI.edit[4]))+tonumber(guiGetText(newPrisonerGUI.edit[3])) >= 1 then
                            ...
                            outputChatBox("Lūdzu izvēlies lielāku arestēšans ilgumu.", 255, 0, 0)
                        end
                    else
                        outputChatBox("The maximum fine amount is "..tostring(maxFineAllowed), 255, 0, 0)
                    end
                elseif source==PrisonGUI.button[4] or source==PrisonGUI.gridlist[1] then -- Update info
                    ...
                    if tonumber(guiGetText(newPrisonerGUI.edit[4]))+tonumber(guiGetText(newPrisonerGUI.edit[3])) > 1 then -- Offline Add
                        ...
                        outputChatBox("Lūdzu ievadiet ilgāku apcietināšanas laiku!", 255, 0, 0)
                    end
                end
                destroyElement(newPrisonerGUI.window[1])
                guiSetInputEnabled(false)
            else
                outputChatBox("Tu ievadīji vārdu?", 255, 0, 0)
            end
        end, false)
        ...
        outputChatBox("Tev jābūt ir pie arestēšanas zonā, lai veiktu arestu!.", 255, 0, 0)
    end
end
...
function toggleGate()
    if isElement(ToggleGateGUI.window[1]) then
        ...
        local w, h = 272, 200
        local x, y = (scr[1]/2)-(w/2), (scr[2]/2)-(h/2)
        ToggleGateGUI.window[1] = guiCreateWindow(x, y, w, h, "Atvērt/Aizvērt vārtus", false)
        ...
        ToggleGateGUI.button[1] = guiCreateButton(11, 54, 120, 37, "Atvērt/Aizvērt vārtus", false, ToggleGateGUI.window[1])
        guiSetProperty(ToggleGateGUI.button[1], "NormalTextColour", "FFAAAAAA")
        ToggleGateGUI.button[2] = guiCreateButton(136, 54, 120, 37, "Atcelt", false, ToggleGateGUI.window[1])
        ...
        ToggleGateGUI.label[1] = guiCreateLabel(183, 120, 45, 20, "Atvērts", false, ToggleGateGUI.window[1])
        ToggleGateGUI.label[2] = guiCreateLabel(183, 140, 40, 20, "Aizvērts", false, ToggleGateGUI.window[1])
        ...
        addEventHandler("onClientGUIClick", ToggleGateGUI.button[1], function()
            ...
            if item>-1 then
                if getElementDimension(getLocalPlayer(  )) == gateDim then
                    ...
                    outputChatBox("Tev jābūt tajā pašā interjerā/dimensijā, kurā ir vārti.", 255, 0, 0)
                end
            else
                outputChatBox("Vispirms izvēlies vārtus.", 255, 0, 0)
            end
        end, false)
...
function checkNameExists()
    ...
    if (count>1) then
        guiSetText(newPrisonerGUI.label[4], "Vairāki atrasti.")
    ...
    elseif (count==0) then
        guiSetText(newPrisonerGUI.label[4], "Netika atrasts.")
        guiLabelSetColor(newPrisonerGUI.label[4], 255, 0, 0)
        guiSetEnabled(newPrisonerGUI.button[2], false)
    end
end
...
function updateGates()
    ...
    PrisonGUI.label[1] = guiCreateLabel(750, 287, 100, 30, open.." Vārti Atvērti", false, PrisonGUI.window[1])
    PrisonGUI.label[2] = guiCreateLabel(750, 247, 100, 30, closed.." Vārti Aizvērti", false, PrisonGUI.window[1])
    guiLabelSetColor(PrisonGUI.label[1], 0, 255, 0)
    guiLabelSetColor(PrisonGUI.label[2], 255, 0, 0)
end

function prisonPed()
    local ped = createPed(283, 216.545, 123.814, 999.01)
    setPedAnimation(ped, "COP_AMBIENT", "Coplook_loop", -1, true, false, false)
    setElementFrozen(ped, true)
    setElementRotation(ped, 0, 0, 181.48)
