...
function validateCredentials(username,password,checksave)
	if not (username == "") then
		if not (password == "") then
			...
			triggerClientEvent(client,"set_warning_text",client,"Login","Lūdzu ievadiet paroli!")
		end
	else
		triggerClientEvent(client,"set_warning_text",client,"Login","Lūdzu ievadiet lietotājvārdu!")
	end
	return false
end
			...
			if result and client then
				if #result > 0 then
					...
					if exports.bans:checkAccountBan(accountData["id"]) then
						triggerClientEvent(client,"set_warning_text",client,"Login","Profils ir banots. Apelāciju veikt: https://discord.gg/S5PDPdDeac")
					...
					triggerClientEvent(client,"set_authen_text",client,"Login","Parole pieņemta! Tiek pārbaudīts..")
					-- Check if old method
					if not string.find(accountData["password"], "$", 1, true) then -- Plain search, not regex
						...
						if not verified then
							triggerClientEvent(client,"set_warning_text",client,"Login","Parole ir nepareiza priekš lietotāja '".. username .."'!")
							return false
						end
					end
					...
					for _, thePlayer in ipairs(exports.pool:getPoolElementsByType("player")) do
						...
						if (playerAccountID) then
							if (playerAccountID == tonumber(accountData["id"])) and (thePlayer ~= client) then
								...
								triggerClientEvent(client,"set_authen_text",client,"Login","Konts šobrīd tiek lietots! Otrs lietotājs tiek izmests..")
								break
							end
						end
					end
					...
					if not exports.serialwhitelist:check(client) then
						triggerClientEvent(client,"set_warning_text",client,"Login","Jūs neesat atļauts pievienoties šim kontam no šī datora. Lūdzu kontaktēties ar administrāciju iekš https://discord.gg/S5PDPdDeac")
					...
					triggerClientEvent(client,"set_warning_text",client,"Login","Konts '".. username .."' neeksistē!")
				end
			else
				triggerClientEvent(client,"set_warning_text",client,"Login","Neizdevās savienoties ar serveri. Datubāzes problēma!")
				dbFree(qh)
			end
		...
		if result then
			if #result == 0 then
				...
				if (mysql:num_rows(Q2) > 0) and usernameExisted["id"] ~= "1" then
					triggerClientEvent(client,"set_warning_text",client,"Register","Vairāki konti nav atļauti! (Existed: "..tostring(usernameExisted["username"])..")")
					return false
				end
				...
				if userid then
								...
								callRemote("http://127.0.0.1:8000/api/send-activation-mail/",
									function(returns)
										if not returns.success then
											outputDebugString("MTA Activation Email Postback "..returns.error)
										end
									end,
								{id=result[1].id})
							else
					...
					triggerClientEvent(client,"set_warning_text",client,"Register","Neizdevās izveidot jaunu kontu!")
					return false
				end
			else
				triggerClientEvent(client,"set_warning_text",client,"Register","Lietotājs vai epasts jau eksistē!")
			end
		else
