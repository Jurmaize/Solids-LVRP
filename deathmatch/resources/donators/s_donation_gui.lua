...
function activateDonationPerk(thePerk, ...)
	...
	if thePerk == 13 then -- GC transfer
		...
		if (data.total <= donPoints) then
			...
			if not result then
				triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Neiznāca aizsūtīt GC. Iemesls: "..message)
				showDonationGUI(client)
				return false
			end

			addPurchaseHistory(client, "Aizsūtīts ("..data.amount.." GC uz "..data.target..")", -data.total)
			showDonationGUI(client)
			...
			if dbExec(exports.mysql:getConn("core"), "UPDATE `accounts` SET `credits`=`credits`+? WHERE `username`=? ", data.amount, data.target) then
				for i, player in pairs(getElementsByType("player")) do
					if getElementData(player, "account:username") == data.target then
						...
						triggerClientEvent(player, "displayAchievement", player, "GC SAŅEMTI!", exports.global:getPlayerFullIdentity(client, 1).." uzdāvināja tev "..data.amount.." GC.", data.amount)
						addPurchaseHistory(player, "Game Coins saņemti no "..exports.global:getPlayerFullIdentity(client, 1).." ("..data.amount.." GC)", data.amount)
						return true
					end
				end
			end
			...
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Tev nepietiek GC, lai tos sūtitu.")
			showDonationGUI(client)
			return false
		end
	elseif thePerk == 14 then -- maxints
		if (tonumber(perkDetails[2])*(tonumber(getElementData(client, "maxinteriors"))-9)*2 <= donPoints) then
			...
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Tev nav pietiekami punkti, lai aktivizētu '"..perkDetails[1] .."'")
			showDonationGUI(client)
			return false
		end
	...
	elseif thePerk == 17 then -- keypad door lock
		...
		if not canPlayerCarryLocks() then
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Kļūda aktivizējot bonusu: Tavas kabatas ir pilnas!")
			showDonationGUI(client)
			return false
		end
		...
		if not takeGC then
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Kļūda aktivizējot bonusu: "..takeGcError)
			showDonationGUI(client)
			return false
		end

		if exports.global:giveItem(client, 169, data[1]) and exports.global:giveItem(client, 169, data[1]) then
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Bonuss aktivizēts! Jūs esat saņēmis digitālās slēdzenes.")
	...
	elseif thePerk == 28 then
		if (perkDetails[2] <= donPoints) then
			...
			if not result then
				triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Kļūda aktivizējot bonusu: "..message)
				showDonationGUI(client)
				return false
			else
				showDonationGUI(client)
				triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Bonuss aktivizēts! Lūdzu dodaties uz 'Aktivizētie bonusi' sadaļu, lai izmantotu savu jauno bonusu.")
				return true
			end
		else
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Tev nav pietiekami punkti, lai aktivizētu '"..perkDetails[1] .."'")
	...
	elseif thePerk == 32 then
		...
		if currentHoursPlayed >= 15 then
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Kļūda bonusa aktivizešanā: Tev jau ir virs 15 stundām nospēlētām šajā personāžā.")
			showDonationGUI(client)
			return false
		end
		...
		showDonationGUI(client)
		triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Tavam personāžam tika uzliktas 15 nospēlētas stundas.")
	...
	elseif thePerk == 34 then --learn language
		...
		if language > 0 and language <= exports["language-system"]:getLanguageCount() then
			...
			if langName == "<Invalid/Bugged Language>" then
				triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Kļūda aktivizējot bonusu: Nederīga valoda!")
				return false
			end
				...
				if skill < 100 then
					...
					if result then
						...
						if not takeGC then
							...
							showDonationGUI(client)
							triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Tavs personāžs iemācījās "..tostring(langName).."!")
							addPurchaseHistory(client, perkDetails[1], -perkDetails[2])
							return true
						end
					else
						triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Kļūda aktivizējot bonusu: "..tostring(resultMsg))
						return false
					end
				else
		...
		if (perkDetails[2] <= donPoints) then
			...
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Tev nepietiek punktu, lai aktivizētu '"..perkDetails[1] .."'")
			showDonationGUI(client)
			return false
		end
	end
...
function removePerk(perkID)
	if not tonumber(perkID) then
		triggerClientEvent(client, "donation-system:getResponseFromServer", client, 2, "Neizdevās nosūtīt informāciju serverim.")
		showDonationGUI(client)
		return false
	else
	...
	if (hasPlayerPerk(client, perkID)) then
		...
		if gameAccountID and gameAccountID > 0 then
			...
			loadAllPerks(client)
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 2, "Bonuss noņemts.")
			showDonationGUI(client)
			return true
		else
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 2, "Neizdevās satvert konta ID.")
			showDonationGUI(client)
			return false
		end
	else
		triggerClientEvent(client, "donation-system:getResponseFromServer", client, 2, "Jums vairs nav šis bonuss.")
		showDonationGUI(client)
		return false
	end

	triggerClientEvent(client, "donation-system:getResponseFromServer", client, 2, "Kaut kas aizgāja greizi.")
	showDonationGUI(client)
	return false
end
