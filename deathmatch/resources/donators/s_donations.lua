...
function givePlayerPerk(targetPlayer, perkID, perkValue, days, points, ...)
		...
		if (gameAccountID > 0) then
			...
			if (tonumber(perkID) == 14) then -- Add an int slot
				if (characterID and tonumber(characterID) > 0) then
					...
					return true, "Bonuss aktivizēts: Maksimālais interjeru skaits palielināts uz " .. nextMaxInts .. "."
				end
			elseif (tonumber(perkID) == 15) then -- Add a vehicle slot
				if (characterID and tonumber(characterID) > 0) then
					...
					return true, "Bonuss aktivizēts: Maksimālais automašīnu skaits palielināts uz " .. currentMaxVehicles .. "."
				end
			elseif (tonumber(perkID) == 42) then -- Extra char slot
				if (characterID and tonumber(characterID) > 0) then
					...
					return true, "Bonuss aktivizēts: "..(donationPerks[tonumber(perkID)][1] or "")
				end
			elseif (tonumber(perkID) == 16) then -- Username change permit
				if (characterID and tonumber(characterID) > 0) then
					...
					if #mysqlQ ~= 0 then
						return false, "Šis vārds jau ir aizņemts"
					end
					...
					return true, "Bonuss aktivizēts: Jūsu lietotājs tika pārmainīts no '"..oldUsername.."' uz '"..username.."' veiksmīgi."
				end
			--[[
			...
			elseif (tonumber(perkID) == 18) or (tonumber(perkID) == 19) then -- Create phone with custom number.
				if (characterID and tonumber(characterID) > 0) then
					...
					if exports.mysql:num_rows(mysqlQ) ~= 0 then
						return false, "Nummurs jau ir aizņemts"
					end
					...
					if exports.global:giveItem(targetPlayer, 2, number) then
						...
						return true, "Bonuss aktivizēts: Jūs saņēmāt mobīlo ar jauno nummuru " .. number .. "."
					else
						return false, "Jūsu kabatas ir pilnas."
					end
				end
	...
	return false, "Spēlētājs nav iekš spēles"
end 
