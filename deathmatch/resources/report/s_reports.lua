...
function handleReport(reportedPlayer, reportedReason, reportType)
	...
	for _, usergroup in ipairs(staff) do
		if string.find(SUPPORTER, usergroup) then -- Supporters
			...
			outputChatBox("Paldies par ielikšanu savu reportu. (Reporta ID: #" .. tostring(slot) .. ").", source, 70, 200, 30)

			outputChatBox("Tu jebkad vari aizvērt so reportu, lietojot /er.", source, 70, 200, 30)
		elseif string.find(auxiliaryTeams, usergroup) then -- Auxiliary Teams
			...
			outputChatBox("Paldies par ielikšanu savu "..name.." reportu. (Reporta ID: #" .. tostring(slot) .. ").", source, 200, 240, 120)
			outputChatBox("Tu reportēji (" .. reportedID .. ") " .. tostring(getPlayerName(reportedPlayer)) .. ". Iemesls: ", source, 237, 145, 33 )
			outputChatBox(reportedReason, source, 200, 240, 120)
			outputChatBox("Tu jebkad vari aizvērt so reportu, lietojot /er.", source, 200, 240, 120)
			break
		else -- Admins
			local admins = exports.global:getAdmins()
			local count = 0
...
function endReport(thePlayer, commandName)
	...
	if not adminreport or not report then
		...
		outputChatBox("[" .. timestring .. "] Tu aizvēri savu reportu ar ID #"..report, thePlayer, 200, 240, 120)
