...
function F1RPhelp( key, keyState )
	...
	if not myWindow then
		...
		myWindow = guiCreateWindow ( 0, 0, 800, 600, "PowerPlay - Palīdzības centrs", false )
		...
		gui.reportWindow.main = guiCreateTab( "Reportu Centrs", tabPanel )
        -- draw the upper part of the tab.
        populateReportCenter()


		local tabCommands = guiCreateTab( "Komandas un Kontrolu palīdzība", tabPanel )
		local tabCommands2, newCmdBtn = nil, nil

		local tabRules = guiCreateTab( "Servera noteikumi", tabPanel )
		...
		local tabExplained = guiCreateTab( "Lomuspēle paskaidrota", tabPanel )
		...
		local tabOverview = guiCreateTab( "Lomuspēles pārskats", tabPanel )
		...
		if canEditCmds() then
			...
			newCmdBtn = guiCreateButton(0, 0.95, 1 , 0.05, "Izveidot jaunu komandu",true,tabCommands)
		...
		for category = 1, 7 do
			...
			col[category][2] = guiGridListAddColumn (grid[category], "Komanda", 0.15)
			col[category][3] = guiGridListAddColumn (grid[category], "Poga", 0.15)
			col[category][4] = guiGridListAddColumn (grid[category], "Paskaidrojums", 0.5)
			col[category][5] = guiGridListAddColumn (grid[category], "Atļaujas līmenis", 0.1)
			if canEditCmds() then
...
local function checkReportLength()
	guiSetText(gui.reportWindow.memo_details_desc, "Garums: " .. string.len(tostring(guiGetText(gui.reportWindow.memo_details)))-1 .. "/150")
...
local function checkNameExists()
	...
	if (count>1) then
		guiSetText(gui.reportWindow.label_player_desc, "Vairāki atrasti - Paņemsim tevi pašu.")
		guiLabelSetColor(gui.reportWindow.label_player_desc, 255, 255, 0)
	elseif (count==1) then
		guiSetText(gui.reportWindow.label_player_desc, "Spēlētājs atrasts: " .. getPlayerName(found) .. " (ID #" .. getElementData(found, "playerid") .. ")")
		guiLabelSetColor(gui.reportWindow.label_player_desc, 0, 255, 0)
		reportedPlayer = found
	elseif (count==0) then
		guiSetText(gui.reportWindow.label_player_desc, "Spēlētājs netika atrasts - Paņemsim tevi pašu.")
...
function populateReportCenter( submited )
	...
	if submited then
		local l = guiCreateLabel ( 0, 0, 1, 1, "Paldies par reporta iesneigšanu. Uzgaidiet pāris minūtes, lai varam apskatīt tavu ziņojumu.", true, gui.reportWindow.topPanel )
	...
	elseif pending_report then
		local l = guiCreateLabel ( 0, 0, 1, 1, "Tavs reports ID #" .. (pending_report[8] or "").. " vēl gaida.\nLūdzu uzgaidiet, vai arī rakstiet /er, lai aizvērtu reportu, pirms veidojat jaunu.", true, gui.reportWindow.topPanel )
		guiLabelSetVerticalAlign( l, 'center' )
		guiLabelSetHorizontalAlign( l, 'center' )
	else
		gui.reportWindow.label_type = guiCreateLabel(23, 22, 351, 22, "Par ko tu vēlies paziņot?", false, gui.reportWindow.topPanel)
        guiSetFont(gui.reportWindow.label_type, "default-bold-small")

        gui.reportWindow.combo_type = guiCreateComboBox(23, 44, 351, 27, "Reporta tips", false, gui.reportWindow.topPanel)
        ...
        gui.reportWindow.label_details = guiCreateLabel(23, 81, 351, 22, "Lūdzu pieraksti visu informāciju, kas var mums palīdzēt:", false, gui.reportWindow.topPanel)
        ...
        gui.reportWindow.label_player = guiCreateLabel(23, 265, 351, 22, "Spēlētājs, ko reportējat (nav obligāts):", false, gui.reportWindow.topPanel)
        guiSetFont(gui.reportWindow.label_player, "default-bold-small")

        gui.reportWindow.edit_player = guiCreateEdit(23, 287, 351, 34, "Spēlētāja daļējs vārds / ID", false, gui.reportWindow.topPanel)
        ...
        gui.reportWindow.btn_submit = guiCreateButton(23, 343, 143, 34, "Iesniegt", false, gui.reportWindow.topPanel)
...
function updateCmdList()
	...
	for i, cmd in ipairs(cmds) do
		...
		if canAccess or exports.integration:isPlayerScripter(localPlayer) then
			...
			guiGridListSetItemText ( grid[category], row, 2, cmd["komanda"], false, false)
			guiGridListSetItemText ( grid[category], row, 3, cmd["poga"] or "N/A", false, false)
			guiGridListSetItemText ( grid[category], row, 4, cmd["paskaidrojums"] or "N/A", false, false)
...
function openNewCommand(id, perm, cmd, key, ex)
	...
	gui.l5 = guiCreateLabel(margin, margin+lineH2, col1, lineH, "Kategorija:", false, gui.wNewStation)
	...
	gui.l2 = guiCreateLabel(margin, margin+lineH2, col1, lineH, "Komandas nos:", false, gui.wNewStation)
	...
	gui.l3 = guiCreateLabel(margin, margin+lineH2, col1, lineH, "Poga (ja ir):", false, gui.wNewStation)
	...
	gui.l4 = guiCreateLabel(margin, margin+lineH2, col1, lineH, "Paskaidrojums:", false, gui.wNewStation)
	...
	gui.bOk = guiCreateButton(margin, margin+lineH/2+lineH2, buttonW , lineH, id and "Saglabāt" or "Izveidot",false,gui.wNewStation)
	...
	gui.bDel = guiCreateButton(margin+buttonW, margin+lineH/2+lineH2, buttonW , lineH, "Izdzēst",false,gui.wNewStation)
	...
	gui.bClose1 = guiCreateButton(margin+buttonW*2, margin+lineH/2+lineH2, buttonW , lineH, "Atcelt",false,gui.wNewStation)
