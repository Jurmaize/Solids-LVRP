...
local BuyerSequence = setmetatable({
    ...
    buyerFinishedWalking = function(self)
        ...
        triggerEvent("addChatBubble", initialPed, "Vot tas ir skaists auto! Vēlies pārdot?", "say")
...
function pedGivesDriverForm()
    triggerEvent("addChatBubble", initialPed, "Super! Aizpildi šo formu un varam pa taisno iemest iekšā!", "say")
    setTimer(triggerEvent, 1000, 1, "addChatBubble", initialPed, PED_NAME .. " padod izsoles formu " .. localPlayer.name:gsub("_", " ") ..".", "ame")
end

function auctionRequestCancelled()
    triggerEvent("addChatBubble", initialPed, "Labi, kā vēlies!", "say")
    BuyerSequence:reset()
end
