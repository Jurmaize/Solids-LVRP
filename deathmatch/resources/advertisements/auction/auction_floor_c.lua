...
local function timeRemaining(expiry)
    ...
    if daysLeft > 0 then
        str = daysLeft .. (daysLeft > 1 and " Dienas" or " Diena")
    elseif hoursLeft > 0 then
        str = hoursLeft .. (hoursLeft > 1 and " Stundas" or " Stunda")
    elseif minutesLeft > 0 then
        str = minutesLeft .. (minutesLeft > 1 and " Minūtes" or " Minūte")
    else
        str = secondsLeft .. (secondsLeft > 1 and " Sekunde" or " Sekundes")
    end
...
local function renderVehicle (vehicle)
    ...
    local text = vehicle:getData("year") .. " " .. vehicle:getData("brand") .. " " .. vehicle:getData("maximemodel")
        .. "\n Apraksts: " .. vehicle:getData("auction_vehicle:description")
        .. "\n\n" .. timeRemaining(tonumber(vehicle:getData("auction_vehicle:expiry"))) .. " Atlicis"
        .. "\nŠobrīdējā likme $" .. formatNumberAsCommaSeparated(vehicle:getData("auction_vehicle:current_bid"))
        .. "\nIzpirkt $" .. formatNumberAsCommaSeparated(vehicle:getData("auction_vehicle:buyout"))


    if not isAuctioneer then
        text = text .. "\n\nNospied 'F', lai solītu"
    end
