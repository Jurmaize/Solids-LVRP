...
function onEmploymentServer()
	exports.global:sendLocalText(source, "Jessie Smith saka: Labdien, vai meklējat jaunu darbu?", nil, nil, nil, 10)
	exports.global:sendLocalText(source, " *Jessie Smith padod sarakstu ar darbiem " .. getPlayerName(source):gsub("_", " ") .. ".", 255, 51, 102)
end
...
function givePlayerJob(jobID)
	...
	if (jobID==4) then -- CITY MAINTENANCE
		...
		outputChatBox("Izmanto šo pūšamo, lai notīrītu graffitī.", source, 255, 194, 14)
...
function printJobInfo(thePlayer)
	outputChatBox("~-~-~-~-~-~-~-~-~Karjeras informācija~-~-~-~-~-~-~-~-~-~", thePlayer, 255, 194, 14)
	outputChatBox("Darbs: "..(getJobTitleFromID(getElementData(thePlayer, "job")) or "Unemployed") , thePlayer, 255, 194, 14)
	outputChatBox("Tituls/Līmenis: "..(tonumber(getElementData(thePlayer, "jobLevel")) or "0") , thePlayer, 255, 194, 14)
...
function quitJob(source)
	...
	if logged == 1 then
		...
		if job == 0 then
			outputChatBox("Tev nav darba.", source, 255, 0, 0)
		else
			local charID = getElementData(source, "dbid")
...
function startEnterVehJob(thePlayer, seat, jacked) 
	...
	if vjob>0 and job~=vjob and seat == 0 and not (getElementData(thePlayer, "duty_admin") == 1) and not (getElementData(thePlayer, "duty_supporter") == 1) then
		if (vjob==1) then
			outputChatBox("Tu neesi preču piegādātājs. Dodies uz domi, lai saņemtu šo darbu!", thePlayer, 255, 0, 0)
		elseif (vjob==2) then
			outputChatBox("Tu neesi taksists. Dodies uz domi, lai saņemtu šo darbu!", thePlayer, 255, 0, 0)
		elseif (vjob==3) then
			outputChatBox("Tu neesi autobusa šoferis. Dodies uz domi, lai saņemtu šo darbu!", thePlayer, 255, 0, 0)
		end
