...
function giveCatch(ThePlayer)
    ...
    if items:hasSpaceForItem(ThePlayer, fishItem, 1) then
        ...
        outputChatBox("Tu noķēri " .. tostring(ourFish.fishname) .. "!", ThePlayer, 0, 255, 0)
        outputChatBox("Raksti /sellfish, lai gūtu norādes, kur atrodas zivju uzpircējs Oltis!", ThePlayer, 0, 255, 0)
    end
end

function showWhereToSellFish(ThePlayer)
    outputChatBox("Atver F11 karti! Tur parādījās violets kvadrāts, kur Oltis šobrīd ir!", ThePlayer, 0, 255, 0)
    oltisBlip = createBlip(153.76, -1939.57, 3.77, 0, 2, 135, 0, 124, 255, 0, 16383, ThePlayer)
end
addCommandHandler("sellfish", showWhereToSellFish)
...
function GenerateFishPayment(ThePlayer)
    ...
    if (fishininventory == 0) then
        exports.global:sendLocalText(ThePlayer, "[English] Zvejnieks Oltis says: Es nevaru pirkt gaisu no tevis. Nepisies un ej noķer kādu zivīti.", 255, 255, 255, 10)
        return
    end
    ...
    --Generate a price 
    local perFishCost = math.random(300, 500)
...
function sellTheFish(ThePlayer, price)
    ...
    exports.global:sendLocalText(ThePlayer, "[English] Zvejnieks Oltis says: Paldies par zivīm, rekur tev naudiņa!", 255, 255, 255, 10)
