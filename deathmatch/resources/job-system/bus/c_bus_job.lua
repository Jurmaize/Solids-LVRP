...
function displayBusJob()
	...
	exports.hud:sendBottomNotification(localPlayer, "Autobusa šoferis", "Lai uzsāktu darbu, iesēdies autobusā. Autobusi ir atrodami, kur kartē rādas dzeltanais kvadrātiņš.")
end
...
function startBusJob()
	...
	if (job == 3) then
		...
		if busMarker then
			exports.hud:sendBottomNotification(localPlayer, "Autobusa šoferis", "Jūs jau esat uzsācis braukt maršrutu!")
		else
			...
			if vehicle and getVehicleController(vehicle) == getLocalPlayer() and bus[getElementModel(vehicle)] then
				...
				exports.hud:sendBottomNotification(localPlayer, "Autobusa šoferis", "Seko maršrutam apstājoties sarkanajos punktos.")
			else
				exports.hud:sendBottomNotification(localPlayer, "Autobusa šoferis", "Lai uzsāktu darbu ir jābūt autobusā!")
			end
		end
	else
		exports.hud:sendBottomNotification(localPlayer, "Paziņojums", "Tu neesi autobusa šoferis. Lai kļūtu par autobusa šoferi dodies uz domi.")
	end
end
	...
	if thePlayer == getLocalPlayer() then
		...
		if vehicle and bus[getElementModel(vehicle)] then
			if curCpType == 3 then
				...
				triggerEvent("hudOverlay:drawOverlayTopRight", thePlayer, { {"Autobusa šoferis"}, {""}, {"Gaidi pieturā līdz pazūd marķieris."}, }, false, false, false, 5)
			...
			elseif curCpType == 1 then
				...
				triggerEvent("hudOverlay:drawOverlayTopRight", thePlayer, { {"Autobusa šoferis"}, {""}, {"Uzgaidi pieturā līdz pazūd marķieris."}, }, false, false, false, 5)
				triggerServerEvent("busAdNextStop", getLocalPlayer(), line, route.points[m_number][5])
			else
				updateBusCheckpoint()
			end
		else
			exports.hud:sendBottomNotification(localPlayer, "Autobusa šoferis", "Tev ir jābūt iekš autobusa, lai uzsāktu maršrutu.") -- Wrong car type.
		end
	end
end
...
function checkWaitAtStop(thePlayer)
	if thePlayer == getLocalPlayer() then
		if busStopTimer then
			exports.hud:sendBottomNotification(localPlayer, "Autobusa šoferis", "Tu neuzgaidīji pieturā!")
...
function endOfTheLine()
	if busNextBlip then
		...
		setVehicleLocked(vehicle, false)
		exports.hud:sendBottomNotification(localPlayer, "Autobusa šoferis", "Maršruts pabeigts. Raksti /startbus, lai uzsāktu jaunu.") --if line is finished
	end
end
			...
			if(seat~=0)then
				...
				if driver then -- you can only pay the driver if the bus has a driver
					if not exports.global:hasMoney(getLocalPlayer(), 5)then
						...
						outputChatBox("Tev nav $5 priekš autobusa.", 255, 0, 0)
					else
						triggerServerEvent("payBusFare", getLocalPlayer(), driver)
						outputChatBox("Tu samaksāji $5, lai brauktu ar autobusu", 0, 255, 0)
					end
				end
			elseif not busMarker and getElementData(getLocalPlayer(), "job") == 3 then
				exports.hud:sendBottomNotification(localPlayer, "Autobusa šoferis", "Lieto /startbus, lai uzsāktu maršruta braukšanu.")
			end
		end
	end
end
...
function startEnterBus(thePlayer, seat)
	if seat == 0 and bus[getElementModel(source)] then
		if getVehicleController(source) then -- if someone try to jack the driver stop him
			cancelEvent()
			if thePlayer == getLocalPlayer() then
				exports.hud:sendBottomNotification(localPlayer, "Paziņojums", "Autobusa vadītāja durvis ir aizslēgtas!")
			end
		else
