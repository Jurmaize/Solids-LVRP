...
function serviceVehicle(veh)
	if (veh) then
		...
		if not (exports.global:hasMoney(source, mechcost) or exports.integration:isPlayerTrialAdmin(source, true)) then
			outputChatBox("Tev nav nauda detaļām, lai apkalpotu šo auto.", source, 255, 0, 0)
		else
			...
			exports.global:sendLocalMeAction(source, "apstrādā auto, to salabojot.")
			exports.logs:dbLog(source, 31, {  veh }, "REPAIR QUICK-SERVICE")
		end
	else
		outputChatBox("Tev vajag būt iekš auto, lai to apstrādātu.", source, 255, 0, 0)
	end
end
...
function changePaintjob( veh, paintjob )
	if (veh) then
		...
		if not exports.global:takeMoney(source, mechcost) then
			outputChatBox("Tev nepietiek nauda, lai pārkrāsotu auto.", source, 255, 0, 0)
		else
			...
			if setVehiclePaintjob( veh, paintjob ) then
				...
				exports.global:sendLocalMeAction(source, "pārkrāso auto.")
				exports.logs:dbLog(source, 6, {  veh }, "MODDING PAINTJOB ".. paintjob)
				exports.vehicle:saveVehicleMods(veh)
			else
				outputChatBox("Automašīnai jau ir šis paintjobs.", source, 255, 0, 0)
			end
		end
	end
end
...
function editVehicleHeadlights( veh, color1, color2, color3 )
	if (veh) then
		...
		if not (exports.global:hasMoney(source, mechcost) or exports.integration:isPlayerTrialAdmin(source, true)) then
			outputChatBox("Tev nepietiek nauda, lai modotu šo auto.", source, 255, 0, 0)
		else
			...
			if setVehicleHeadLightColor ( veh, color1, color2, color3) then
				...
				exports.global:sendLocalMeAction(source, "pārmaina auto lukturus.")
				...
				outputChatBox("Šim auto jau ir šie lukturi.", source, 255, 0, 0)
			end
		end
	end
end
...
function changeVehicleUpgrade( veh, upgrade )
	if (veh) then
		...
		if not u then
			outputDebugString( getPlayerName( source ) .. " centās uzlikt nederīgu uzlabojumu #" .. upgrade )
			return
		end
		name = u[1]
		local mechcost = u[2]
		...
		if not item and not exports.global:hasMoney( source, mechcost ) then
			outputChatBox("Tev nepietiek naudas, lai pievienotu " .. name .. " šim auto.", source, 255, 0, 0)
		else
			for i = 0, 16 do
				if upgrade == getVehicleUpgradeOnSlot( veh, i ) then
					outputChatBox("Šim auto jau ir šis tūnējums.", source, 255, 0, 0)
					return
				end
			end
			if addVehicleUpgrade( veh, upgrade ) then
				...
				exports.global:sendLocalMeAction(source, "pievienoja " .. name .. " automašīnai.")
				exports.vehicle:saveVehicleMods(veh)
				exports.logs:dbLog(source, 6, {  veh }, "MODDING ADDUPGRADE "..name)
			else
				outputChatBox("Neiznāca pievienot detaļu.", source, 255, 0, 0)
			end
		end
	end
end
