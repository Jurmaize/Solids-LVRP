...
function openMapManager()
	if not lockGui then
		...
		elseif getElementData( localPlayer, 'loggedin' ) == 1 then
		    ...
		    gui.window[1] = guiCreateWindow(651, 276, 800, 600, "Mappingu menedžeris", false)
		    ...
		    gui.tab[1] = guiCreateTab("Mani pieteikumi", gui.tabpanel[1])
	        ...
	        gui.gridlist.type = guiGridListAddColumn(gui.gridlist[1], "Tips", 0.08)
	        gui.gridlist.name = guiGridListAddColumn(gui.gridlist[1], "Nosaukums", 0.2)
	        gui.gridlist.prev = guiGridListAddColumn(gui.gridlist[1], "Pārskats", 0.1)
	        gui.gridlist.status = guiGridListAddColumn(gui.gridlist[1], "Statuss", 0.17)
	        gui.gridlist.date = guiGridListAddColumn(gui.gridlist[1], "Datums", 0.2)
	        gui.gridlist.reviewer = guiGridListAddColumn(gui.gridlist[1], "Apskatītājs", 0.1)
			gui.button.refresh = guiCreateButton(10, 499, 121, 32, "Atjaunot", false, gui.tab[1])
			...
			gui.button.close = guiCreateButton(650, 499, 121, 32, "Aizvērt", false, gui.tab[1])
		    ...
		    gui.tab[2] = guiCreateTab("Mapping pieteikums", gui.tabpanel[1])
			gui.memo[1] = guiCreateMemo(30, 272, 450, 235, "", false, gui.tab[2])
		    gui.label[1] = guiCreateLabel(20, 21, 740, 59, "* Ja vēlaties, lai Jūsu mappings nonāk serverī, lūdzu ievadiet visu nepieciešamo informāciju precīzi.\n* Jūsu pieteikumu apskatīs mappingu komanda. Jūsu pieteikuma rezultāts tiks Jums paziņots caur iekš-spēles paziņojumiem.", false, gui.tab[2])
		    guiLabelSetHorizontalAlign(gui.label[1], "left", true)
		    gui.label[2] = guiCreateLabel(20, 80, 365, 21, "Iedodiet savam mappingam īsu, korektu nosaukumu:", false, gui.tab[2])
	        guiSetFont(gui.label[2], "default-bold-small")
	        gui.edit[1] = guiCreateEdit(30, 101, 355, 26, "", false, gui.tab[2])
	        gui.label[3] = guiCreateLabel(20, 137, 365, 21, "Kam šis mappings tiks pielietots?", false, gui.tab[2])
	        guiSetFont(gui.label[3], "default-bold-small")
	        gui.edit[2] = guiCreateEdit(30, 158, 355, 26, "", false, gui.tab[2])
	        gui.label[4] = guiCreateLabel(20, 194, 740, 21, "Kādēļ šis mappings ir nepieciešams?", false, gui.tab[2])
	        guiSetFont(gui.label[4], "default-bold-small")
	        gui.edit[3] = guiCreateEdit(30, 215, 730, 26, "", false, gui.tab[2])
	        gui.label[5] = guiCreateLabel(20, 251, 740, 21, ".map faila informācija:", false, gui.tab[2])
	        guiSetFont(gui.label[5], "default-bold-small")
	        gui.label[6] = guiCreateLabel(498, 272, 262, 61, "*Atver savu .map failu izmantojot jebkuru teksta redaktoru un ielīmējiet tekstu šeit. Derīga .map faila informācija izskatas līdzīgi šim.", false, gui.tab[2])
	        ...
	        gui.label[7] = guiCreateLabel(395, 80, 365, 21, "URL uz mappinga ekrānšāviņu:", false, gui.tab[2])
	        guiSetFont(gui.label[7], "default-bold-small")
	        gui.label[8] = guiCreateLabel(395, 137, 365, 21, "Kurš lietos šo mappingu?", false, gui.tab[2])
	        ...
	        if canAccessMgmtTab( localPlayer ) then
		        ...
		        gui.gridlist.type2 = guiGridListAddColumn(gui.gridlist[2], "Tips", 0.08)
		        gui.gridlist.name2 = guiGridListAddColumn(gui.gridlist[2], "Nosaukums", 0.15)
		        gui.gridlist.prev2 = guiGridListAddColumn(gui.gridlist[2], "Pārskats", 0.1)
		        gui.gridlist.status2 = guiGridListAddColumn(gui.gridlist[2], "Statuss", 0.17)
		        gui.gridlist.date2 = guiGridListAddColumn(gui.gridlist[2], "Datums", 0.2)
		        gui.gridlist.uploader = guiGridListAddColumn(gui.gridlist[2], "Augšupielādētājs", 0.1)
		        gui.gridlist.reviewer2 = guiGridListAddColumn(gui.gridlist[2], "Apskatītājs", 0.1)
		        gui.button.refresh2 = guiCreateButton(10, 499, 121, 32, "Atjaunot", false, gui.tab[3])
		        gui.button.unload_all = guiCreateButton(140, 499, 121, 32, "Izlādēt visus testa mappingus", false, gui.tab[3])

				gui.button.close2 = guiCreateButton(650, 499, 121, 32, "Aizvērt", false, gui.tab[3])
		    ...
		    addEventHandler( 'onClientGUIClick', gui.window[1], function ()
		    	if source == gui.button[1] then
		    		...
		    		if string.len( url ) < 1 or string.len( name ) < 1 or string.len( who ) < 1 or string.len( what ) < 1 or string.len( why ) < 1 or string.len( content ) < 2 or string.len( content ) > settings.map_content_max_length then
		    			...
		    			openPopUp( 'Viens no lauciņiem bija tukšs vai arī mappings bija pa lielu. Lūdzu pārbaudiet un meiģiniet vēlreiz. ', true, false )
		    		else
		    			...
		    			if map then
		    				...
		    				openPopUp( 'Analizē mappingu...', false, true )
	        	...
	        	elseif source == gui.button.unload_all then
	        		...
	        		if res then
	        			exports.global:playSoundSuccess()
	        			openPopUp( "Izlādēja "..#res.." testa mappingus.", true, false )
	        		else
	        			exports.global:playSoundError()
		...
		if tabID == 1 then -- my req
			if not response then
				...
				if not dontShowPopUp then
					openPopUp( 'Sinhronizē...', false, true )
				end
...
function openPopUp( text, dismissable, lockMainWindow )
    ...
    gui2.label[7] = guiCreateLabel(12, 31, 344, 49, text or 'Šis ir kaut kas.', false, gui2.window[2])
...
addEventHandler( 'maps:exteriorMapRequestResponse', resourceRoot, function( response )
	if response == 'ok' then
		openPopUp( 'Jūsu mappings ir veiksmīgi iesniegts apskatei. Rezultāts tiks nosūtīts Jums iekš-spēles paziņojumos.', true, false )
		tabPopulated[ 1 ] = nil
	else
		openPopUp( response, true, false )
	end
...
local function updateReqDetailsBtn( map_id )
	...
	if gui3.button[6] and isElement( gui3.button[6] ) then
		guiSetText( gui3.button[6], exports.map_load:isMapLoaded( tonumber( map_id ) ) and "Izlādēt mappingu" or "Testēt" )
	end
...
function openReqDetails( id, reqs, tabID, silence )
	...
	if req then
		if tabID == 3 and req.uploader == getElementData( localPlayer, 'account:id' ) and not (exports.integration:isPlayerScripter( localPlayer ) or exports.integration:isPlayerMappingTeamLeader(localPlayer)) then
			exports.global:playSoundError()
			if not silence then
				openPopUp( "Tu nevari administrēt pats savu mappingu.", true, false )
			end
		else
			closeReqDetails()
			togWindow( gui.window[1], false )

		    gui3.window[2] = guiCreateWindow(581, 302, 756, 584, "Pieteikuma ID #"..req.id.." - ["..req.type.."] "..req.name, false)
			guiWindowSetSizable(gui3.window[2], false)
		    exports.global:centerWindow( gui3.window[2] )

			gui3.label[9] = guiCreateLabel(58, 55, 637, 21, "Dod savam mappingam īsu nosaukumu:", false, gui3.window[2])
			guiSetFont(gui3.label[9], "default-bold-small")
			gui3.edit[6] = guiCreateEdit(78, 76, 617, 26, req.name or "", false, gui3.window[2])
			gui3.label[10] = guiCreateLabel(58, 112, 637, 21, "URL uz mappinga ekrānšāviņu:", false, gui3.window[2])
			guiSetFont(gui3.label[10], "default-bold-small")
			gui3.edit[7] = guiCreateEdit(78, 133, 617, 26, req.preview or '', false, gui3.window[2])
			gui3.label[11] = guiCreateLabel(58, 169, 637, 21, "Kam tiks pielietots šis mappings?", false, gui3.window[2])
			guiSetFont(gui3.label[11], "default-bold-small")
			gui3.edit[8] = guiCreateEdit(78, 190, 617, 26, req.purposes or '', false, gui3.window[2])
			gui3.label[12] = guiCreateLabel(58, 226, 637, 21, "Kurš lietos šo mappingu?", false, gui3.window[2])
			guiSetFont(gui3.label[12], "default-bold-small")
			gui3.edit[9] = guiCreateEdit(78, 247, 617, 26, req.used_by or '', false, gui3.window[2])
			gui3.label[13] = guiCreateLabel(58, 283, 637, 21, "Kādēļ šis mappings ir nepieciešams?", false, gui3.window[2])
			guiSetFont(gui3.label[13], "default-bold-small")
			gui3.label[14] = guiCreateLabel(58, 381, 637, 21, tabID == 1 and "Statuss:" or "Internal note:", false, gui3.window[2])
		    ...
		    local xoffset = 0
		    local yoffset = 0
		    gui3.button[3] = guiCreateButton(58, 503, 84, 36, "Aizvērt", false, gui3.window[2])
			gui3.button[4] = guiCreateButton(152, 503, 84, 36, "Saglabāt", false, gui3.window[2])
			gui3.button[5] = guiCreateButton(246, 503, 84, 36, "Dzēst", false, gui3.window[2])
		    gui3.button[6] = guiCreateButton(340, 503, 84, 36, isTmpMapLoaded and "Izlādēt mappingu" or "Testēt", false, gui3.window[2])
	    	gui3.button[7] = guiCreateButton(434, 503, 84, 36, "Akceptēt", false, gui3.window[2])
	    	gui3.button[8] = guiCreateButton(528, 503, 84, 36, "Noraidīt", false, gui3.window[2])
	    	gui3.button[9] = guiCreateButton(622, 503, 84, 36, req.enabled == 1 and "Izslēgt" or "Ievietot", false, gui3.window[2])
		    ...
		    addEventHandler( 'onClientGUIClick', gui3.window[2], function()
		    	...
		    	elseif source == gui3.button[4] then
		    		...
		    		if string.len( name ) < 1 or string.len( prev ) < 1 or string.len( what ) < 1 or string.len( who ) < 1 or string.len( why ) < 2 or string.len( why ) > 200 then 
		    			exports.global:playSoundError()
		    			if not silence then
		    				openPopUp( "Pārbaudi ierakstīto un meiģini vēlreiz.", true, false )
		    			end
		    		else
		    			if not silence then
		    				openPopUp( "Sūta informāciju serverim...", false, true )
		    			end
		    			triggerServerEvent( 'maps:updateReq', resourceRoot, tabID, name, prev, what, who, why, id )
		    		end
		    	elseif source == gui3.button[5] then
		    		if not silence then
		    			openPopUp( 'Apstrādā...', false, true )
		    		end
		    	...
		    	elseif source == gui3.button[6] then -- test/untest
		    		if isTmpMapLoaded then
		    			...
		    			if result then
		    				if not silence then
		    					openPopUp( "Iznīcināja "..result.objects.." objektus, "..result.blips.." blipi & atjaunoja "..result.removals.." pasaules modeļus.", true, false )
		    				end
							exports.global:playSoundSuccess()
							closeReqDetails()
						else
		    			...
		    			if not silence then
		    				openPopUp( 'Satver mappinga info no servera...', false, true )
		    			end
		    	...
		    	elseif source == gui3.button[9] then -- implement/disable.
		    		...
		    		openPopUp( (req.enabled ~= 1 and "Ievieto" or "Izslēdz").." mappingu...", false, true )
...
addEventHandler( 'maps:approveRequest', resourceRoot, function( response, map_id, accepting )
	if response == 'ok' then
		openPopUp( "Request ID #"..map_id.." tika "..(accepting and "apstiprināts" or "noraidīts")..".", true, false )
		exports.global:playSoundSuccess()
		closeReqDetails()
		tabPopulated[ 3 ] = nil
...
function openMessenger( map_id, accepting )
	closeMessenger()
    gui4.window[1] = guiCreateWindow(423, 500, 454, 206, "Ziņu, ko nosūtīt spēlētājam pēc pieņemšanas/noraidīšanas:", false)
    ...
    addEventHandler( 'onClientGUIClick', gui4.window[1], function()
    	...
    	elseif source == gui4.button[2] then
    		...
    		if string.len( text ) < 2 then
    			exports.global:playSoundError()
    			openPopUp( "Ziņu, ko nosūtīt spēlētājam pēc pieņemšanas/noraidīšanas. Šī ziņa tiks iekšeji saglabāta.", true, false )
    		elseif string.len( text ) > 200 then
    			exports.global:playSoundError()
    			openPopUp( "Ziņa par lielu. (Maksimums ir 200 rakstzīmes).", true, false )
    		else
...
addEventHandler( 'maps:implement', resourceRoot, function( response, map_id, implementing )
	if response == 'ok' then
		openPopUp( "Mappinga ID #"..map_id.." tika "..(implementing and "ievietots" or "noraidīts")..".", true, false )
		exports.global:playSoundSuccess()
		closeReqDetails()
		tabPopulated[ 3 ] = nil
		populateTab( 3, nil, nil, true )
