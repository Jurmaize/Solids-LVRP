...
addEventHandler( 'maps:approveRequest', resourceRoot, function( map_id, note, accepting )
	...
	dbQuery( function( qh, client, map_id, note )
		...
		if res and nums > 0 then
			...
			notifyPlayer( map_id, "Tava mappinga pieprasījuma pieprasījums tika atjaunots.", "Sveiki <Username>!\n\nTavs mappings #" .. map_id .. " tika "..(accepting and "APSTIPRINĀTS" or "NORAIDĪTS")..".\n\n"..note.."\nAr cieņu,\nPowerPlay administrācija" )
		else
			dbFree( qh )
	...
	dbQuery( function( qh, client, map_id, note )
		...
		if res and nums > 0 then
			if implementing and exports.map_load:loadMap( map_id ) or exports.map_load:unloadMap( map_id ) then
				...
				notifyPlayer( map_id, "Tava mappinga pieprasījuma pieprasījums tika atjaunots.", "Sveiki <Username>!\n\nTavs mappings #" .. map_id .. " tika "..(implementing and "IEVIETOTS" or "IZSLĒGTS")..".\n\n"..note.."\nAr cieņu,\nPowerPlay administrācija" )
			else
