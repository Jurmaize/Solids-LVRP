...
function createLookWindow( name, description, attributes, picture, editFunction )
	...
	local column = guiGridListAddColumn( grid, "Objekti, kas ir uzvilkti (Piem: Žetoni, Ķiveres u.c.)", 0.95)
	...
	if editFunction then
		...
		local edit = guiCreateButton( x, 360, 130, 25, "Rediģēt", false, advlook_window )
		addEventHandler( "onClientGUIClick", edit, editFunction, false )

		x = x + 140
	end
	local close = guiCreateButton( x, 360, 130, 25, "Aizvērt", false, advlook_window )
		...
		addEventHandler("onClientGUIClick", browser,
			function()
				if largePictureWindow then
					...
					largePictureWindow = guiCreateWindow(x, y, width, height, "Bilde ar "..tostring(name), false)
					local close2 = guiCreateButton((width/2)-(130/2), height-30, 130, 25, "Aizvērt", false, largePictureWindow )
...
addEventHandler( "social:look", getRootElement( ),
	function( age, race, gender, weight, height, description )
		...
		local editButton = createLookWindow( currentLookData["name"],
			currentLookData["name"] .. " ir " .. ( race == 0 and "afroamerikānis" or race == 1 and "baltais" or "aziāts" ) .. " " .. ( gender == 1 and "sieviete" or "vīrietis" ) ..
			" ar svaru (" .. weight .. "kg)" ..
			" un garumu " .. height .. "cm.\n\n" ..
			"Matu krāsa: " .. description[1] .. "\n" ..
			"Matu stils: " .. description[2] .. "\n" ..
			"Sejas izskats: " .. description[3] .. "\n" ..
			"Fiziskās īpašības: " .. description[4] .. "\n" ..
			"Apģērbs: " .. description[5] .. "\n" ..
			"Aksesuāri: " .. description[6],
			description[8] or {},
			description[7],
			editFunction
		)
...
function createEditablesWindow( )
	...
	advlook_edit_window = guiCreateWindow( screen_x - 191, 172, 181, 297, "Rediģēt savu izskatu!", false )
	...
	local column = guiGridListAddColumn( grid, "Opcijas", 0.9 )
	...
	local close = guiCreateButton( 12, 266, 157, 22, "Aizvērt", false, advlook_edit_window )
...
function createEditValueWindow( key )
	...
	advlook_editvalue_window = guiCreateWindow( 349, 300, 285, 105, "Rediģēt izskatu - " .. stuff.name, false )
	...
	guiCreateLabel(11, 26, 258, 16, "Tu tagad rediģē " .. stuff.name .. ".", false, advlook_editvalue_window)

	local edit = guiCreateEdit(12, 46, 250, 22, tostring( stuff.value ),false,advlook_editvalue_window)

	local save = guiCreateButton(15,72+addY,123,21,"Saglabāt",false,advlook_editvalue_window)
	...
	local cancel = guiCreateButton(142,72+addY,123,21,"Atcelt",false,advlook_editvalue_window)
