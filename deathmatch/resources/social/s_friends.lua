...
--
local status_on = 'Ir ielogojies!'
local status_off = 'Izgāja.'
local status_msg = 'Ir atjaunojies statusa ziņu.'
local status_char = 'Tagad spēlē ar..'
...
addEventHandler( "social:remove", root,
	function( otherAccount )
		...
		if accountID and friends[ accountID ] and friends[ otherAccount ] then
			...
			if not found then
				outputChatBox( "Nav tavs draugs.", client, 255, 0, 0 )
			else
				...
				outputChatBox( "Tu noņēmi " .. friends[ otherAccount ].name .. " no sava draugu saraksta.", client, 0, 255, 0 )
...
function new_addFriend( from, to )
	...
	if fromID and friends[ fromID ] and toID and friends[ toID ] then
		...
		if onFromList then
			outputChatBox( getPlayerName( to ):gsub("_", " ") .. " ir tavā draugu sarakstā, kā " .. friends[ toID ].name .. ".", from, 255, 194, 14 )
		else
			...
			if onToList then -- the OTHER player has him on his friends list. shouldn't happen, but oh well.
				...
				outputChatBox( getPlayerName( to ):gsub("_", " ") .. " ir pievienots tavam draugu sarakstā, kā " .. friends[ toID ].name .. ".", from, 0, 255, 0 )
				...
				if getElementData(to,'social_invite_only') == '1' then
					outputChatBox(getPlayerName( to ):gsub("_", " ").." ignorē visus draugu uzaicinājumus. Tev ir jāsaņem uzaicinājums no viņa, lai draudzētos.", from, 255, 0, 0 )
				else
...
function addFriendCmd( thePlayer, commandName, targetPlayer )
   ...
   elseif targetPlayer == thePlayer then
   		return not outputChatBox( "Tu nevari pievienot sevi.", thePlayer, 255, 0, 0 )
   end
   for k, v in ipairs({fromID, toID}) do
      if not friends[ v ] then
         friends[ v ] = { }
   ...
   if fromID and friends[ fromID ] and toID and friends[ toID ] then
      ...
      if onFromList then
         outputChatBox( getPlayerName( targetPlayer ):gsub("_", " ") .. " ir tavā draugu sarakstā, kā " .. friends[ toID ].name .. ".", thePlayer, 255, 194, 14 )
      else
         local onToList = false
         for k, v in ipairs( friends[ toID ] ) do
            if v == fromID then
         ...
         if onToList then -- the OTHER player has him on his friends list. shouldn't happen, but oh well.
            mysql:query_free("INSERT INTO friends VALUES(" .. mysql:escape_string(fromID) .. ", " .. mysql:escape_string(toID) .. ")" )
            outputChatBox( getPlayerName( targetPlayer ):gsub("_", " ") .. " ir pievienots tavam draugu sarakstam, kā " .. friends[ toID ].name .. ".", thePlayer, 0, 255, 0 )
            table.insert( friends[ fromID ], toID )
            sendFriends( thePlayer, fromID )
         else
            -- need permissiosn first
            if getElementData(targetPlayer,'social_invite_only') == '1' then
            	outputChatBox(getPlayerName( targetPlayer ):gsub("_", " ").." ignorē visus ienākošos draudzības uzaicinājumus. Tev ir jāsaņem aicinājums no viņa.", thePlayer, 255, 0, 0 )
            else
...
addEventHandler( "social:acceptFriend", root,
	function( )
		...
		if not from or not to then
			...
			sendFriends( from, fromID )
			sendFriends( to, toID )
			outputChatBox( getPlayerName( to ):gsub("_", " ") .. " ir pievienots draugu sarakstam, kā " .. friends[ toID ].name .. ".", from, 0, 255, 0 )
			outputChatBox( getPlayerName( from ):gsub("_", " ") .. " ir pievienots draugu sarakstam, kā " .. friends[ fromID ].name .. ".", to, 0, 255, 0 )
		end
	end
)
