				...
				if targetPlayer then
					...
					if (logged==0) then
						...
						if not (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerScripter(thePlayer) or exports.integration:isPlayerVCTMember(thePlayer)) and (exports.integration:isPlayerMappingTeamMember(thePlayer)) then
                            ...
                            outputChatBox(username.." teleportējās pie tevis.", targetPlayer)
                            return 
                        end
						...
						if exports.integration:isPlayerSupporter(thePlayer) then
							outputChatBox(" Palīgs " .. username .. " teleportējās pie tevis. ", targetPlayer)
						else
							local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
							if hiddenAdmin == 0 then
								outputChatBox(" Administrātors " .. username .. " teleportējās pie tevis. ", targetPlayer)
							end
						end
					end
				end
		...
		if(not from or not to) then
			...
			if(fromplayer and toplayer) then
				...
				if(not logged1 or not logged2) then
					...
					local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
					if hiddenAdmin == 0 then
						outputChatBox(" Administrātors " .. admin .. " tevi teleportēja pie " .. targetPlayerName2:gsub("_"," ") .. ". ", fromplayer)
						outputChatBox(" Administrātors " .. admin .. " atteleportēja " .. targetPlayerName1:gsub("_"," ") .. " pie tevis.", toplayer)
					else
						outputChatBox(" Neredzams Administrātors tevi teleportēja pie " .. targetPlayerName2:gsub("_"," ") .. ". ", fromplayer)
						outputChatBox(" Neredzams Administrators atteleportēja " .. targetPlayerName1:gsub("_"," ") .. " pie tevis.", toplayer)
					end
				end
			end
		end
		...
		if not target then
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					if exports.integration:isPlayerSupporter(thePlayer) then
						outputChatBox(" Palīgs " .. username .. " atteleportēja Jūs pie viņa. ", targetPlayer)
					else
						local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
						if hiddenAdmin == 0 then
                                outputChatBox(" Administrātors " .. username .. " atteleportēja Jūs pie viņa. ", targetPlayer)
						end
					end
