...
function btEditRanks(button, state)
	if (source==gButtonEditRanks) and (button=="left") and (state=="up") then
		...
		wRanks = guiCreateWindow(sX, sY, wX, wY, "Pakāpju rediģēšanas sistēma", false)
		guiWindowSetSizable(wRanks, false)
		guiSetAlpha(wRanks, 1.0)
		-- Labels
		ranksLabel1 = guiCreateLabel(9, 27, 172, 15, "Frakciju rangu saraksts", false, wRanks)
		...
		ranksLabel2 = guiCreateLabel(288, 27, 142, 15, "Frakciju atļauju saraksts", false, wRanks)
		...
		ranksLabel3 = guiCreateLabel(193, 48, 335, 32, "Atļaujas ir lietas, kam šis rangs var piekļūt. Lai mainītu, izņem/ieliec ķeksīti.", false, wRanks)
		guiLabelSetHorizontalAlign(ranksLabel3, "center", true)
		-- Gridlist
		ranksGridlist = guiCreateGridList(9, 76, 175, 211, false, wRanks)
		guiGridListAddColumn(ranksGridlist, "Frakciju rangu saraksts", 0.9)
		guiGridListSetSortingEnabled(ranksGridlist, false)
		-- Buttons
		btRanksClose = guiCreateButton(489, 28, 38, 15, "Aizvērt", false, wRanks)
		btRanksMvUp = guiCreateButton(9, 292, 85, 22, "Uz augšu", false, wRanks)
		btRanksMvDn = guiCreateButton(99, 292, 85, 22, "Uz leju", false, wRanks)
		btRanksUpdOrd = guiCreateButton(9, 48, 175, 22, "Atjaunot rangu sarakstu", false, wRanks)
		btRanksAdd = guiCreateButton(9, 320, 55, 22, "Pievienot", false, wRanks)
		btRanksRem = guiCreateButton(129, 320, 55, 22, "Noņemt", false, wRanks)
		btRanksRen = guiCreateButton(69, 320, 55, 22, "Pārsaukt", false, wRanks)
		btRanksApply = guiCreateButton(284, 313, 158, 30, "Atjaunot ranga opcijas", false, wRanks)
...
function saveRanks(button, state)
	if (source==bRanksSave) and (button=="left") and (state=="up") then
		...
		if (found) then
			outputChatBox("Tavos rangos ir nederīgi simboli, pārliecinies, ka tajos nav simboli, kā: '@.;", 255, 0, 0)
		elseif not (isNumber) then
			outputChatBox("Tavas uzliktās algas ir nederīgas. Pārliecinies, ka tajās ir cipari un nav valūtas simboli.", 255, 0, 0)
		else
			local sendRanks = { }
			local sendWages = { }
...
function btAddRanks()
	...
	wAddRank = guiCreateWindow(sX, sY, wX, wY, "Pievienot frakcijas rangu...", false)
	guiWindowSetSizable(wAddRank, false)
	guiSetAlpha(wAddRank, 1.00)
	-- Labels
	lAddRank1 = guiCreateLabel(9, 74, 223, 15, "Ranga nosaukums", false, wAddRank)
	guiLabelSetHorizontalAlign(lAddRank1, "center", false)
	lAddRank2 = guiCreateLabel(9, 26, 224, 15, "Mantot atļaujas no...", false, wAddRank)
	...
	-- Buttons
	bAddRankCreate = guiCreateButton(250, 35, 85, 28, "Izveidot", false, wAddRank)
	guiSetFont(bAddRankCreate, "default-bold-small")
	guiSetProperty(bAddRankCreate, "NormalTextColour", "FFAAAAAA")
	bAddRankClose = guiCreateButton(250, 78, 85, 28, "Aizvērt", false, wAddRank)
...
function addRank()
	...
	if (rank == "") then
		outputChatBox("Ierakstiet ranga nosaukumu.", 255, 100, 100)
		return
	end
...
function btRenameRank()
	...
	wRenameRank = guiCreateWindow(sX, sY, wX, wY, "Pārsaukt frakcijas rangu", false)
	guiWindowSetSizable(wRenameRank, false)
	-- Label
	lRenameRank1 = guiCreateLabel(-18, 28, 277, 15, "Izmainīt '"..currentRank.."' ranga nosaukumu uz:", false, wRenameRank)
	...
	-- Button
	btRenameRankChange = guiCreateButton(53, 77, 63, 26, "Mainīt", false, wRenameRank)
	guiSetProperty(btRenameRankChange, "NormalTextColour", "FFAAAAAA")
	btRenameRankCancel = guiCreateButton(125, 77, 63, 26, "Atcelt", false, wRenameRank)
...
function btRemoveRank()
	...
	wRemoveRank = guiCreateWindow(660, 366, 285, 138, "Frakcijas ranga dzēšana", false)
	guiWindowSetSizable(wRemoveRank, false)
	-- Labels
	local row = guiGridListGetSelectedItem(ranksGridlist)
	local rankName = guiGridListGetItemText(ranksGridlist, row, 1)
	lRemoveRank1 = guiCreateLabel(26, 24, 225, 30, "Vai esat pārliecināts, ka vēlaties dzēst \n'"..rankName.."' rangu?", false, wRemoveRank)
	...
	lRemoveRank2 = guiCreateLabel(10, 58, 257, 31, "Visiem biedriem ar šo rangu, tiks izsneigts\n'"..def_rank.."' rangs.", false, wRemoveRank)
	guiLabelSetHorizontalAlign(lRemoveRank2, "center", false)
	-- Buttons
	btRemoveRankDel = guiCreateButton(68, 96, 67, 29, "Dzēst", false, wRemoveRank)
	guiSetProperty(btRemoveRankDel, "NormalTextColour", "FFAAAAAA")
	btRemoveRankCancel = guiCreateButton(141, 96, 67, 29, "Atcelt", false, wRemoveRank)
...
function setRankInfo(rankTbl, rankperms, perms, wageTable)
	...
	if not isElement(ranksLabel5) then
		ranksLabel5 = guiCreateLabel(0, 0, 337, 15, "Izvēlaties rangu, lai mainītu tā atļaujas", false, ranksScrollPane)
	...
	for i,perm in ipairs(perms) do
		...
		if #perms == i  then
			ranks_gui.label[1] = guiCreateLabel(0, (i*25+25), 39, 15, "Alga:", false, ranksScrollPane)
...
function selectRankOffList()
	...
	guiSetText(ranksLabel5, "Maina atļaujas priekš '"..ranks[row+1][2].."'")
end
...
function applyPermissions()
	...
	if (not row or row == -1) then 
		outputChatBox("Izvēlaties lūdzu rangu.", 255, 100, 100)
	return end
	local rankID = ranks[row+1][1]
...
function updateRankOrder()
	...
	if (source == btRanksMvUp or source == btRanksMvDn) then
		local row = guiGridListGetSelectedItem(ranksGridlist)
		if (not row or row == -1) then 
			outputChatBox("Izvēlaties lūdzu rangu.", 255, 125, 0)
		return end
...
function renameRank()
	...
	if (rankName == "") then
		outputChatBox("Ievadiet ranga nosaukumu.", 255, 100, 100)
		return
	end
