...
addEventHandler("faction-system.addFactionRank", root, function(rank, perms, factionID)
	...
	for i,rankID in ipairs(getFactionRanks(fID)) do
		...
		if (rankName == rank) then
			outputChatBox("Rangs ar šo nosaukumu jau eksistē. Izvēlaties citu", client, 255, 100, 100)
			return
		end
	end
	...
	outputChatBox("Frakcijas rangs '"..rank.."' veiksmīgi izveidots.", client, 255, 100, 100)
...
addEventHandler("faction-system.setFactionRankName", root, function(rankID, rankName, factionID)
	...
	outputChatBox("Rangs '"..oldName.."' tika pārsaukts uz '"..rankName.."'", client, 255, 100, 100)
	FactionRanks[rankID]["name"] = rankName
	local factionRanks = {}
	local theTeam = getFactionFromID(factionID)
	for i,v in pairs(getElementData(theTeam, "ranks")) do
...
addEventHandler("faction-system.removeFactionRank", root, function(rankID, factionID)
	...
	outputChatBox("Rangs '"..rankName.."' tika veiksmīgi izdzēsts.", client, 255, 100, 100)
...
addEventHandler("faction-system.updateRankPermissions", root, function(rankID, permissions, wage, factionID)
	...
	if factionID == 1 then -- Max Payday per faction. - Solid 28/11/2021 - 1 LSPD (Police) - 2 LSFD/ES (Fire Dep / Emergancy Services) - 3 LSG (Government) - 4 LSRT (Rapid Towing)
	    newWage = math.min(3500, math.max(0, tonumber(wage) or 0)) -- You get the ability to set max pay per faction. So LSPD who do their job can get a max pay of 2k
	elseif factionID == 2 then                                     -- but lazy ass tow truck drivers can only get 1k maximum.
	    newWage = math.min(3000, math.max(0, tonumber(wage) or 0))
    elseif factionID == 3 then
        newWage = math.min(3000, math.max(0, tonumber(wage) or 0))
    elseif factionID == 4 then
        newWage = math.min(1850, math.max(0, tonumber(wage) or 0))
    else
        newWage = math.min(1000, math.max(0, tonumber(wage) or 0))
    end

	setAllRankPermissions(rankID, permissions, newWage, factionID)
	triggerClientEvent("faction-system.cacheRanks", root, FactionRanks)
	outputChatBox("Atļaujas priekš '"..rankName.."' veiksmīgi izmainītas.", client, 255, 100, 100)
end) 
