...
function spawnRoute(thePlayer, spawnNewRoute)
	...
	if currentSpot and not spawnNewRoute then
		...
		if not selectedRoute then
			exports.hud:sendBottomNotification(thePlayer, "RS Haul Operators:", "Šobrīd nav nevian pasūtījuma, uzgaidi nedaudz...")
		else
...
function giveTruckingMoney(vehicle)
	...
	if takenWeight then
		exports.hud:sendBottomNotification(source, "RS Haul Operators:", "RS Haul has unloaded. "..takenWeight.." Kg preču palika aizmugurē. ")
	end
	-- level up and reset runs/wage
	local vehicle = getPedOccupiedVehicle(source)
	if getElementData(vehicle, "job") ~= 1 then
		exports.hud:sendBottomNotification(source, "RS Haul Operators:", "Vecīt, tev jālieto RS Haul transports.")
	else
			...
			if truckruns > 0 then
				if truckrunsTilNextLevel then
					...
					if truckrunCarry >= 0 then -- level up
						...
						local info = {
							{string.upper("Preču piegādātāja jauns sasneigums!"), 255,194,14,255,1,"default-bold"},
							{""},
							{"Apsveicam! Tu esi dabūjis jaunu preču piegādātāja līmeni "..tostring(currentJobLevel+1)..".", 0,255,0,255,1,"default"},
						}
				...
				if not notified then
					if not truckrunsTilNextLevel then
						exports.hud:sendBottomNotification(source, "Jauns Sasniegums!", "Progress: "..(getElementData(source, "jobProgress") or 0).." kravas aizvestas (Meistars :) ).")
					else
						exports.hud:sendBottomNotification(source, "Jauns Sasniegums!", "Progress: "..math.floor((getElementData(source, "jobProgress") or 0)/truckrunsTilNextLevel*100).."%")
					end
				end
				playSoundFX(vehicle)
			end
	...
	if getElementData(source,"job") == 1 and truckerJobVehicleInfo[truckModel] then
		...
		if playerJobLevel < truckLevelRequire then
			...
			if truckLevelRequire == 1 then
				exports.hud:sendBottomNotification(thePlayer, "RS Haul Operators:", "Tu neesi RS Haul Darbinieks, lūdzu reģistrējies šim darbam domē.")
			else
				exports.hud:sendBottomNotification(thePlayer, "RS Haul Operators:", "Tev nepieciešams  "..truckLevelRequire.." prasmju līmenis, lai brauktu ar "..truckName..".")
			end
...
function checkTruckingEnterVehicle(thePlayer, seat)
	if seat == 0 and getElementData(source,"job") == 1 and getElementData(thePlayer,"job") == 1 then
		...
		if curentCrates <= 0 then
			exports.hud:sendBottomNotification(thePlayer, "RS Haul Operators: Tavs busiņš ir tukšs!", "Dodies atpakaļ uz RS Haul noliktavu, lai ielādētu vēl preces!")
		else
			exports.hud:sendBottomNotification(thePlayer, "RS Haul Operators: ", "Tavā busiņā ir "..exports.global:formatWeight(curentCrates).." preču. Nogādā tās mūsu klientiem!")
...
function startLoadingUp()
	local vehicle = getPedOccupiedVehicle(source)
	if not vehicle then
		exports.hud:sendBottomNotification(source, "RS Haul Operators: ", "Vecīt, kur ir tavs busiņš?")
		return false
	end
	if getElementData(vehicle, "job") ~= 1 then
		exports.hud:sendBottomNotification(source, "RS Haul Operators: ", "Vecīt, tev jālieto RS Haul transports.")
	...
	if truck then
		...
		if crateWeight > 0 then
			...
			exports.hud:sendBottomNotification(source, "RS Haul Operators: ", "Tavs transports ir pilns ar precēm! Brauc uz dzeltenajiem un orandžajiem punktiem, lai piegādātu preces.")
			triggerClientEvent(source, "job-system:trucker:leaveStationLoadup", source, true)
		end
...
function checkIfPlayerTruckHasEnoughtShit()
	...
	local truck = truckerJobVehicleInfo[truckModel]
	if not truck then
		exports.hud:sendBottomNotification(source, "RS Haul Operators: ", "Vecīt, RS Haul neatļauj šāda modeļa transportus.")
		return false
	end
	...
	if success then
		...
		exports.hud:sendBottomNotification(source, "RS Haul Operators: ", "Pircējs tev samaksāja $"..formartedEarned.." par aizvešanu "..exports.global:formatWeight(droppedWeight).." produktu, "..exports.global:formatLength(distance)..".")
	else
		exports.hud:sendBottomNotification(source, "RS Haul Operators: ", marker.." Vispirms atgriezies uz RS Haul noliktavu.")
	end
...
function unloadCrates(thePlayer, veh)
	if getVehicleOccupant(veh) == thePlayer then
		...
		local truck = truckerJobVehicleInfo[truckModel]
		if not truck then
			return false, "Vecīt, RS Haul neatļauj šāda modeļa transportus."
		end
		local currentSpot = handledSpot(thePlayer)
		if currentSpot then
			...
			if whatPlayerHas > 0 then
				...
				return false, "Tavs businš ir tukšs."
			end
		else
			return false, "((Script Error Code 407))"
		end
	else
		return false, "Tu nevari lietot citu cilvēku busiņus."
	end
