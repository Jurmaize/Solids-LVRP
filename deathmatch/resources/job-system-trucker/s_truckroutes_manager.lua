...
function selectAFreeSpot(thePlayer)
	...
	if route[8] and tonumber(route[8]) and tonumber(route[8]) > 0 then -- if it is an actual order from other players
		isActualOrder = "Īsts pasūtījums (Priekš veikala ID#"..route[8]..")"
		-- notify truckers
		local jobLevel = getElementData(thePlayer, 'jobLevel')
		notifyTruckers("RS Haul Operators: Pasūtījums #"..routes[rand][7].." - Jāizved uz "..routes[rand][6].." tika piešķirts darbiniekam "..jobLevel.." - "..exports.global:getPlayerName(thePlayer)..".", true)
	else
