...
function getRandomRequiredWeight(model) 
	...
	local from_10_to_30_percent = math.random(5,10)/100
...
function calculateEarning(player, supplies, veh_hp, distance)
	-- core formula
	local earning = supplies*veh_hp*distance                                                                    --200*1000*2200 = 440'000'000
	
	-- reduce it to something realistic & consider hoursplayed too
	local multifier_min = 750000                                                                                -- 750'000
	local multifier_max = multifier_min*8                                                                       -- 6'000'000
	local multifier_gap = multifier_max-multifier_min                                                           -- 6'000'000 - 750'000 = 5'250'000

	local effected_hours_range = 1000
	local hoursplayed = math.min(getElementData(player, 'hoursplayed') or 0, effected_hours_range)              -- 30
	local additional = hoursplayed/effected_hours_range*multifier_gap                                           -- 30 / 1000 * 5'250'000 = 157500
	local devider = multifier_min+additional                                                                    -- 750'000 + 157500 = 907500
	earning = math.ceil(earning/devider)                                                                        -- 440'000'000 / 907500 = 484

	--Cap at around $500~$700 max
	if earning > 350 then
		earning = 350 + math.random(0,200)
	end
