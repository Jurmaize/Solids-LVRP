...
function sellProperty(thePlayer, commandName, bla)
	if bla then
		outputChatBox("Lieto /sell, lai pārdotu šo īpašumu citam spēlētājam..", thePlayer, 255, 0, 0)
		return
	end
	...
	if dbid > 0 then
		if interiorType == 2 then
			outputChatBox("Tu nevari pārdot valdības īpašumu.", thePlayer, 255, 0, 0)
		elseif interiorType ~= 3 and commandName == "unrent" then
			outputChatBox("Tu neīrē šo īpašumu.", thePlayer, 255, 0, 0)
		else
			...
			if interiorStatus.owner == getElementData(thePlayer, "dbid") or ( leader and faction ) then
				...
				outputChatBox("Tev nepieder šis īpašums.", thePlayer, 255, 0, 0)
			end
		end
	else
		outputChatBox("Tu neesi īpašumā.", thePlayer, 255, 0, 0)
	end
end
...
function publicSellProperty(thePlayer, dbid, showmessages, givemoney, CLEANUP)
	...
	if query then
		...
		if interiorType == 0 or interiorType == 1 then
			...
			if interiorStatus.owner == getElementData(thePlayer, "dbid") then
				...
				if showmessages then
					...
					elseif givemoney then
						outputChatBox("Tu pārdevi savu īpašumu pa " .. exports.global:formatMoney(money) .. "$.", thePlayer, 0, 255, 0)
					else -- Token
						outputChatBox("Tu pārdevi savu īpašumu pa 0$, jo tas tika iegādāts izmantojot tokenu.", thePlayer, 0, 255, 0)
					end
				end
			...
			elseif exports.factions:isPlayerInFaction(thePlayer, interiorStatus.faction) then
				...
				if showmessages then
					...
					elseif faction then
						if givemoney then
							outputChatBox("Tu pārdevi savu īpašumu pa " .. exports.global:formatMoney(money) .. "$ (Aizsūtīts uz banku '"..getTeamName(faction).."')", thePlayer, 0, 255, 0)
						else
							outputChatBox("Tu pārdevi savas frakcijas īpašumu pa 0$, jo tas tika iegādāts ar tokenu.", thePlayer, 0, 255, 0)
						end
					end
				end
				...
				if showmessages then
					if CLEANUP == "FORCESELL" then
						...
						outputChatBox("Tu uzsetoji, ka šim īpašumam nav īpašnieka.", thePlayer, 0, 255, 0)
					end
				end
			end
		else
			if showmessages then
				if CLEANUP == "FORCESELL" then
					...
					outputChatBox("Tu vairs neīrē šo īpašumu!", thePlayer, 0, 255, 0)
				end
			end
			-- take all keys
			call( getResourceFromName( "item-system" ), "deleteAll", interiorType == 0 and 4 or 5, dbid )
		...
		outputChatBox("Error 504914 - Bugs! Reporto to šeit: https://discord.gg/S5PDPdDeac", thePlayer, 255, 0, 0)
	end
end
...
function unownProperty(intid, reason) --This function is meant to be used by the system or to be triggered from UCP remotely. Works similarly to the publicSellProperty however, it's simplier. / MAXIME / 2015.1.11
	if intid and tonumber(intid) then
		...
		return false, "Interjers neeksistē vai nestrādā"
	end
	...
	if int and int.id ~= mysql_null() then
		...
		return false, "Interjers nav datubāzē."
	end
...
function sellTo(thePlayer, commandName, targetPlayerName)
	...
	if dbid > 0 and not isPedInVehicle( thePlayer ) then
		...
		elseif not targetPlayerName then
			...
			outputChatBox("Pārdod īpašumu citam spēlētājam.", thePlayer, 255, 194, 14)
			outputChatBox("Prasi īpašniekam, lai raksta /buy, lai saņemtu naudu pa īpašumu", thePlayer, 255, 194, 14)
		else
			...
			if targetPlayer and getElementData(targetPlayer, "dbid") then
				...
				if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) < 10 and getElementDimension(targetPlayer) == getElementDimension(thePlayer) then
					if not exports.global:canPlayerBuyInterior(targetPlayer) then
						outputChatBox(targetPlayerName .. " jau pieder pārāk daudz interjeri.", thePlayer, 255, 0, 0)
						outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " centās tev pārdot interjeru, bet tev jau ir pārāk daudz interjeri.", targetPlayer, 255, 0, 0)
						return false
					end
					if interiorStatus.tokenUsed then
						outputChatBox("Šis īpašums tika iegādāts ar tokeniem, tādēļ jālieto /sellproperty.", thePlayer, 255, 0, 0)
						return
					end
						...
						if getElementData(targetPlayer, "dbid") ~= interiorStatus.owner then
							if exports.global:hasSpaceForItem(targetPlayer, 4, dbid) then
								...
								if query then
									...
									if interiorType == 0 or interiorType == 1 then
										outputChatBox("Tu veiksmīgi pārdevi savu īpašumu  " .. targetPlayerName .. ".", thePlayer, 0, 255, 0)
										outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " sold you this property.", targetPlayer, 0, 255, 0)
									else
										outputChatBox(targetPlayerName .. " ir pārņemis tavu īres līgumu.", thePlayer, 0, 255, 0)
										outputChatBox("Tu pārņēmi " .. getPlayerName(thePlayer):gsub("_", " ") .. "īres līgumu.",  targetPlayer, 0, 255, 0)
									end
									...
									outputChatBox("Error 09002 - Bugs! Reporto to: https://discord.gg/S5PDPdDeac", thePlayer, 255, 0, 0)
								end
							else
								outputChatBox(targetPlayerName .. " nav vietas īpašuma atslēgam.", thePlayer, 255, 0, 0)
							end
						else
							outputChatBox("Tu nevari pārdot īpašumu sev... :/.", thePlayer, 255, 0, 0)
						end
					else
						outputChatBox("Šis īpašums nepieder tev.", thePlayer, 255, 0, 0)
					end
				else
					outputChatBox("Tu esi pārāk tālu no " .. targetPlayerName .. ".", thePlayer, 255, 0, 0)
				end
			end
		end
	end
...
function buyInterior(player, pickup, cost, isHouse, isRentable)
	if not exports.global:canPlayerBuyInterior(player) then
		outputChatBox("Tev jau pieder pārāk daudz interjeri.", player, 255, 0, 0)
		return false
	end
	...
	if isRentable then
		...
		if result then
			...
			if count ~= 0 then
				outputChatBox("Tu jau īrē citu māju!", player, 255, 0, 0)
				return
			end
		end
	elseif not exports.global:hasSpaceForItem(player, 4, 1) then
		outputChatBox("Tev nav vietas atslēgām.", player, 255, 0, 0)
		return
	end

	if exports.global:takeMoney(player, cost) then
		if (isHouse) then
			outputChatBox("Apsveicam! Tu esi nopircis māju pa $" .. exports.global:formatMoney(cost) .. ".", player, 255, 194, 14)
			exports.global:giveMoney( getTeamFromName("Los Santos Valdība"), cost )
		elseif (isRentable) then
			outputChatBox("Apsveicam! Tu īrē šo īpašumu pa $" .. exports.global:formatMoney(cost) .. ".", player, 255, 194, 14)
		else
			outputChatBox("Apsveicam! Tu iegādājies šo biznesu pa $" .. exports.global:formatMoney(cost) .. ".", player, 255, 194, 14)
			exports.global:giveMoney( getTeamFromName("Los Santos Valdība"), cost )
		end
		...
		exports["interior-manager"]:addInteriorLogs(pickupid, "Nopirkts/Izīrēts, $"..exports.global:formatMoney(cost)..", "..getPlayerName(thePlayer), thePlayer)
	else
		outputChatBox("Atvaino, bet tev nav ģenga, lai nopirktu šo.", player, 255, 194, 14)
	end
end
...
function buypropertyForFaction(interior, cost, isHouse, furniture, factionName)
	...
	if not exports.global:takeMoney(theFaction, cost) then
		outputChatBox("Nevarēja izņemt naudu no frakcijas bankas", source, 255, 0, 0)
		return
	end
	local gov = getTeamFromName("Los Santos Valdība")
	...
	outputChatBox("Apsveicam! Tu esi iegādājies īpašumu savai frakcijai '"..factionName.."' pa $" .. exports.global:formatMoney(cost) .. ".", source, 255, 194, 14)
	...
	exports["interior-manager"]:addInteriorLogs(intId, "Nopirka frakcijai '"..factionName.."', $"..exports.global:formatMoney(cost)..", "..getPlayerName(source), source)
...
function buyInteriorCash(pickup, cost, isHouse, isRentable, furniture)
	if not exports.global:canPlayerBuyInterior(client) then
		outputChatBox("Tu jau esi sasniedzis maksimālo daudzumu interjeru! Tu vari palielināt maksimalo daudzumu ejot uz F10 -> Ziedotāju panelis.", client, 255, 0, 0)
		return
	end
	...
	if isRentable then
		...
		if result then
			local count = tonumber(result['cntval'])
			if count ~= 0 then
				outputChatBox("Tu jau īrē citu īpašumu!", client, 255, 0, 0)
				return
			end
		end
	elseif not exports.global:hasSpaceForItem(client, 4, 1) then
		outputChatBox("Tev nav vieta atslēgām.", client, 255, 0, 0)
		return
	end
	...
	if exports.global:takeMoney(client, cost) then
		...
		local gov = getTeamFromName("Los Santos Valdība")
		if (isHouse) then
			outputChatBox("Apsveicam! Tu nopirki šo māju pa $" .. exports.global:formatMoney(cost) .. ".", client, 255, 194, 14)
			exports.global:giveMoney( gov, cost )
			exports.bank:addBankTransactionLog(charid, -(getElementData(gov, "id")), cost, 5 , "Interior Purchase", intName.." (ID: "..pickupid..")" )
		elseif (isRentable) then
			outputChatBox("Apsveicam! Tu izīreji šo īpašumu pa $" .. exports.global:formatMoney(cost) .. ".", client, 255, 194, 14)
		else
			outputChatBox("Apsveicam! Tu iegādājies šo biznesu pa $" .. exports.global:formatMoney(cost) .. ".", client, 255, 194, 14)
			exports.global:giveMoney( getTeamFromName("Los Santos Valdība"), cost )
			exports.bank:addBankTransactionLog(charid, -(getElementData(gov, "id")), cost, 5 , "Interior Purchase", intName.." (ID: "..pickupid..")" )
		end
		...
		outputChatBox("Atvaino, tev nav ģenga, lai iegādātos šo īpašumu.", client, 255, 194, 14)
	end
end
...
function buyInteriorBank(pickup, cost, isHouse, isRentable, furniture)
	...
	elseif not exports.global:hasSpaceForItem(client, 4, 1) then
		outputChatBox("Tev nav vieta atslēgai!", client, 255, 0, 0)
		return
	end

	if not exports.bank:takeBankMoney(client, cost) then
		outputChatBox( "Tev nav naudas bankā, lai šo īpašumu iegādātos", client, 255, 0, 0 )
	else
		local charid = getElementData(client, "dbid")
		local pickupid = getElementData(pickup, "dbid")
		local gov = getTeamFromName("Los Santos Valdība")
		...
		if (isHouse) then
			...
			exports.global:giveMoney( gov, cost )
		elseif (isRentable) then
			outputChatBox("Apsveicam! Tu izīreji šo māju pa $" .. exports.global:formatMoney(cost) .. ".", client, 255, 194, 14)
		else
			outputChatBox("Apsveicam! Tu nopirki šo biznesu pa $" .. exports.global:formatMoney(cost) .. ".", client, 255, 194, 14)
			exports.global:giveMoney( gov, cost )
		end
		...
		exports.logs:dbLog(client, 37, { "in"..tostring(pickupid) } , "BUYPROPERTY $"..cost)
		exports["interior-manager"]:addInteriorLogs(pickupid, "Nopirka/Izīrējat, $"..exports.global:formatMoney(cost)..", "..getPlayerName(client), client)
...
function buyInteriorToken(pickup, furniture)
	...
	if not exports.global:takeItem(client, 262) then
		outputChatBox( "Tev nav tokens, lai iegādātos šo īpašumu ar to", client, 255, 0, 0 )
	else
		local charid = getElementData(client, "dbid")
		local pickupid = getElementData(pickup, "dbid")
		outputChatBox("Apsveicam! Tu izmantoji tokenu, lai iegādātos šo īpašumu. Tam nav nekāda vērtība un to nevar pārdot draugiem.", client, 255, 194, 14)
...
function moveSafe ( thePlayer, commandName )
	...
	if (dbid < 19000 and (exports.global:hasItem( thePlayer, 5, dbid ) or exports.global:hasItem( thePlayer, 4, dbid))) or (dbid >= 20000 and exports.global:hasItem(thePlayer, 3, dbid - 20000)) then
		if getPedContactElement(thePlayer) == safeTable[dbid] or getPedContactElement(thePlayer) == exports.vehicle:getSafe(dbid-20000) then
            outputChatBox("Lūdzu aizejat uz citu pozīciju, lai noliktu seifu.", thePlayer, 255, 0, 0)
        else
			z = z - 0.5
			rotz = rotz + 180
			...
			elseif dbid > 0 and getSafe( dbid ) then
				if not updateSafe( dbid, { x, y, z }, rotz ) then
					outputChatBox("Kļūda! Neizdevās.", thePlayer, 255, 0, 0)
				end
			else
				outputChatBox("Tev vajag seifu, ko nolikt!", thePlayer, 255, 0, 0)
			end
		end
	else
		outputChatBox("Tev vajag īpašuma atslēgu, lai pārvietotu seifu", thePlayer, 255, 0, 0)
	end
		...
		if locked == 0 then
			...
			if byAdmin then
				if getElementData(source, "hiddenadmin") == 0 then
					...
					exports.global:sendLocalText(source, " * Durvīm vajadzētu būt vaļā. *", 255, 51, 102, 30, {}, true)
					exports["interior-manager"]:addInteriorLogs(itemValue, "unlock without key", source)
				end
			else
				triggerEvent('sendAme', source, "ieliek atslēgu durvīs tās atslēdzot.")
			end
			...
			if byAdmin then
				if getElementData(source, "hiddenadmin") == 0 then
					...
					exports.global:sendLocalText(source, " * Durvīm vajadzētu būt aizslēgtām. *", 255, 51, 102, 30, {}, true)
					exports["interior-manager"]:addInteriorLogs(itemValue, "lock without key", source)
				end
			else
				triggerEvent('sendAme', source, "ievieto atslēgu durvīs, to aizslēdzot.")
			end
			...
			if not usingKeypad then
				...
				if locked == 0 then
					if byAdmin then
						...
						triggerEvent('sendAme', source, "ievieto atslēgu durvīs tās atslēdzot.")
					end
					...
					if byAdmin then
						...
						triggerEvent('sendAme', source, "ievieto atslēgu durvīs tās aizslēdzot.")
					end
				...
				if locked == 0 then
					triggerEvent('sendAme', source, "atslēdz durvis.")
					exports.logs:dbLog(source, 31, {  "in"..tostring(id) }, "UNLOCK INTERIOR")
				else
					newRealLockedValue = true
					triggerEvent('sendAme', source, "aizslēdz durvis.")
					exports.logs:dbLog(source, 31, {  "in"..tostring(id) }, "LOCK INTERIOR")
				end
...
function timedInteriorView(houseID)
	...
	if entrance then
		...
		outputChatBox( "Tu šobrīd apskati īpašumu. Tu nevari nomest mantas. Tu vari beigt skatīt interjeru izejot ārā vai izgaidot 60 sekunžu taimeri.", client, 0, 255, 0)
		...
		if getElementDimension(client) > 0 then
			...
			outputChatBox( "Ja tu vēlies apskatīt īpašumu no citiem leņķiem, lūgums lietot /freecam.", client, 0, 255, 0 )
		end
...
function endTimedInteriorView(thePlayer, changedCharacter )
	...
	if info and pos then
		...
		if entrance then
			...
			outputChatBox( "Tavs apskatīšanas laiks ir beidzies.", thePlayer, 0, 255, 0 )
		else
