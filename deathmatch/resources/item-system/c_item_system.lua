...
--function useItem(button)
	...
	if itemSlot > 0 then -- ITEMS
		...
		elseif (itemID==34) then -- COCAINE
			triggerEvent("soliddrugs:clientTakeDrugs", localPlayer, 2, tonumber(string.match(itemValue , "%d+")))
		elseif (itemID==35) then
			doDrug2Effect()
		elseif (itemID==36) then
			doDrug3Effect()
		elseif (itemID==37) then
			triggerEvent("soliddrugs:clientTakeDrugs", localPlayer, 3, tonumber(string.match(itemValue , "%d+")))
		...
		elseif itemID == 300 then
			triggerEvent("solidnews:openBrowser", localPlayer, 1)
		elseif itemID == 301 then
			triggerEvent("solidnews:openBrowser", localPlayer, 2)
		elseif itemID == 305 then 
			triggerServerEvent("crushAspirin", localPlayer)
		else
			triggerServerEvent("useItem", localPlayer, itemSlot, additional)
		end
	else
