...
armor = {
	[219]  = {49, "type2", "paņem bruņuvesti un pārliek to pāri savai galvai", "satver savu bruņuvesti un novelk to"},
	[220]  = {74, "type3", "paņem bruņuvesti un pārliek to pāri savai galvai", "satver savu bruņuvesti un novelk to"},
	[221]  = {100, "type4", "paņem bruņuvesti un pārliek to pāri savai galvai", "satver savu bruņuvesti un novelk to"},
}
...
function useItem(itemSlot, additional)
	...
	if itemID then
		if (itemID==1) then -- hotdog
			...
			triggerEvent('sendAme', source, "apēd hotdogu.")
			...
			if veh and getElementData(veh, "dbid") == itemValue then
				...
				if value then
					...
					if getDistanceBetweenPoints3D(x, y, z, vx, vy, vz) <= 30 then -- car found
						...
						outputChatBox("Jūs esat pārāk tālu no auto.", source, 255, 194, 14)
					end
				else
					outputChatBox("Nederīgs auto.", source, 255, 194, 14)
				end
			end
		elseif (itemID==4) or (itemID==5) then -- house key or business key
			...
			if found then
				...
				if locked == 0 then
					triggerEvent('sendAme', source, "ieliek atslēgu slēdzenē, atslēdzot durvis.")
				else
					newRealLockedValue = true
					triggerEvent('sendAme', source, "ieliek atslēgu slēdzenē, aizslēdzot durvis.")
				end
				...
				outputChatBox("Tu esi pārāk tālu no durvīm.", source, 255, 194, 14)
			end
		...
		elseif (itemID==8) then -- sandwich
			...
			triggerEvent('sendAme', source, "apēd sviestmaizi.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==9) then -- sprunk
			...
			toggleAllControls(source, true, true, true)
			triggerEvent('sendAme', source, "izdzer koliņu.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==10) then -- red dice
			...
			exports.global:sendLocalText(source, " ✪ " .. getPlayerName(source):gsub("_", " ") .. " uzrullē " .. (max == 6 and "" or (max .. "-pusīgu ")) .. "kauliņu un dabū " .. math.random( 1, max ) ..".", 255, 51, 102)
		elseif (itemID==11) then -- taco
			giveHealth(source, 10)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "apēd taco.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==12) then -- cheeseburger
			giveHealth(source, 10)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			setTimer(removeAnimation, 4000, 1, source)
			triggerEvent('sendAme', source, "apēd siera burgeri.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==13) then -- donut
			setElementHealth(source, 100)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "apēd virtuli.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==14) then -- cookie
			giveHealth(source, 80)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "apēd cepumu.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==15) then -- water
			giveHealth(source, 90)
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			triggerEvent('sendAme', source, "izdzer ūdens pudeli.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==16) then -- clothes
			...
			if clothingid or fits then
				...
				triggerEvent('sendAme', source, "pārmainīja savas drēbes.")
			else
				outputChatBox("Šīs drēbes tev neder.", source, 255, 0, 0)
			end
		elseif (itemID==17) then -- watch
			local metaName = getItemName(itemID, itemValue, metadata)
			triggerEvent('sendAme', source, "paskatās uz savu " ..metaName .. ".")
			outputChatBox("Pulkstens ir " .. string.format("%02d:%02d", getTime()) .. ".", source, 255, 194, 14)
		...
		elseif (itemID==20) then -- STANDARD FIGHTING
			...
			outputChatBox("Tu izlasi grāmatu par Standardta Kaušanos.", source, 255, 194, 14)
		...
		elseif (itemID==21) then -- BOXING
			...
			outputChatBox("Tu izlasi grāmatu par Boxu.", source, 255, 194, 14)
		...
		elseif (itemID==22) then -- KUNG FU
			...
			outputChatBox("Tu izlasi grāmatu par Kung Fu.", source, 255, 194, 14)
		...
		elseif (itemID==23) then -- KNEE HEAD
			...
			outputChatBox("Tu atver grāmatu un saproti, ka tā rakstīta sengrieķu valodā.", source, 255, 194, 14)
		...
		elseif (itemID==24) then -- GRAB KICK
			...
			outputChatBox("Tu izlasi grāmatu par Tveršanas kaušanās stiļu.", source, 255, 194, 14)
		...
		elseif (itemID==25) then -- ELBOWS
			...
			outputChatBox("Tu izlasi grāmatu par Elkoņu Kaušanos.", source, 255, 194, 14)
		...
		elseif (itemID==27) then -- FLASHBANG
			...
			triggerEvent('sendAme', source, "uzmet apdullināšanas granātu.")
		...
		elseif (itemID==28) then -- GLOWSTICK
			...
			triggerEvent('sendAme', source, "nomet spīdošo kociņu.")
		...
		elseif (itemID==34) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "iešņauc kokaīnu.")
			triggerClientEvent(source,"triggerDrug1Effect",source)
		elseif (itemID==35) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "norij Morfīna tableti.")
			triggerClientEvent(source,"triggerDrug2Effect",source)
		elseif (itemID==36) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "norij Ectasy tableti.")
			triggerClientEvent(source,"triggerDrug3Effect",source)
		elseif (itemID==37) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "injekcē Heroīnu.")
			triggerClientEvent(source,"triggerDrug4Effect",source)
                ...
                if sucess then
                    if (tonumber(itemvalue) >= 1) then
                        if (hasSpaceForItem(source, 182, 1)) then
                            ...
                            exports.global:sendLocalMeAction(source, "satver marihuānas pumpuru, uzsākot tīt sev jointu.")
                            giveItem(source, 181, itemvalue - 1)
                        else
                            outputChatBox( "Tavas kabatas ir pilnas.", source, 255, 0, 0 )
                        end
                    else
                        outputChatBox("Tev ir beigušies tinamie papīrīši!", source, 255, 0, 0)
                    end
                else
                    outputChatBox("Tev vajag tinamos papīrīšus, lai šo lietotu.", source, 255, 0, 0)
                end
            else
                triggerEvent('sendAme', source, "ieskatās iekšā savā Marihuānas maisiņā. Tas ir tukšs.")
            end
		elseif (itemID==39) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "iešņauc Metamfetamīnu.")
            triggerClientEvent(source,"triggerDrug5Effect",source)
		elseif (itemID==40) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "injekcē Epinefrīnu.")
			triggerClientEvent(source,"triggerDrug6Effect",source)
		elseif (itemID==41) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "iepilina pilienu LSD sev mutē.")
			triggerClientEvent(source,"triggerDrug1Effect",source)
		elseif (itemID==42) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "apēd izžāvētas sēnes.")
			triggerClientEvent(source,"triggerDrug2Effect",source)
		elseif (itemID==43) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "norij PCP tabletes.")
			triggerClientEvent(source,"triggerDrug3Effect",source)
		...
		elseif (itemID==50) then -- highway code book
			local bookTitle = "Los Santos Lielceļa kods"
		...
		elseif (itemID==57) then -- FUEL CAN
			...
			if found then
				...
				outputChatBox("Tu esi pārāk tālu no transporta.", source, 255, 194, 14)
			end
		elseif (itemID==58) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "iedzer labu Ziebrand Alu.")
			setElementHealth(source,getElementHealth(source)-5)
		elseif (itemID==59) then -- MUDKIP
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "apēd mudkipu.")
			--killPed(source)
		elseif (itemID==60) then
			...
			-- world map.
			if dim == 0 then
				exports.hud:sendBottomNotification( source, itemName, "Šo seifu var ielikt tikai interjeros." )
				playSoundFrontEnd( source, 4 )
			-- vehicle interiors
			elseif dim >= 20000 then
				local vid = dim - 20000
				if exports.vehicle:getSafe( vid ) then
					exports.hud:sendBottomNotification( source, itemName, "Šajā īpašumā jau ir seifs. Raksti /movesafe, lai pārvietotu to vietā, kur stāvi." )
			...
			elseif dim >= 19000 then
				exports.hud:sendBottomNotification( source, itemName, "Tu nevari ievietot seifu īslaicīgā automašīnu interjerā." )
				playSoundFrontEnd( source, 4 )
			-- normal interiors.
			elseif hasItem( source, 5, dim ) or hasItem( source, 4, dim) then
				if exports.interior_system:getSafe( dim ) then
					...
					if exports.interior_system:addSafe( dim, nil, pos, int, rot, true, false ) then
						exports.hud:sendBottomNotification( source, itemName, "Nolikts." )
						takeItemFromSlot( source, itemSlot )
					end
				end
			end
		elseif (itemID==62) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "izdzer Bastradov Vodku.")
			setElementHealth(source,getElementHealth(source)-10)
		elseif (itemID==63) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "izdzer Skotu Viskiju.")
			setElementHealth(source,getElementHealth(source)-10)
		elseif (itemID==68) then
			triggerEvent('sendAme', source, "apskata savu loterijas biļeti.")
			outputChatBox("Tu paskaties uz loterijas biļeti un nosauc ciparu: " .. itemValue, source, 0, 255, 0)
		elseif (itemID==69) then -- Dictionary
			...
			if (learned) then
				...
				outputChatBox("Tu esi iemācījies " .. lang .. ", Spied F6, lai rediģētu savas valodas.", source, 0, 255, 0)
			end
		elseif (itemID==72) then -- Note
			triggerEvent('sendAme', source, "izlasa līmlapiņu.")
		...
		elseif (itemID==83) then -- Coffee
			...
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			triggerEvent('sendAme', source, "izdzer tasi kafijas.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==88) then -- earpiece
			outputChatBox("Tu vari lietot šo austiņu, kopā ar rāciju.", source, 255, 194, 14)
		elseif (itemID==89) then -- Generic Food
			...
			triggerEvent('sendAme', source, "apēd " .. itemExploded[1] .. ".")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==91) then
			takeItemFromSlot(source, itemSlot)
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			triggerEvent('sendAme', source, "izdzer labu Eggnogu.")
			setElementHealth(source, 100)
		elseif (itemID==92) then
			setElementHealth(source, 100)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "apēd Tītaru.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==93) then
			setElementHealth(source, 100)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "apēd Ziemassvētku Pudiņu.")
			takeItemFromSlot(source, itemSlot)
		...
		elseif (itemID==99) then --[ADDED: 2/22/10 by herbjr] tray
			...
			triggerEvent('sendAme', source, "apēd pārtiku no paplātes.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==100) then --[ADDED: 2/22/10 by herbjr] milk
			...
			triggerEvent('sendAme', source, "izdzer mazu paciņu piena.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==101) then --[ADDED: 2/22/10 by herbjr] juice
			...
			triggerEvent('sendAme', source, "izdzer mazu sulas paciņu.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==102) then --[ADDED: 2/23/10 by herbjr] Cabbage (Asked for by Misha)
			...
			triggerEvent('sendAme', source, "apēd Kāpostu.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==104) then -- portable TV
			...
			if isTvUsed == nil or isTvUsed == false then
				triggerEvent('sendAme', source, "ieslēdz mazu TV.")
		...
		elseif (itemID == 105) then -- Pack of cigarettes. Withdraw one if it still has one
			if (itemValue > 0) then
				if (hasSpaceForItem(source, 106, 1)) then
					...
					triggerEvent('sendAme', source, "ieskatas iekšā cigarēšu paciņā, izņemot vienu ārā.")
				else
					outputChatBox( "Tavas kabatas ir pilnas.", source, 255, 0, 0 )
				end
			else
				triggerEvent('sendAme', source, "ieskatas iekšā cigarešu paciņā. Tā ir tukša.")
			end
		elseif (itemID == 106) then -- Cigarette
			if hasItem( source, 107 ) then
				triggerEvent('sendAme', source, "aidedzina cigareti.")
				outputChatBox( "(( /throwaway, lai to aizmestu, /switchhand, lai izmainītu rokas ))", source )
				triggerEvent("realism:startsmoking", source, 0) -- 0 = left hand -- 1 = right hand
				takeItemFromSlot(source, itemSlot)
			else
				triggerEvent('sendAme', source, "parāda visiem cigareti.")
				outputChatBox( "Tev vajadzēs šķiltavas vai sērkociņus, lai to pareizi lietotu.", source, 255, 0, 0 )
			end
		elseif (itemID == 107) then
			triggerEvent('sendAme', source, "parāda visiem škiltavas.")
		elseif (itemID==108) then -- donut
			setElementHealth(source, 100)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "apēd pankūku.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==109) or (itemID==110) then -- Fruit
			giveHealth(source, tonumber(getItemValue(itemID, itemValue)))
			exports.global:applyAnimation(source, "food", "eat_burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "apēd " .. itemName .. ".")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID == 113) then -- Pack of glowsticks, Withdraw one if it still has one
			if tonumber(itemValue) and tonumber(itemValue) > 0 then
				if (hasSpaceForItem(source, 28, 1)) then
					...
					triggerEvent('sendAme', source, "apskatās savā spīdošo kociņu paciņā, izslidinot vienu ārā.")
				else
					outputChatBox( "Tavas kabatas ir pilnas.", source, 255, 0, 0 )
				end
			else
				triggerEvent('sendAme', source, "apskatās savā spīdošo kociņu paciņā. Tas ir tukšs.")
			end
		elseif itemID == 114 then
			...
			if vehicle and not noUpgrades[getVehicleType(vehicle)] --[[and getElementDimension(vehicle) > 0]] and getItemDescription(itemID, itemValue) ~= "?" then
				...
				outputChatBox("Izmanto šo preci automašīņa, lai to pievienotu savam auto.", source, 255, 194, 14)
			end
		elseif itemID == 126 then
			outputChatBox("Forša josta, čempi.", source, 0, 255, 0)
		elseif itemID == 130 then
			outputChatBox("Ievieto šo signalizācijas sistēmu automašīnā, lai to lietotu.", source, 0, 255, 0)
		elseif itemID == 132 then
			takeItemFromSlot(source, itemSlot)
			outputChatBox("Tu paņēmi savu " .. itemValue .. " dozu.", source)
			triggerEvent('sendAme', source, "norij ārsta ieteiktās zāles.")
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
		elseif itemID == 134 then
			outputChatBox("$" .. exports.global:formatMoney(itemValue) .. " naudas.", source)
		elseif itemID == 137 then -- New snake cam, Adams
			triggerEvent('snakecam:toggleSnakeCam', root, source)
		elseif itemID == 138 then
			outputChatBox("Izmanto šo preci automašīņa, lai to pievienotu savam auto.", source, 0, 255, 0)
		...
		elseif masks[itemID] then
			...
			elseif getPedOccupiedVehicle(source) and getElementData(getPedOccupiedVehicle(source), "job") > 0 and getVehicleOccupant(getPedOccupiedVehicle(source)) == source then
				outputChatBox("Tu nevari šo lietot Civilajā transportā!", source, 255, 0, 0)
			else
		...
		elseif itemID == 139 then
			outputChatBox("((Vaicā adminam, lai šo ieinstalē tavā auto.))", source, 255, 0, 0)
		...
		elseif itemID == 173 then
			...
			if distance < 25 then
				...
				outputChatBox("Lai pārdotu savu auto, tev ir jābūt CSDD stāvlaukumā.", source, 0, 255, 0)
			end
        ...
        elseif itemID == 181 then
        	triggerEvent("sendAme", source, "parāda visiem Kūpināšanas komplektu.")
    	elseif (itemID == 182) then
            if hasItem( source, 107 ) then -- If da player has a lighter, how else would he smoke it dawg
                exports.global:sendLocalMeAction(source, "aizdedzina Jointu.")
                outputChatBox( "(( /throwaway, lai to aizmestu, /switchhand, lai mainītu roku, /passjoint, lai padotu jointu ))", source )
                ...
                exports.global:sendLocalMeAction(source, "parāda visiem foršu, uztītu jointu.")
                outputChatBox("Tev vajadzēs šķiltavas vai sērkociņus.", source, 255, 0, 0)
            end
		...
		elseif (itemID==210) then -- Coca-Cola Christmas Edition
			...
			triggerEvent('sendAme', thePlayer, "izdzer koliņu.")
		...
		elseif (itemID==213) then --pinnekjott
			giveHealth(source, 10)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "apēd pinnekjotti.")
			takeItemFromSlot(source, itemSlot)
		...
		elseif (itemID==214) then
			triggerEvent('sendAme', source, "paņem ".. itemName ..".")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID >= 224 and itemID <= 228) then
			triggerEvent('sendAme', source, "paņem nedaudz ".. itemName ..".")
			takeItemFromSlot(source, itemSlot)
		elseif itemID == 232 then -- car battery charger
			local veh = additional
			local batt = getElementData( veh, 'battery' ) or 100
			if batt == 100 then
				exports.hud:sendBottomNotification( source, itemName, "Šī "..exports.global:getVehicleName(veh).." baterija jau ir pilna." )
			else
				if takeItemFromSlot( source, itemSlot) then
					exports.anticheat:setEld( veh, 'battery', math.min( 100, batt + 20 ) )
					exports.hud:sendBottomNotification( source, itemName, "Šī "..exports.global:getVehicleName(veh).." baterija tika uzpildīta pa 20%." )
					triggerEvent( 'fuel:sync', veh, source )
				end
			end
		elseif itemID == 260 then
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			exports.global:sendLocalMeAction(source, "padzer nedaudz Amonjaku.")
		...
		elseif itemID == 270 then
			...
			if getElementData(source, "gloves") then
				triggerEvent('sendAme', source, "izslidina pāri " .. metaName .. " no savām rokām.")
				...
				triggerEvent('sendAme', source, "uzslidina sev pāri " .. metaName .. " uz rokām.")
...
function npcUseItem(element, itemID)
	...
	if itemID then
		...
		if(hasItem) then
			...
			if badges[itemID] then
				if ( getElementData( source, badges[itemID][1] ) ) then
					...
					if pedname then
						...
						exports.global:sendLocalText(source, "* "..tostring(pedname).." noņem "..badges[itemID][2]..".", 255, 51, 102, 5)
					end
				else
						...
						if key ~= itemID then
							if ( getElementData ( source, badge[1] ) ) then
								...
								if pedname then
									...
									exports.global:sendLocalText(source, "* "..tostring(pedname).." noņem "..badge[2]..".", 255, 51, 102, 5)
								end
							end
						end
					end
					...
					if pedname then
						...
						exports.global:sendLocalText(source, "* "..tostring(pedname).." uzliek "..badges[itemID][2]..".", 255, 51, 102, 5)
					end
					return true
				end
			...
			outputDebugString("Pedam nav šīs mantas.")
			return false
		end
	end
end
...
function destroyItem(itemID)
	...
	outputChatBox("Tu iznīcināji " .. itemName .. ".", source, 255, 194, 14)
	triggerEvent('sendAme', source, "iznīcināja "..itemName..".")
end
...
function showItem(itemName)
	if isPedDead(source) or getElementData(source, "injuriedanimation") then return end
	triggerEvent('sendAme', source, "parāda visiem "..(isVowel(itemName) and "" or "").." " .. removeOOC(itemName) .. ".")
end
	...
	if not (moneyAmountCurrent) or not (moneyAmountSort) or tonumber(moneyAmountCurrent) < 0 or tonumber(moneyAmountSort) < 0 then
		...
		if tonumber(moneyAmountCurrent) <= tonumber(playerMoney) and tonumber(moneyAmountSort) <= tonumber(playerMoney) then
			if hasItem(thePlayer, 134, tonumber(moneyAmountCurrent)) then
				outputChatBox("Tu izņemi $" .. exports.global:formatMoney(moneyAmountSort) .. " no $" .. exports.global:formatMoney(moneyAmountCurrent) .. ".", thePlayer)
				...
				outputChatBox("Tev nav tik daudz naudas vienā no čupām.", thePlayer, 255, 0, 0)
			end
		else
			outputChatBox("Tev nav tik daudz naudas.", thePlayer, 255, 0, 0)
		end
	end
end
...
function combineMoney(thePlayer, commandName)
		...
		if tonumber(playerMoney) > 0 then
			...
			outputChatBox("Tu savienoji $" .. exports.global:formatMoney(playerMoney) .. ".", thePlayer)
		else
			outputChatBox("Tu nevari savienot naudu, kuras tev nav!", thePlayer, 255, 0, 0)
		end
end
...
function toggleBadge(source, badges, itemID , itemValue)
	if ( getElementData( source, badges[itemID][1] ) ) then
		...
		if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 or itemID == 158 or itemID == 168 then
			triggerEvent('sendAme',  source, "noņem " .. badges[itemID][1] .. " no savas galvas." )
		else
			triggerEvent('sendAme', source, "noņem " .. badges[itemID][1] .. ".")
		end
	else
		for key, badge in pairs ( badges ) do
			if key ~= itemID then
				if ( getElementData ( source, badge[1] ) ) then
					exports.anticheat:changeProtectedElementDataEx( source, badge[1], false, true )
					if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 or itemID == 158 or itemID == 168 then
					triggerEvent('sendAme',  source, "noņem " .. badge[1] .. " no savas galvas." )
					else
					triggerEvent('sendAme',  source, "noņem " .. badge[1] .. "." )
					end
				end
			end
		end
		...
		triggerEvent('sendAme',  source, "aptin " .. badges[itemID][1] .. " ap savu galvu." )
		else
			exports.anticheat:changeProtectedElementDataEx( source, badges[itemID][1], removeOOC(itemValue), true )
			triggerEvent('sendAme',  source, "uzvelk " .. badges[itemID][1] .. "." )
		end
	end
	exports.global:updateNametagColor(source)
end
