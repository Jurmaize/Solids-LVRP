	...
	if name and ip then
		if donorStation then
			...
			if not exports.donators:takeGC(client, perk[2]) then
				exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Tev nepietiek GC, lai iegādātos šo. Lūdzu apmeklē F10 izvēlne -> Ziedotāju bonusi, lai iegādātos vēl GC.")
				return false
			end
			...
			exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Jauna radio stacija veiksmīgi izveidota! (Nosaukums: "..name..", URL: "..ip..")")
			...
			if tonumber(maxStations["max"]) >= 30 then
				exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Servera maksimālais skaits, 30 radio stacijas, ir sasniegts. Izdzēs kādu pirms pievieno jaunu.")
				return false
			end
			local smallestID = SmallestID()
		...
		exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Neizdevās izveidot radio staciju.")
	end
end
...
function editStation(id, name, ip)
	...
	if id and name and ip and mysql:query_free("UPDATE `radio_stations` SET `station_name`='"..exports.global:toSQL(name).."', `source`='"..exports.global:toSQL(ip).."' WHERE `id`='"..id.."' ") then
		exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Atjaunoja radio staciju #"..id.." successfully!")
		forceUpdateClientsGUI()
	else
		exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Neizdevās atjaunot radio staciju #"..id)
	end
end
...
function deleteStation(id)
	...
	if id and mysql:query_free("DELETE FROM `radio_stations` WHERE `id`='"..id.."' ") then
		exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Izdzēsa radio staciju #"..id..".")
		forceUpdateClientsGUI()
	else
		exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Neizdevās izdzēst staciju #"..id..".")
	end
end
...
function togStation(id, state)
	...
	if id and state and mysql:query_free("UPDATE `radio_stations` SET `enabled`='"..(state == "Activated" and "1" or "0").."' WHERE `id`='"..id.."' ") then
		exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Radio stacija #"..id.." ir "..state.."!")
		forceUpdateClientsGUI()
	else
		exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Neizdevās atjaunot radio staciju #"..id)
	end
	...
	if id and tonumber(id) and order and tonumber(order)  then
		...
		if donorStation then
			...
			if not exports.donators:takeGC(client, moveCost) then
				exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Tev nepietiek GameCoins, lai nopirktu šo. Lūdzu dodies uz F10 izvēlni -> Ziedotāju Bonusi, lai iegādātos papildus GC.")
				return false
			end
			addPurchaseHistory(client, "Izmainīja radio pozīciju (Nos: '"..name.."', Virziens: '"..(movingUp and "Augšā" or "Lejā").."')", moveCost)
		end
		...
		if movingUp then
			if order < 2 then
				exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Šī radio stacija jau ir pašā galā!")
				return false
			end
			if mysql:query_free("UPDATE `radio_stations` SET `order`=`order`-1 WHERE `id`='"..(id).."' ") then
				forceUpdateClientsGUI()
				exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Radio stacija '"..name.."' ir sarakstā nolikta uz augšu!")
			else
				exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Nevarēja izmainīt radio stacijas pozīciju!")
			end
		else
			if mysql:query_free("UPDATE `radio_stations` SET `order`=`order`+1 WHERE `id`='"..(id).."' ") then
				forceUpdateClientsGUI()
				exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Radio stacija '"..name.."' ir sarakstā nolikts uz leju.")
			else
				exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Nevarēja izmainīt radio stacijas pozīciju!")
			end
		end
...
function renewStation(station, duration)
	if station and duration and tonumber(duration) then
		...
		elseif duration == 90 then
			...
			exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Neiznāca atjaunot radio staciju #"..id)
			return false
		end
		
		if not exports.donators:takeGC(client, cost) then
			exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Tev nepietiek GameCoins, lai iegādātos šo. Lūdzu apmeklē F10 izvēlni -> Ziedotāju bonusi.")
			return false
		end
		addPurchaseHistory(client, "Atjaunoja radio staciju (ID: '"..id.."', Nos: '"..station[2].."', Ilgums: '"..duration.." dienas')", cost)
		
		if not mysql:query_free("UPDATE `radio_stations` SET `expire_date`=(`expire_date` + interval "..duration.." day) WHERE `id`='"..(id).."' ") then
			return false
		end
		
		exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Tu esi veksmīgi atjaunojis radio staciju ID#"..id.."!")
		forceUpdateClientsGUI()
	else
		exports.hud:sendBottomNotification(client, "Radio Staciju Menedžeris", "Nevarēja atjaunot radio staciju #"..id)
	end
end
