...
function setVolume(commandname, val)
	...
	outputChatBox ( "* KĻŪDA: /setvol 0 - 100", 255, 0, 0, false )
end
...
function ( )
	...
	if mp3Station and mp3Station ~= nil then
		local song = 'Gaida dziesmas nosaukumu...'
		if mp3Sound and isElement(mp3Sound) and getSoundMetaTags(mp3Sound) then
			song = getSoundMetaTags(mp3Sound)['stream_title'] or 'Gaida dziesmas nosaukumu...'
		end
...
function showMP3GUI()
	...
	local off = guiCreateButton(10, height - 35, 85, 25, 'Izslēgt', false, mp3Window)
	...
	local close = guiCreateButton(105, height - 35, 85, 25, 'Aizvērt', false, mp3Window)
