...
function carshop_buyVehicle(paymentMethod)
	...
	local license = (getElementData(client, "license.car") == 1) and "" or "Tev nav B kategorijas tiesības! Labāk ar šo uz ielām nebraukā!"

	if isOverlayDisabled then
		outputChatBox("Apsveicam! Tu iegādājies jaunu transportlīdzekli", client)
		outputChatBox("Pārliecinies, ka veic /park komandu stundas laikā,", client)
		outputChatBox("savādāk auto tiks izdzēsts un netiks atgriezsts.", client)
		outputChatBox(license, client)
		outputChatBox("Rediģē auto aprakstu ar /ed vai /editdescription.", client)
	else
		local content = {}
		table.insert(content, {"Pārliecinies, ka veic /park komandu stundas laikā, savādāk auto tiks izdzēsts un netiks atgriezsts."})
		table.insert(content, {license})
		table.insert(content, {"Rediģē auto aprakstu ar /ed vai /editdescription."})

		exports.hud:sendBottomNotification(client, "Apsveicam, tu iegādājies auto!", content, 30)
	end
		...
		if shopname and shopnicename and vehicle_shop_id and vehYear and vehBrand and vehModel and vehPrice then
			...
			if orderedVehID and tonumber(orderedVehID) == tonumber(vehicle_shop_id) then
				if exports.global:hasItem(player, 2) then
					...
					exports.global:sendLocalText(player, "*"..playerName.." saņem īsziņu.", 255, 51, 102, 30, {}, true)
					outputChatBox("["..languagename.."] SMS no "..shopnicename..": Sveiki! Jūs vēlējāties atgādinājumu, mums ir "..itemName.." inventārā pa $"..exports.global:formatMoney(vehPrice)..". Atbrauciet un apskatiet to!" , player, 120, 255, 80)
				end
			end
		end
	end
