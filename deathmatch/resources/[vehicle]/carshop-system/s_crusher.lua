...
function resetPrice(theVehicle, matching)
	if isElement(theVehicle) and getElementType(theVehicle) == "vehicle" then
		if getElementData(theVehicle, "crushing") then
			...
			if thePlayer then
				outputChatBox("Brauc atpakaļ, kad esi to apdomājis.!", thePlayer, 255, 194, 14)
		...
		if theVehicle and getVehicleOccupant(theVehicle) == thePlayer then -- player is driver
			if getElementData(theVehicle, "owner") == getElementData(thePlayer, "dbid") then
				...
				if getElementData(theVehicle, "requires.vehpos") then
					outputChatBox("(( Lūdzu noparko auto, tad dodies šurp. ))", thePlayer, 255, 0, 0)
				elseif price == 0 and not getElementData(theVehicle, "token") then
					outputChatBox("Šis " .. exports.global:getVehicleName(theVehicle) .. " nav nekā vērts.", thePlayer, 255, 0, 0 )
					outputChatBox("(( Kontaktējies ar adminu, lai pārdotu šo auto. ))", thePlayer, 255, 194, 14)
				elseif getVehicleType(theVehicle) == "Trailer" then
					outputChatBox("Šis " .. exports.global:getVehicleName(theVehicle) .. " nevar tikt likvidēts.", thePlayer, 255, 0, 0 )
					outputChatBox("(( Kontaktējies ar adminu, lai pārdotu šo auto. ))", thePlayer, 255, 194, 14)
				else
				...
				outputChatBox("Tev ir šitam reģistrācija? Nav? Nu tad pis prom. ((Auto nepieder tev))", thePlayer, 255, 0, 0)
			end
		end
	end
end
	...
	if theVehicle and thePlayer then
		if getElementData( theVehicle, "owner" ) == getElementData( thePlayer, "dbid" ) then
			...
			if price and price >= 0 and dbid > 0 then
				...
				-- notify
				outputChatBox("Tu likvidēji savu " .. exports.global:getVehicleName(theVehicle) .. " par $" .. exports.global:formatMoney(price) .. ".", thePlayer, 0, 255, 0)
