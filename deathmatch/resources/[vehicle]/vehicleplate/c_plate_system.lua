...
function getPaperFromTony(button, state, absX, absY, wx, wy, wz, element)
    if (element) and (getElementType(element)=="ped") and (button=="right") and (state=="down") then --if it's a right-click on a object
        ...
        if(pedName == "Tony Johnston") then
            ...
            showCursor(true)
            local row = exports.rightclick:addRow("Pirkt CSDD pārdošanas atļauju - 100$")
...
function cBeginGUI()
	...
	--Buttons
	plates = guiCreateButton(0.1, 0.1, 0.75, 0.15, "Nummuri", true, greetingWindow)
	addEventHandler("onClientGUIClick", plates, fetchPlateList)

	register = guiCreateButton(0.1, 0.32, 0.75, 0.15, "Reģistrācija", true, greetingWindow)
	...
	neither = guiCreateButton(0.1, 0.75, 0.75, 0.15, "Nevienu", true, greetingWindow)
...
function PlateWindow(vehicleList)
	...
	mainVehWindow = guiCreateWindow(x, y, width, height, "Auto nummuru: Reģistrācija", false)

	guiCreateLabel(0.03, 0.08, 2.0, 0.1, "Kuram auto vēlētos reģistrēt jaunu nummuru?", true, mainVehWindow)
	vehlist = guiCreateGridList(0.03, 0.15, 1, 0.73, true, mainVehWindow)

	ovid = guiGridListAddColumn(vehlist, "ID", 0.1)
	ov = guiGridListAddColumn(vehlist, "Piederošie auto", 0.5)
	ovcp = guiGridListAddColumn(vehlist, "Šobrīdējie nummuri", 0.3)
	...
	--Buttons
	close = guiCreateButton(0.50, 0.90, 0.50, 0.10, "Iziet", true, mainVehWindow)
...
function registerWindow(vehicleList)
	...
	mainVehWindow = guiCreateWindow(x, y, width, height, "Auto reģistrācija", false)

	guiCreateLabel(0.03, 0.08, 2.0, 0.1, "Kuru auto vēlētos reģistrēt/izņemt?", true, mainVehWindow)
	vehlist = guiCreateGridList(0.03, 0.15, 1, 0.73, true, mainVehWindow)

	ovid = guiGridListAddColumn(vehlist, "ID", 0.1)
	ov = guiGridListAddColumn(vehlist, "Piederošie auto", 0.65)
	ovcp = guiGridListAddColumn(vehlist, "Reģistrēšanas statuss", 0.15)
	...
	for i,r in ipairs(vehicleList) do
		...
		if plateState == 0 then
		    guiGridListSetItemText(vehlist, row, ovcp, "Nepiereģistrēts", false, false)
		else
		    guiGridListSetItemText(vehlist, row, ovcp, "Reģistrēts", false, false)
		end
	end
...
function editPlateWindow()
	...
	if (source == vehlist) and not (svnum == "") then
		...
		local mainT = guiCreateLabel(0.03, 0.08, 0.96, 0.2, "Reģistrē nummuru auto ar VIN: " .. svnum .. ".\nLūdzu ievadiet jauno nummuru:", true, efinalWindow)
		...
		plateCheck = guiCreateLabel(0, 0.5, 1, 0.1, "Lūdzu izvēlaties nummur, kurš Jums patīk.", true, efinalWindow)
		guiLabelSetHorizontalAlign(plateCheck, "center", true)

		--Buttons

		submitNP = guiCreateButton(0.1, 0.60, 0.8, 0.15, "Pirkt (Cena: "..cost.." GC)", true, efinalWindow)
		...
		finalx = guiCreateButton(0.1, 0.80, 0.8, 0.15, "Iziet", true, efinalWindow)
...
function checkPlateBox()
	...
	if checkPlate(theText) then
		guiSetText(plateCheck, "'"..theText.."' ir derīgs nummurs!")
		guiLabelSetColor(plateCheck, 0, 255, 0)
		guiSetEnabled(submitNP, true)
	else
		guiSetText(plateCheck, "'"..theText.."' ir nederīgs.")
...
function updateRegistration()
	...
	if (source == vehlist) and not (svnum == "") then
		...
		efinalWindow = guiCreateStaticImage(x, y, width, height, ":resources/window_body.png", false)
		local mainT = guiCreateLabel(0.03, 0.08, 0.96, 0.2, (state == "Unregistered" and "Registering" or "Unregistering")..(" auto VIN: " .. svnum .. "."), true, efinalWindow)
		taxLabel = guiCreateLabel(0.03, 0.18, 0.96, 0.2, "Auto nodoklis: Lādējas...", true, efinalWindow)
		...
		local submitNP2 = guiCreateButton(0.1, 0.60, 0.8, 0.15, (state == "Unregistered" and "Reģistrēšana (Cena: $175)" or "Izņemt (Cena: $50)") , true, efinalWindow)
...
addEventHandler("plate:updateTaxLabel", localPlayer, function(tax)
	guiSetText(taxLabel, "Auto nodoklis: $" .. exports.global:formatMoney(tax))
end)
