...
function createPermanentVehicle(player, command, ...)
	if hasCreatePermission( player ) then
		...
		if isFactionOwned then
			...
			if other then
				...
				if not exports.global:canPlayerBuyVehicle(other) then
					outputChatBox("[MAKEVEH] Šim spēlētājam ir pārāk daudz auto.", player, 255, 0, 0)
					outputChatBox("Tev ir pārāk daudz auto.", other, 255, 0, 0)
					return
				elseif not exports.global:takeMoney(other, cost) then
					outputChatBox("[MAKEVEH] Šis spēlētājs nevar atļauties šo auto.", player, 255, 0, 0)
					outputChatBox("Tu nevari atļauties šo auto.", other, 255, 0, 0)
					return
				end
			else
		...
		if (result) then
			...
			if not isFactionOwned then
				outputChatBox("[MAKEVEH] $"..cost.." tika izņemti no spēlētāja kabatām.", player, 0, 255, 0)
				outputChatBox("$"..cost.." tika izņemti no tavām kabatām..", targetPlayer, 0, 255, 0)
			else
				outputChatBox("[MAKEVEH] $"..cost.." tika izņemts no spēlētāja frakcijas bankas.", player, 0, 255, 0)
			end
...
function toggleLock(source, key, keystate)
	...
	elseif (lockSpam[source] == 5) then
		outputChatBox("Nespamo komandas!", source, 255, 0, 0)
				...
				--if (owner ~= -1) then
					if ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
						...
						if (locked) then
							...
							triggerEvent('sendAme', source, "atslēdz auto durvis.")
						else
							...
							triggerEvent('sendAme', source, "aizslēdz auto durvis.")
						end
					else
						outputChatBox("(( Tu nevari atslēgt konfiscētus auto. ))", source, 255, 195, 14)
					end
				--else
...
function toggleLights(source, key, keystate)
	...
	if veh then
		if ( getElementData( veh, 'battery' ) or 100 ) > 0 then
			...
			exports.hud:sendBottomNotification( source, exports.global:getVehicleName(veh), "Baterija beidzās." )
		end
	end
end
...
function checkBikeLock(thePlayer)
	if (isVehicleLocked(source)) and (getVehicleType(source)=="Bike" or getVehicleType(source)=="Boat" or getVehicleType(source)=="BMX" or getVehicleType(source)=="Quad" or getElementModel(source)==568 or getElementModel(source)==571 or getElementModel(source)==572 or getElementModel(source)==424 or getElementModel(source)==431 or getElementModel(source)==437) then
		if not getElementData(thePlayer, "interiormarker") then
			outputChatBox("Tas auto ir aizslēgts.", thePlayer, 255, 194, 15)
		end
		cancelEvent()
	end
end
...
function setRealInVehicle(thePlayer)
	if isVehicleLocked(source) then
		...
		if owner < 0 and faction == -1 then
			exports.hud:sendBottomNotification( thePlayer, vehName, "(( Šis "..vehName.." ir civilais transports. ))" )
		elseif (faction==-1) and (owner>0) then
			...
			if ownerName and exports.integration:isPlayerTrialAdmin( thePlayer, true ) then
				exports.hud:sendBottomNotification( thePlayer, vehName, "(( Šis "..vehName.." pieder " .. ownerName .. " ))" )
			end
		elseif (faction~=-1) then
			local factionName = exports.cache:getFactionNameFromId(faction) or "Unknown Faction"
			exports.hud:sendBottomNotification( thePlayer, vehName, "(( Šis auto pieder " .. factionName .. ". ))" )
		end
	end
end
...
function sellVehicle(thePlayer, commandName, targetPlayerName, itemValue)
	...
	if isPedInVehicle(thePlayer) then
		if not targetPlayerName then
			...
			outputChatBox("Pārdod auto, kurā tu esi, spēlētājam.", thePlayer, 255, 194, 14)
			outputChatBox("Palūdz pircējam lietot /pay, lai saņemtu naudu par auto.", thePlayer, 255, 194, 14)
		else
						...
						if getElementData(theVehicle, "owner") == getElementData(thePlayer, "dbid") or isLeader or (exports.integration:isPlayerAdmin(thePlayer, true)) then
							if getElementData(targetPlayer, "dbid") ~= getElementData(theVehicle, "owner") then
								if getElementData(theVehicle, "token") then
									outputChatBox("Tu nevari pārdot šo auto citam spēlētājam, jo tas tika iegūts ar tokenu.", thePlayer, 255, 0, 0)
									return
								end
									...
									if exports.global:canPlayerBuyVehicle(targetPlayer) then
										--if exports.integration:isPlayerTrialAdmin(thePlayer) --[[or exports['carshop-system']:isForSale(theVehicle)]] then
											...
											if query then
												...
												outputChatBox("Tu esi veiksmīgi pārdevis savu " .. exports.global:getVehicleName(theVehicle) .. " spēlētājam " .. targetPlayerName .. ".", thePlayer, 0, 255, 0)
												outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " pārdeva tev " .. exports.global:getVehicleName(theVehicle) .. ".", targetPlayer, 0, 255, 0)
												outputChatBox("Neaizmirsti uzrakstīt /park savam " .. exports.global:getVehicleName(theVehicle) .. ".", targetPlayer, 255, 255, 0)
												...
												outputChatBox("Unable to process request - report on https://discord.gg/S5PDPdDeac", thePlayer, 255, 0, 0)
											end
										--else
											--outputChatBox("You can not sell special vehicles. Contact an admin via F2 to have it refunded.", thePlayer, 255, 0, 0)
										--end
									else
										outputChatBox(targetPlayerName .. " pieder jau pārāk daudz auto.", thePlayer, 255, 0, 0)
										outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " centās pārdot tev auto, bet tev jau ir pārāk daudz auto!", targetPlayer, 255, 0, 0)
									end
								else
									outputChatBox(targetPlayerName .. " nav vietas auto atslēgām.", thePlayer, 255, 0, 0)
									outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " centās pārdot tev auto, bet tev nav vietas atslēgām!", targetPlayer, 255, 0, 0)
								end
							else
								outputChatBox("Tu nevari pārdot auto sev.", thePlayer, 255, 0, 0)
							end
						else
							outputChatBox("Šis auto nav tavs.", thePlayer, 255, 0, 0)
						end
					else
						outputChatBox("Tev ir jaatrodas automašīnā.", thePlayer, 255, 0, 0)
					end
				else
					outputChatBox("Tu esi pārāk tālu prom no " .. targetPlayerName .. ".", thePlayer, 255, 0, 0)
				end
			end
		end
	end
...
function lockUnlockInside(vehicle)
	...
	--if (owner ~= -1) then
		if ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
			...
			outputChatBox("(( Tu nevari aizslēgt konfiscētus auto. ))", source, 255, 195, 14)
		end
	--else
		--outputChatBox("(( You can't lock civilian vehicles. ))", source, 255, 195, 14)
	--end
...
function lockUnlockOutside(vehicle)
	if (not source or exports.integration:isPlayerTrialAdmin(source)) or ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
		...
		if (isVehicleLocked(vehicle)) then
			setVehicleLocked(vehicle, false)
			triggerEvent('sendAme', source, "uzspiež uz atslēgas, lai atslēgtu auto. ((" .. getVehicleName(vehicle) .. "))")
			exports.logs:dbLog(source, 31, {  vehicle }, "UNLOCK FROM OUTSIDE")
		else
			setVehicleLocked(vehicle, true)
			triggerEvent('sendAme', source, "uzspiež uz atslēgas, lai aizslēgtu auto. ((" .. getVehicleName(vehicle) .. "))")
			local doors = getDoorsFor(getElementModel(vehicle), -1)
			for index, doorEntry in ipairs(doors) do
				setVehicleDoorOpenRatio(vehicle, doorEntry[2], 0, 0.5)
			end
...
function fillFuelTank(veh)
	...
	if (math.ceil(currFuel)==max) then
		outputChatBox("Šis auto jau ir pilns.", source)
	elseif (fuel==0) then
		outputChatBox("Šī degvielas kanniņa ir tukša.", source, 255, 0, 0)
	elseif (engine==1) then
		outputChatBox("Tu nevari uzpildīt auto, kurš ir iedarbināts. Izslēdziet dzinēju virspirms.", source, 255, 0, 0)
	else
		...
		if not (exports['item-system']:updateItemValue(source, itemSlot, math.floor(fuel - fuelAdded))) then
			outputChatBox("Kaut kas aizgāja greizi, rakstiet /report.", source)
			return
		end

		outputChatBox("Tu ielēji " .. math.floor(fuelAdded) .. " litrus ar degvielu savā auto no degvielas kanniņas.", source, 0, 255, 0 )
		...
		triggerEvent('sendAme', source, "piepilda " .. genderm .. " auto no mazas degvielas kanniņas.")
...
function getYearDay(thePlayer)
	...
	outputChatBox("Gada diena ir " .. currYearday, thePlayer)
end
...
function removeNOS(theVehicle)
	...
	triggerEvent('sendAme', source, "izņem NOS no " .. getVehicleName(theVehicle) .. ".")
...
local function canParkHere( thePlayer, veh )
	if call( getResourceFromName("tow-system"), "cannotVehpos", thePlayer, veh ) and not exports.integration:isPlayerTrialAdmin(thePlayer, true) and not exports.integration:isPlayerSupporter(thePlayer, true) then
		return not outputChatBox("Nav iespējams noparkot auto šeit.", thePlayer, 255, 0, 0)
	elseif isElementWithinColShape( thePlayer, HospitalCol ) and not exports.factions:isPlayerInFaction(thePlayer, 2) and not exports.integration:isPlayerTrialAdmin(thePlayer, true) and not exports.integration:isPlayerSupporter(thePlayer, true) then
		return not outputChatBox("Tikai Los Santos Glābšanas Dienasts var noparkot savus auto pie slimnīcas.", thePlayer, 255, 0, 0)
	elseif isElementWithinColShape( thePlayer, PershingSquareCol ) and not exports.factions:isPlayerInFaction(thePlayer, 1) and not exports.integration:isPlayerTrialAdmin(thePlayer, true) and not exports.integration:isPlayerSupporter(thePlayer, true) then
		return not outputChatBox("Tikai Los Santos Policijas Departaments var noparkot savus auto Pershing Square apkārtnē.", thePlayer, 255, 0, 0)
	end
	return true
end
...
local function parkVeh( thePlayer, veh, commandName )
	if canParkHere( thePlayer, veh ) then
		...
		if (owner==playerid and TowingReturn == 0) or exports.global:hasItem(thePlayer, 3, dbid) or TowingReturn == 2 or exports.integration:isPlayerSupporter(thePlayer, true) or exports.integration:isPlayerTrialAdmin(thePlayer, true)  then
			if (dbid<0) then
				outputChatBox("Šis auto nav bezgalīgi iespawnots.", thePlayer, 255, 0, 0)
			else
				...
				outputChatBox("Auto parkinga vieta ir uzstādīta.", thePlayer)
				-- logs
				...
				if ( getElementData(veh, "Impounded") or 0 ) > 0 then
					...
					if isElement( owner ) and exports.global:hasItem( owner, 2 ) then
						outputChatBox("((SFT&R)) #5555 [SMS]: Tavs " .. getVehicleName(veh) .. " tika konfiscēts. Dodies uz Los Santos Auto Konfiscētuvi, lai to izpirktu.", owner, 120, 255, 80)
					end
				end
			end
		end
...
function setVehiclePosition(thePlayer, commandName)
	...
	if not veh or getElementData(thePlayer, "realinvehicle") == 0 then
		outputChatBox("Tu neatrodies auto.", thePlayer, 255, 0, 0)
	else
	...
	if exports.integration:isPlayerTrialAdmin( thePlayer ) then
		...
		if not vehicleID or vehicleID < 0 then
			...
			if veh then
				...
				exports.anticheat:changeProtectedElementDataEx(veh, "interior", interior)
				exports.anticheat:changeProtectedElementDataEx(veh, "dimension", dimension)
				outputChatBox("Atjaunošanās punkts priekš #" .. vehicleID .. " uzstādīts.", thePlayer)
				exports.logs:dbLog(thePlayer, 6, {  veh }, "PARK")
				for key, value in ipairs(destroyTimers) do
				...
				outputChatBox("Auto netika atrasts.", thePlayer, 255, 0, 0 )
			end
		end
	end
end
...
function setVehiclePosition3(veh)
	if call( getResourceFromName("tow-system"), "cannotVehpos", source ) then
		outputChatBox("Tikai Los Santos Auto Konfiscētuve drīkst noparkot savus auto iekš konfiscētuves plača.", source, 255, 0, 0)
	elseif isElementWithinColShape( source, HospitalCol ) and not exports.factions:isPlayerInFaction(source, 2) and not exports.integration:isPlayerTrialAdmin(source) then
		outputChatBox("Tikai Los Santos Glābšanas Dienests drīkst noparkot savus auto pie slimnīcas.", source, 255, 0, 0)
	elseif isElementWithinColShape( source, PershingSquareCol ) and not exports.factions:isPlayerInFaction(source, 1) and not exports.integration:isPlayerTrialAdmin(source) then
		outputChatBox("Tikai Los Santos Policijas Departaments drīkst noparkot savus auto Pershing Square apkārtnē.", source, 255, 0, 0)
	else
		...
		if (owner==playerid and TowingReturn == 0) or (exports.global:hasItem(source, 3, dbid)) or (TowingReturn == 2) or (exports.integration:isPlayerTrialAdmin(source)) then
			if (dbid<0) then
				outputChatBox("Šis auto nav bezgalīgi iespawnots.", source, 255, 0, 0)
			else
				if (call(getResourceFromName("tow-system"), "CanTowTruckDriverGetPaid", source)) then
					...
					exports.global:takeMoney(getTeamFromName("Los Santos Policijas Departaments"), 75)
				end
				exports.anticheat:changeProtectedElementDataEx(veh, "requires.vehpos")
				local rx, ry, rz = getVehicleRotation(veh)
				...
				exports.anticheat:changeProtectedElementDataEx(veh, "interior", interior)
				exports.anticheat:changeProtectedElementDataEx(veh, "dimension", dimension)
				outputChatBox("Auto atjaunošanās punkts uzstādīts.", source)
				exports.logs:dbLog(thePlayer, 6, {  veh }, "PARK")
				for key, value in ipairs(destroyTimers) do
					if (tonumber(destroyTimers[key][2]) == dbid) then
						local timer = destroyTimers[key][1]
			...
			outputChatBox( "Tu nevari noparkot savu auto.", source, 255, 0, 0 )
		end
	end
end
...
function setVehiclePosition4(thePlayer, commandName, vehicle)
	...
	if not veh or commandName and not vehicle and getElementData(thePlayer, "realinvehicle") == 0 then
		outputChatBox("Tu neatrodies iekš auto.", thePlayer, 255, 0, 0)
	else
		local playerid = getElementData(thePlayer, "dbid")
		local owner = getElementData(veh, "owner")
		local dbid = getElementData(veh, "dbid")
		...
		if (leader) and (f) then
			...
			exports.anticheat:changeProtectedElementDataEx(veh, "interior", interior)
			exports.anticheat:changeProtectedElementDataEx(veh, "dimension", dimension)
			outputChatBox("Auto atjaunošanās punkts #" .. dbid .. " uzstādīts.", thePlayer)
			exports.logs:dbLog(thePlayer, 6, {  veh }, "PARK")
...
function detachVehicle(thePlayer)
	if isPedInVehicle(thePlayer) and getPedOccupiedVehicleSeat(thePlayer) == 0 then
		...
		if getVehicleTowedByVehicle(veh) then
			...
			outputChatBox("Traileris atvienots.", thePlayer, 0, 255, 0)
		else
			outputChatBox("Nav treilera...", thePlayer, 255, 0, 0)
		end
	end
end
