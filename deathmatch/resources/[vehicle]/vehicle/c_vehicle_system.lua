...
function build_SaleGUI(itemValue)
	...
	gui["_root"] = guiCreateWindow(left, top, windowWidth, windowHeight, "CSDD", false)
	guiWindowSetSizable(gui["_root"], false)

	gui["label"] = guiCreateLabel(170, 25, 70, 45, "Auto pārdošana", false, gui["_root"])
	guiLabelSetHorizontalAlign(gui["label"], "left", false)
	guiLabelSetVerticalAlign(gui["label"], "center")

	gui["label_2"] = guiCreateLabel(30, 75, 331, 21, "Parakstot šo dokumentu, es piekrītu atdod visas īpašumtiesības", false, gui["_root"])
	guiLabelSetHorizontalAlign(gui["label_2"], "left", false)
	guiLabelSetVerticalAlign(gui["label_2"], "center")

	gui["label_3"] = guiCreateLabel(30, 95, 331, 21, "pār šo automašīnu.", false, gui["_root"])
	...
	gui["label_4"] = guiCreateLabel(30, 135, 150, 16, "Jaunā īpašnieka vārds", false, gui["_root"])
	guiLabelSetHorizontalAlign(gui["label_4"], "left", false)
	guiLabelSetVerticalAlign(gui["label_4"], "center")

	gui["pushButton"] = guiCreateButton(180, 195, 91, 31, "Pārdot", false, gui["_root"])
	...
	gui["pushButton_2"] = guiCreateButton(290, 195, 91, 31, "Atcelt", false, gui["_root"])
...
function openVehSales(cmd, ...)
	...
	if veh then
		...
		if (exports.integration:isPlayerAdmin(localPlayer) and getElementData(localPlayer, "duty_admin") == 1 ) or (getElementData(veh, "owner") == getElementData(localPlayer, "dbid")) or (leader and faction) then
			...
			GUIEditor.window[1] = guiCreateWindow(1107, 377, 391, 181, "CSDD", false)
			guiWindowSetSizable(GUIEditor.window[1], false)
			exports.global:centerWindow(GUIEditor.window[1])

			GUIEditor.label[1] = guiCreateLabel(35, 36, 323, 79, "Tu taisies pārdot ID# () citam spēlētājam.\n\nLai samazinātu OOC auto scamus, lūdzu ierakstiet to daudzumu naudas par cik Jūs sarunājāt veikt darījumu:", false, GUIEditor.window[1])
			...
			return outputChatBox("Tev nepieder šis auto vai arī nēesi tās frakcijas līderis, kurai tas pieder.", 255, 0, 0)
		end
	end
end
...
addEventHandler("onClientVehicleExit", getRootElement(), function(thePlayer)
	if thePlayer == localPlayer and (getVehicleType(source) == "Bike" or getVehicleType(source) == "BMX") and not clientStartExit then
		...
		if isVehicleLocked(source) then
			...
			outputChatBox("Tavs bmx/mocis tika automātiski atslēgts, jo nokriti no tā.", 255, 0, 0)
		end
		clientStartExit = false
