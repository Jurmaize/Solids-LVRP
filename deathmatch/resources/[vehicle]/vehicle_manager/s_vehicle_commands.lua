	...
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) or exports.integration:isPlayerVehicleConsultant(thePlayer) then
		if not (id) or not (toPlayer) then
			...
			if theVehicle then
				...
				if getElementData(thePlayer, "hiddenadmin") == 1 then
					outputChatBox("Neredzams Administrators atteleportēja auto pie Jums.", targetPlayer, 255, 194, 14)
				else
					outputChatBox(exports.global:getPlayerFullIdentity(thePlayer).." atteleportēja auto pie Jūs.", targetPlayer, 255, 194, 14)
				end
			else
				outputChatBox("Invalid Vehicle ID.", thePlayer, 255, 0, 0)
			end
				...
				if (logged==0) then
					...
					if (veh) then
						...
						if thePlayer == targetPlayer then
							outputChatBox("Tu salaboji savu auto.", thePlayer, 100, 255, 100)
						else
							outputChatBox("Tu salaboji " .. targetPlayerName .. " auto.", thePlayer, 100, 255, 100)
							outputChatBox("Tavu auto salaboja administrātors " .. username:gsub("_", " ") .. ".", targetPlayer, 100, 255, 100)
						end
					else
						outputChatBox("Tas spēlētājs nav autiņā.", thePlayer, 255, 0, 0)
					end
				end
			end
		end
...
function fixAllVehicles(thePlayer, commandName)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		...
		executeCommandHandler("ann", thePlayer, "Visas automašīnas salaboja administrātors " .. username .. ".")
