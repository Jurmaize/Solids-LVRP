...
function startTalkToPed ()
	...
	if not (isPedInVehicle(thePlayer)) or (isPedInVehicle(thePlayer) and getVehicleType(getPedOccupiedVehicle(thePlayer)) == "BMX") then
		processMessage(thePed, "Labdien, kā varu palīdzēt?")
 		setConvoState(thePlayer, 3)
		local responseArray = { "Vai Jūs varētu uzpildīt manu degvielas kanniņu?", "Eh.. īstenībā neko.", "Vai jums ir cigarete priekš manis?", "Man patīk tavs tērps." }
		...
		if (exports.vehicle:isVehicleWindowUp(theVehicle)) then
			outputChatBox("Nolaid logu pirms runā ar kādu ārpusē!", thePlayer, 255,0,0)
			return
		end
		-- processMeMessage(thePed, "leans against " .. getPlayerName(thePlayer):gsub("_"," ") .. "'s vehicle.", thePlayer )
		triggerEvent('sendAme', thePed, "atliecas pret " .. getPlayerName(thePlayer):gsub("_"," ") .. " auto.")
		processMessage(thePed, "Labdien, kā varu palīdzēt?")
 		setConvoState(thePlayer, 1)
		local responseArray = { "Man vajadzētu nedaudz degvielas.", "Nē paldies.", "Vai Jums ir cigarete priekš manis?", "Beidz balstīties pret mana auto!" }
		triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
	end
end
...
function talkToPed(answer, answerStr)
	...
	if (convState == 1) then -- "Hey, how could I help you?"
		...
		if (languageSkill < 60) or (currLang ~= 1) then
			processMessage(thePed, "Uh, atvaino, es nerunāju angliski!")
		...
		if (answer == 1) then -- "Ehm, fill my tank up, please."
			...
			if (getElementData(theVehicle, "engine") == 1) then
				processMessage(thePed, "Vai Jūs lūdzu varētu izslēgt auto dzinēju?")
				local responseArray = { "Jā, mierīgi, bez problēmām", "Tu to nevari izdarīt ar dzinēju ieslēgtu?", "Eh, KO BĻE?" }
				triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
				setConvoState(thePlayer, 2)
				return
			elseif getElementData(theVehicle, 'fuel') > (getMaxFuel(getElementModel(theVehicle))-1) then
				processMessage(thePed, "Izskatas diezgan pilns no mana skatu punkta.")
			else
				processMessage(thePed, "Mierīgi... cik vēlaties samaksāt?")
				if getATMCardFromATMMachine(thePlayer) then
					local responseArray = { "Skaidrā lūdzu!", "Es maksāšu ar bankas palīdzību.", "Ko jūs teicāt?" }
					triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
				else
					local responseArray = { "Skaidrā lūdzu!", false, "Ko jūs teicāt?" }
					triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
				end
		...
		elseif (answer == 2) then -- "No thanks."
			processMessage(thePed, "Labi, braucat atpakaļ, kad ir nepieciešama degviela.")
			setConvoState(thePlayer, 0)
		elseif (answer == 3) then -- "Do you have a sigarette for me?"
			processMessage(thePed, "Emm nē. Tu varētu pačekot tuvākos veikalus.")
			setConvoState(thePlayer, 0)
		elseif (answer == 4) then -- stop leaning against my car
			processMessage(thePed, "Okay, okay, nelec no biksītēm ārā.")
			--processMeMessage(thePed, "pushes himself up again, standing on his feet.", thePlayer )
			triggerEvent('sendAme', thePed, "pieceļas stāvus.")
			processMessage(thePed, "Tad man uzpildīt vai nē?")
			local responseArray = {  "Droši!", "Nē, vairs nepildīsiet." }
			triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
			setConvoState(thePlayer, 1)
		end
	elseif (convState == 2) then -- "Could you please turn your engine off?"
		if (answer == 1) then -- "Sure, no problemo." / "Ok, okay.."
			...
			triggerEvent('sendAme', thePlayer, "izslēdz auto dzinēju.")
			...
			processMessage(thePed, "Labi, paldies. Kā jūs vēlaties samaksāt.")
			if getATMCardFromATMMachine(thePlayer) then
				local responseArray = { "Skaidrā lūdzu!", "Samaksāšu ar bankas palīdzību", "Ko bļe?" }
				triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
			else
				local responseArray = { "Skaidrā lūdzu!", false, "Atvainojos ko?" }
				triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
			end
			setConvoState(thePlayer, 4)
		elseif (answer == 2) then -- "Can't you do it with the engine running?"
			--processMeMessage(thePed, "sighs.",thePlayer )
			triggerEvent('sendAme', thePed, "nopūšas.")
			processMessage(thePed, "Ehm... nē. Es negribu nomirt. Tad slēgsiet ārā vai nē?")
			local responseArray = {  "Droši.", false, false, "Nu labi, tad aizpisies."  }
			triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
			setConvoState(thePlayer, 2)
		elseif (answer == 3) then -- "Eh, WHAT?"
			processMessage(thePed, "Es jautāju: Vai Jūs varētu izslēgt auto dzinēju!")
			local responseArray = {  "Labi", false,false, "Ugh, nē."  }
			triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
			setConvoState(thePlayer, 2)
		elseif answer == 4 then -- "Ugh, shut up then." / "Ugh, no."
			processMessage(thePed, "Labi, labi, nu tad tik pisies prom.")
			setConvoState(thePlayer, 0)
		end
	elseif (convState == 3) then
		if answer == 1 then -- Could you fill my fuelcan?
			if (exports.global:hasItem(thePlayer, 57)) then
				processMessage(thePed, "Davaj, daram!")
				...
				triggerEvent('sendAme', thePed, "paķer šļauku!")
				triggerEvent('sendAme', thePed, "attaisa kanniņas korķi, lēnām piepildot kanniņu.")
				setTimer(pedWillFillFuelCan, 3500, 1, thePlayer, thePed)
			else
				processMessage(thePed, "Uhm.. tev nav degvielas kanniņas. Tu vari dabūt vienu no tuvākā veikala.")
				setConvoState(thePlayer, 0)
			end
		elseif answer == 2 then -- No thanks
			processMessage(thePed, "Labi, lai Jums jauka diena..")
			setConvoState(thePlayer, 0)
		elseif answer == 3 then -- do you have a cigarette for me?
			processMessage(thePed, "Nē, nav. Varat iegādāties to tuvākajā veikalā.")
			setConvoState(thePlayer, 0)
		elseif answer == 4 then -- I like your suit
			processMessage(thePed, "Eh... paldies?")
			setConvoState(thePlayer, 0)
		end
	elseif (convState == 4) then -- How would you like to pay?
		if answer == 1 or answer == 2 then  -- Bank = 2 Cash = 1
			...
			processMessage(thePed, "Labi tad, cik jūs vēlaties, lai iepildu?")
			local responseArray = { "$40", "$60", "$100", "Līdz lūpai!" }
			triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
			exports.anticheat:changeProtectedElementDataEx(thePlayer, "ped:Type", answer, false)
			setConvoState(thePlayer, 5)
		elseif answer == 3 then
			processMessage(thePed, "Es vaicāju cik Jūs vēlaties iepildīt!")
			if getATMCardFromATMMachine(thePlayer) then
				local responseArray = { "Skaidrā lūdzu!", "Lietošu bankas starpniecību!", "Ko?", "Vienkārši aizmirstat." }
				triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
			else
				local responseArray = { "Skaidrā lūdzu!", false, "Ko?", "Vienkārši aizmirstat." }
				triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
			end
			setConvoState(thePlayer, 4)
		elseif answer == 4 then
			processMessage(thePed, "Labi, tad vienalga..")
			setConvoState(thePlayer, 0)
		end
	elseif (convState == 5) then
		...
		if getElementData(theVehicle, 'fuel') > (getMaxFuel(getElementModel(theVehicle))-1) then
			processMessage(thePed, "Man izskatas diezgan pilna.")
		else
			pedWillFillVehicle(thePlayer, thePed)
		end
	end
...
function pedWillFillFuelCan(thePlayer, thePed)
	...
	local pedX, pedY, pedZ = getElementPosition(thePed)
	if not (getDistanceBetweenPoints3D(posX, posY, posZ, pedX, pedY, pedZ) <= 7) then
		exports['chat-system']:localShout(thePed, "do", "Labi, nedabūsiet tad degvielu!")
		return
	end

	local hasItem, itemSlot, itemValue, itemUniqueID = exports.global:hasItem(thePlayer, 57)
	if not (hasItem) then
		processMessage(thePed, "Smieklīgi...")
		--processMeMessage(thePed, "sighs.",thePlayer )
		triggerEvent('sendAme', thePed, "nopūšas.")
		return
	end
	...
	if itemValue >= 25 then
		processMessage(thePed, "Eh... tā jau ir pilna.")
		return
	end
	...
	if tonumber(money) == 0 then
		processMessage(thePed, "Kā tu domāji pa šo samaksāt?! Daunis!")
		return
	else
		if not exports.global:takeMoney(thePlayer, fuelCost) then
			processMessage(thePed, "Jā, šis Jums izmaksās $" .. fuelCost .. ", zināt to?")
			return
		end
	...
	if not (exports['item-system']:updateItemValue(thePlayer, itemSlot, itemValue + theLitres)) then
		outputChatBox("Kaut kas aizgāja pa pipeli, rakstiet /report.", thePlayer)
		return
	end
	...
	local info = {
			{"STATOIL čeks"},
			{""},
			{"    " .. math.ceil(theLitres) .. " Litri ar petroleju    -    " .. fuelCost .. "$"},
		}
...
function pedWillFuelTheVehicle(thePlayer, thePed, amount, moneyType)
	...
	local pedX, pedY, pedZ = getElementPosition(thePed)
	if not (getDistanceBetweenPoints3D(posX, posY, posZ, pedX, pedY, pedZ) <= 7) then
		exports['chat-system']:localShout(thePed, "do", "EU IDIOT! VĒLIES NOMIRT?")
		return
	end
	...
	if exports.factions:isPlayerInFaction(thePlayer, factionVehicle) and factionsThatPayForFuel[factionVehicle] then
		...
		if exports.global:takeMoney(theTeam, fuelCost, true) then
			processMessage(thePed, "Aizsūtīšu čeku Jūsu darba vadībai.")
			...
			if not exports.donators:hasPlayerPerk(thePlayer, 7) then
				if moneyType == 1 then
					if exports.global:takeMoney(thePlayer, fuelCost) then
						processMessage(thePed, "Šeit ir jūsu čeks.")
					else
						processMessage(thePed, "Kā to par šo taisījies samaksāt, dauni?")
						return
					end
				else
					if exports.bank:updateBankMoney(thePlayer, getElementData(thePlayer, "dbid"), fuelCost, "minus") then
						...
						processMessage(thePed, "Jūsu karte tika atcelta...")
						return
					end
				end
			else
	...
	if exports.donators:hasPlayerPerk(thePlayer, 7) and not factionPaid then
		table.insert(info, {"    " .. math.ceil(theLitres) .. " litri ar petroleju    - (( Pabrīvu degviela ))"})
	else
		table.insert(info, {"    " .. math.ceil(theLitres) .. " litri ar petroleju    -    " .. fuelCost .. "$"})
		if factionPaid then
			table.insert(info, {"    Samaksāja darbadevējs "..tostring(exports.factions:getFactionName(factionVehicle))})
		end
	end
...
function payShopOwner(thePed, fuelCost)
	...
	outputDebugString( 'Veikala links: ' .. shopLink )
	if shopLink > 0 then
		local money = math.floor( tonumber( fuelCost ) * 0.4 ) -- give 40% to shop owner
		outputDebugString( 'Nauda: ' .. money )
		...
		if row then
			for index, shopPed in pairs( getElementsByType( 'ped')) do
				if tonumber( getElementData( shopPed, "dbid") ) == tonumber( row.id ) and getElementData( shopPed, "ped:type" ) == 'shop' then
					outputDebugString( 'Veikala ID: ' .. row.id )
...
function makeFuelPed(thePlayer, commandName, skin, ...)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		...
		if (id) then
			...
			outputChatBox("Kļūda veidojot bendzīntanka pedu.", thePlayer, 255, 0, 0)
		end
	end
end
...
function getNearByFuelPeds(thePlayer, commandName) --maxime
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) then
		local posX, posY, posZ = getElementPosition(thePlayer)
		outputChatBox("Tuvumā esošie degvielas NPC:", thePlayer, 255, 126, 0)
		local count = 0
			...
			if (pedType) then
				if (pedType=="fuel") then
					...
					if (distance<=10) and (dimension==cdimension) then
						...
						local shopLink = tonumber(  getElementData(thePed, "shop_link") )
						outputChatBox("   Degvielas NPC ID #" .. dbid .. ", vārds: "..tostring(pedName):gsub("_", " ") .. ( shopLink > 0 and ' veikala links: ' .. shopLink or ''), thePlayer, 255, 126, 0)
						count = count + 1
					end
				end
			end
	...
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) then
		if not tonumber(shopID) then
			...
			if not foundShop then
				outputChatBox("Nav veikals ar ID #"..shopID, thePlayer, 255, 0, 0)
				return false
			end
...
function startGoingToShop(thePlayer, x,y,z,r,interior,dimension,shopID) --maxime
	...
	outputChatBox(" Tu aizteleportējies uz degvielas NPC ID#"..shopID, thePlayer)
end
...
function deleteFuelPed(thePlayer, commandName, id) -- maxime
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not (id) then
				...
				if (pedType) then
					if (pedType=="fuel") then
						...
						if (tonumber(id)==dbid) then
							...
							outputChatBox("      Izdzēsi degvielas NPC ar ID #" .. id .. ".", thePlayer, 0, 255, 0)
							counter = counter + 1
			...
			if (counter==0) then
				outputChatBox("Nav degvielas NPC ar tādu ID", thePlayer, 255, 0, 0)
				return false
			end
			return true
		end
				...
				if exports.mysql:num_rows( findFuelPed ) > 0 then
					...
					if exports.mysql:num_rows( findShop ) > 0 then
						...
						if exports.mysql:num_rows( findShopPed ) > 0 then
							...
							outputChatBox( 'Tas interjers tika savienots ar degvielas npc.', player, 155, 255, 155 )
						else
							outputChatBox( 'Tam veikalam nav veikala tips ar ID 15.', player, 255, 155, 155 )
						end
						exports.mysql:free_result( findShopPed )
					else
						outputChatBox( 'Tāds interjers netika atrasts.', player, 255, 155, 155 )
					end
					mysql:free_result( findShop )
				else
					outputChatBox( 'Nav tāda degvielas NPC.', player, 255, 155, 155 )
				end
