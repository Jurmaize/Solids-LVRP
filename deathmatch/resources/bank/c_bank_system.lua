...
function startFactionSelect(fTable, selectiontype)
    ...
    factionSelectGUI.window[1] = guiCreateWindow(766, 385, 399, 121, "Izvēlaties frakciju, ko skatīt", false)
    ...
    factionSelectGUI.button[1] = guiCreateButton(13, 64, 111, 42, "Atcelt", false, factionSelectGUI.window[1])
    guiSetProperty(factionSelectGUI.button[1], "NormalTextColour", "FFAAAAAA")
    factionSelectGUI.combobox[1] = guiCreateComboBox(13, 35, 366, 113, "Izvēlaties frakciju, ko skatīt", false, factionSelectGUI.window[1])
    factionSelectGUI.button[2] = guiCreateButton(268, 64, 111, 42, "Apstiprināt", false, factionSelectGUI.window[1])
    ...
    addEventHandler("onClientGUIClick", factionSelectGUI.button[2], function()
    	...
    	if name ~= "Izvēlaties frakciju, ko skatīt" then
    		...
    		outputChatBox("Lūdzu izvēlaties frakciju!", 255, 0, 0)
    	end
    end, false)
...
function fillBusiness(name)
	if wBank and isElement(wBank) then
		...
		if faction then
			...
			guiSetText(lBalanceB, "Bilance: $" .. exports.global:formatMoney(gfactionBalance))
		end
	end
end
...
function showBankUI(fTable, depositable, limit, withdrawable, ped)
	if not (wBank) then
		...
		wBank = guiCreateWindow(x, y, width, height, "Los Santos Banka | "..version.."", false)
		...
		tabPersonalTransactions = guiCreateTab("Personīgās tranzakcijas", tabPanel)

		local hoursplayed = getElementData(localPlayer, "hoursplayed") or 0
		if (exports.global:countTable(fTable) > 0) then
			tabBusiness = guiCreateTab("Biznesa konts", tabPanel)

			lBalanceB = guiCreateLabel(0.1, 0.05, 0.9, 0.05, "Bilance: $", true, tabBusiness)
			guiSetFont(lBalanceB, "default-bold-small")

			if (withdrawable) then
			-- WITHDRAWAL BUSINESS
				lWithdrawB = guiCreateLabel(0.1, 0.15, 0.2, 0.05, "Izņemt:", true, tabBusiness)
				...
				bWithdrawB = guiCreateButton(0.44, 0.13, 0.2, 0.075, "Izņemt", true, tabBusiness)
				addEventHandler("onClientGUIClick", bWithdrawB, withdrawMoneyBusiness, false)
			else
				lWithdrawB = guiCreateLabel(0.1, 0.15, 0.5, 0.05, "Šajā bankomātā nevar izņemt naudu.", true, tabBusiness)
				guiSetFont(lWithdrawB, "default-bold-small")
			end

			if (depositable) then
				-- DEPOSIT BUSINESS
				lDepositB = guiCreateLabel(0.1, 0.25, 0.2, 0.05, "Iemaksāt:", true, tabBusiness)
				...
				bDepositB = guiCreateButton(0.44, 0.23, 0.2, 0.075, "Iemaksāt", true, tabBusiness)
				addEventHandler("onClientGUIClick", bDepositB, depositMoneyBusiness, false)
			else
				lDepositB = guiCreateLabel(0.1, 0.25, 0.5, 0.05, "Šajā bankomātā nevar iemaksāt naudu.", true, tabBusiness)
				guiSetFont(lDepositB, "default-bold-small")

				if limitedwithdraw > 0 and withdrawable then
					tDepositB = guiCreateLabel(0.67, 0.15, 0.2, 0.05, "Maksimums: $" .. exports.global:formatMoney( limitedwithdraw - ( getElementData( source, "withdrawn" ) or 0 ) ) .. ".", true, tabBusiness)
					guiSetFont(tDepositB, "default-bold-small")
				end
			end
			...
			if hoursplayed >= 12 then
				-- TRANSFER BUSINESS
				lTransferB = guiCreateLabel(0.1, 0.45, 0.2, 0.05, "Pārsūtīt:", true, tabBusiness)
				...
				bTransferB = guiCreateButton(0.44, 0.43, 0.2, 0.075, "Pārsūtīt uz", true, tabBusiness)
				addEventHandler("onClientGUIClick", bTransferB, transferMoneyBusiness, false)

				eTransferB = guiCreateEdit(0.66, 0.43, 0.3, 0.075, "<Bankas konta nummurs>", true, tabBusiness)
				addEventHandler("onClientGUIClick", eTransferB, function()
					if guiGetText(eTransferB) == "<Bankas konta nummurs>" then
						guiSetText(eTransferB, "")
					end
				end, false)

				lTransferBReason = guiCreateLabel(0.1, 0.55, 0.2, 0.05, "Iemesls:", true, tabBusiness)
				guiSetFont(lTransferBReason, "default-bold-small")

				tTransferBReason = guiCreateEdit(0.22, 0.54, 0.74, 0.075, "<Transakcijas iemsls>", true, tabBusiness)
				addEventHandler("onClientGUIClick", tTransferBReason, function()
					if guiGetText(tTransferBReason) == "<Transakcijas iemsls>" then
			...
			-- TRANSACTION HISTORY
			tabBusinessTransactions = guiCreateTab("Biznesa tranzakcijas", tabPanel)
		...
		bClose = guiCreateButton(0.75, 0.91, 0.2, 0.1, "Aizvērt", true, wBank)
		...
		--shit
		lBalance = guiCreateLabel(0.1, 0.05, 0.9, 0.05, "Bilance: $" .. exports.global:formatMoney(balance), true, tabPersonal)
		guiSetFont(lBalance, "default-bold-small")

		if withdrawable then
			-- WITHDRAWAL PERSONAL
			lWithdrawP = guiCreateLabel(0.1, 0.15, 0.2, 0.05, "Izņemt:", true, tabPersonal)
			...
			bWithdrawP = guiCreateButton(0.44, 0.13, 0.2, 0.075, "Izņemt", true, tabPersonal)
			addEventHandler("onClientGUIClick", bWithdrawP, withdrawMoneyPersonal, false)
		else
			lWithdrawP = guiCreateLabel(0.1, 0.15, 0.5, 0.05, "Šajā bankomātā nav iespējams izņemt naudu.", true, tabPersonal)
			guiSetFont(lWithdrawP, "default-bold-small")
		end

		if (depositable) then
			-- DEPOSIT PERSONAL
			lDepositP = guiCreateLabel(0.1, 0.25, 0.2, 0.05, "Iemaksāt:", true, tabPersonal)
			...
			bDepositP = guiCreateButton(0.44, 0.23, 0.2, 0.075, "Iemaksāt", true, tabPersonal)
			addEventHandler("onClientGUIClick", bDepositP, depositMoneyPersonal, false)
		else
			lDepositP = guiCreateLabel(0.1, 0.25, 0.5, 0.05, "Šajā bankomātā nevar iemaksāt naudu.", true, tabPersonal)
			guiSetFont(lDepositP, "default-bold-small")

			if limitedwithdraw > 0 and withdrawable then
				tDepositP = guiCreateLabel(0.67, 0.15, 0.2, 0.05, "Maksimums: $" .. ( limitedwithdraw - ( getElementData( source, "withdrawn" ) or 0 ) ) .. ".", true, tabPersonal)
				guiSetFont(tDepositP, "default-bold-small")
			end
		end
		...
		if hoursplayed >= 12 then
			-- TRANSFER PERSONAL
			lTransferP = guiCreateLabel(0.1, 0.45, 0.2, 0.05, "Pārsūtīt uz:", true, tabPersonal)
			...
			bTransferP = guiCreateButton(0.44, 0.43, 0.2, 0.075, "Pārsūtīt uz", true, tabPersonal)
			addEventHandler("onClientGUIClick", bTransferP, transferMoneyPersonal, false)

			eTransferP = guiCreateEdit(0.66, 0.43, 0.3, 0.075, "<Spēlētaja/Frakcijas nosaukums>", true, tabPersonal)
			addEventHandler("onClientGUIClick", eTransferP, function()
				if guiGetText(eTransferP) == "<Spēlētaja/Frakcijas nosaukums>" then
					guiSetText(eTransferP, "")
				end
			end, false)

			lTransferPReason = guiCreateLabel(0.1, 0.55, 0.2, 0.05, "Iemesls:", true, tabPersonal)
			guiSetFont(lTransferPReason, "default-bold-small")

			tTransferPReason = guiCreateEdit(0.22, 0.54, 0.74, 0.075, "<Transakcijas iemesls>", true, tabPersonal)
...
function withdrawMoneyPersonal(button)
	if (button=="left") then
		...
		if not amount or amount <= 0 or math.ceil( amount ) ~= amount then
			outputChatBox("Lūdzu ievadiet pozitīvu daudzumu!", 255, 0, 0)
		elseif (amount>money) then
			outputChatBox("Jums nav tik daudz naudas. LOL", 255, 0, 0)
		elseif not _depositable and limitedwithdraw ~= 0 and oldamount + amount > limitedwithdraw then
			outputChatBox("Šis bankomāts atļauj izņemt tikai $" .. exports.global:formatMoney( limitedwithdraw - oldamount ) .. ".")
		else
...
function depositMoneyPersonal(button)
	if (button=="left") then
		...
		elseif not exports.global:hasMoney(localPlayer, amount) then
			outputChatBox("Jums nav pietiekami naudas! LOL", 255, 0, 0)
		else
			hideBankUI()
...
function transferMoneyPersonal(button)
	if (button=="left") then
		...
		elseif (amount>money) then
			outputChatBox("Jums nav pietiekami naudas! LOL", 255, 0, 0)
		elseif reason == "" then
			outputChatBox("Lūdzu uzrakstiet transakcijas iemeslu!", 255, 0, 0)
		elseif playername == "" then
			outputChatBox("Lūdzu ievadiet pilnu personas vārdu!", 255, 0, 0)
		else
...
function withdrawMoneyBusiness(button)
	if (button=="left") then
		...
		elseif (amount>gfactionBalance) then
			outputChatBox("Jums nav pietiekami naudas! LOL", 255, 0, 0)
		elseif not _depositable and limitedwithdraw ~= 0 and oldamount + amount > limitedwithdraw then
			outputChatBox("Šis bankomāts ļauj izņemt tikai $" .. exports.global:formatMoney( limitedwithdraw - oldamount ) .. ".")
		else
			setElementData( lastUsedATM, "withdrawn", oldamount + amount, false )
			setTimer(
				function( atm, amount )
...
function transferMoneyBusiness(button)
	if (button=="left") then
		...
		elseif (amount>gfactionBalance) then
			outputChatBox("Jums nav pietiekami naudas! LOL", 255, 0, 0)
		elseif reason == "" then
			outputChatBox("Lūdzu ievadiet transakcijas iemeslu!", 255, 0, 0)
		elseif playername == "" then
			outputChatBox("Lūdzu ievadiet saņēmēja pilno vārdu!", 255, 0, 0)
		else
