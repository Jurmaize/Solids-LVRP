...
function createShopKeeper(x,y,z,interior,dimension,id,shoptype,rotation, skin, sPendingWage, sIncome, sCapacity, currentCap, sSales, pedName, sContactInfo, faction_belong, faction_access)
	if not g_shops[shoptype] then
		outputDebugString("Cenšas atrast shopu ar ID #" .. id .. " ar nederīgu veikala tipu " .. shoptype)
		return
	end
...
function getNearbyGeneralshops(thePlayer, commandName)
	...
	if (found and status.owner == getElementData(thePlayer, "dbid")) or exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) then
		...
		outputChatBox("Parasti spēlētāji šo komandu var lietot tikai savos veikalos!", thePlayer, 255, 194, 14)
	end
end
...
function moveNPCshop(thePlayer, commandName, value)
	...
	if not foundShop then
		outputChatBox("Nav veiakala ar ID #"..value, thePlayer, 255, 0, 0)
		return
	elseif getElementDimension(foundShop) ~= dim then
		outputChatBox("Veikala NPC ir jābūt tajā pašā interjerā, kurā jūs to cenšaties novietot.", thePlayer, 255, 0, 0)
		return
	end
	...
	outputChatBox("Atjaunoja veikala NPC pozīciju.", thePlayer, 0, 255, 0)
...
function gotoShop(thePlayer, commandName, shopID)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not tonumber(shopID) then
			...
			if not foundShop then
				outputChatBox("Nav veikala NPC ar ID #"..shopID, thePlayer, 255, 0, 0)
				return false
			end
...
function clickStoreKeeper()
	...
	if not success then
		outputChatBox(currentUser.." šobrīd lieto šo NPC, uzgaidiet brīdi!", client, 255,0,0)
		return false
	end
...
addEventHandler( "shop:buy", resourceRoot, function( index )
	...
	_G['shop'] = shop
	if not shop then
		outputChatBox("Kļūda " .. error .. ", reporto https://discord.gg/S5PDPdDeac.", client, 255, 0, 0 )
		return
	end
	...
	if not item then
		outputChatBox("Error " .. error .. "-NEI-" .. index .. ", reporto https://discord.gg/S5PDPdDeac.", client, 255, 0, 0 )
		return
	end
	...
	if not exports.global:hasSpaceForItem( client, item.itemID, item.itemValue ) then
		outputChatBox("Tavas kabatas ir pilnas.", client, 255, 0, 0)
		return
	end

	--checking age
	if item.minimum_age and getElementData(client, "age") < item.minimum_age then
		outputChatBox( "Tev ir jābūt vismaz " .. item.minimum_age .. " gadus vecam, lai iegādātos šo preci.", client, 255, 0, 0 )
		return
	end

	-- check for money
	if not exports.global:hasMoney( client, price ) then
		outputChatBox( "Tev nepietiek naudas, lai iegādātos " .. item.name .. ".", client, 255, 0, 0 )
		return
	end
	...
	elseif itemID == 68 then -- Lottery Tickets / Check and
		outputChatBox( "Šī prece ir šobrīd izslēgta.", client, 255, 0, 0 )
	...
	elseif itemID == 115 or itemID == 116 then
		if item.license and getElementData( client, "license.gun" ) ~= 1 then
			outputChatBox( "Tev nav ieroču licenzes.", client, 255, 0, 0 )
			return
		else
	...
	if exports.global:takeMoney( client, price ) and exports.global:giveItem( client, itemID, itemValue, item.metadata ) then
		outputChatBox( "Tu nopirki " .. item.name .. " pa $" .. exports.global:formatMoney( price ) .. ".", client, 0, 255, 0 )
	else
		outputChatBox( "Kļūda!", client, 255, 0, 0 )
		return
	end
	...
	if itemID == 2 then
		...
		outputChatBox("Tavs nummurs ir #" .. itemValue .. ".", client, 255, 194, 14 )
	elseif itemID == 114 then -- vehicle mods
		outputChatBox("Lai pievienotu šo preci savam auto, atrodaties garāžā un uzspiežat divas reizes uz tā, kamēr sēžat savā auto.", client, 255, 194, 14 )
	elseif itemID == 115 then -- log weapon purchases
		exports.logs:dbLog( client, 36, client, "nopirka IEROCI - " .. itemValue )

		local govMoney = math.floor( price / 6 )
		exports.global:giveMoney(getTeamFromName("Los Santos Valdība"), govMoney)
		price = price - govMoney -- you'd obviously get less if the gov asks for percentage.
	elseif itemID == 116 then -- log weapon purchases
		exports.logs:dbLog( client, 36, client, "nopirka MUNĪCIJU - " .. itemValue )
		...
		if setShopStats(source, 'sPendingWage', 0) and setShopStats(source, 'sIncome', 0) then
			if faction then
				if exports.bank:giveBankMoney(faction, profit) then
					exports.bank:addBankTransactionLog(0, -getElementData(faction, 'id'), profit, 13, 'Ienākumi no veikala '..tostring(getPedName(source)), getPedName(source))
					playCollectMoneySound(source)
					outputChatBox("Kopējais $"..exports.global:formatMoney(profit).." daudzums ienākumu ir pārskaitīts uz "..getTeamName(faction).." frakcijas banku.", client, 0, 255, 0)
...
function checkSupplies(thePlayer)
    ...
    if (dbid==0) then
        ...
        if exports.integration:isPlayerTrialAdmin(thePlayer) or tonumber(owner)==getElementData(thePlayer, "dbid") or exports.global:hasItem(thePlayer, 4, dbid) or exports.global:hasItem(thePlayer, 5, dbid) then
            outputChatBox("Tu vari pārbaudīt inventāru atverot tā veikala NPC izvēlni.", thePlayer, 255, 194, 14)
        else
            outputChatBox("Tu neesi biznesā vai tas nav tavs bizness!", thePlayer, 255, 0, 0)
        end
    end
end
...
function canIUseThisName(pedName)
	...
	if row3 then
		return false, "Kādam spēlētājam jau ir veikals ar šādu nosaukumu '"..pedName.."'."
	end
...
function updateShopSupplies(interiorID, supplies, thePlayer, cost, pedName)
	...
	if success then
		...
		if query then
			...
			exports.bank:addBankTransactionLog(getElementData(thePlayer, "dbid"), nil, cost, 0, "SHOP RESTOCK", "Iegādājās "..tostring(cost).."$ daudzumu ar inventāru priekš biznesa ar ID"..tostring(interiorID), nil, nil)
			storeKeeperSay(thePlayer, "Mēs esam sagādājuši Jums pusi no pasūtītajām precēm. Otru pusi piegādās RS Haul preču parvadātāji!", pedName)
		end
	else
		outputChatBox(why, thePlayer, 255, 0, 0)
	end
