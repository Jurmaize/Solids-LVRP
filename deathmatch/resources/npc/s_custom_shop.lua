...
function addItemToCustomShop(element, slot, event, worldItem)
	...
	if exports.global:hasItem(source, 4, dbid) or exports.global:hasItem(source, 5, dbid) then
		...
		outputChatBox("Tev jābūt ir atslēgai, lai papildinātu preces.",source, 255,0,0)
		return false
	end
...
function updateItemToShop(source, item, price, desc, npcID, slot, worldItem, shopElement)
	...
	if tonumber(getElementData(shopElement, "sIncome")) < tonumber(getElementData(shopElement, "sPendingWage")) then
		storeKeeperSay(source, "Kā ar manu algu??", pedName)
	...
	if item['id'] == 116 or ( item['id'] == 115 and not allowedWeapons[tonumber( split( item['value'], ":")[1] )] ) then
		storeKeeperSay(source, "Ej tu dirst! Mums nav atļaujas šo pārdot!", pedName)
		setElementData(source, "shop:NoAccess", false, true )
		triggerClientEvent(source, "hideGeneralshopUI", source)
		return false
	end
	...
	if item['id'] == 3 or item['id'] == 4 or item['id'] == 5 then --Keys , to prevent alt->alt
		...
		storeKeeperSay(source, "Haha, tu tiešām domā, kāds pirktu tādu atslēgu?", pedName)
		setElementData(source, "shop:NoAccess", false, true )
		triggerClientEvent(source, "hideGeneralshopUI", source)
		return false
	end
	...
	if tonumber(price) < 0 then
		exports.global:sendLocalText(source, "* "..pedName.." nepiekrīt ar "..playerName.." par "..itemName.." preces cenu.", 255, 51, 102, 30, {}, true)
		storeKeeperSay(source, "Neviens nevar pārdot preces pa negatīvu cenu?", pedName)
		setElementData(source, "shop:NoAccess", false, true )
		triggerClientEvent(source, "hideGeneralshopUI", source)
		return false
	end
	...
	if addToShop then
		...
		if playerGender == 0 then
			storeKeeperSay(source, "Labi, sapratu ser.", pedName)
		else
			storeKeeperSay(source, "Labi, sapratu kundze.", pedName)
		end
		setElementData(source, "shop:NoAccess", false, true )
...
function editItemToShop(source, price, desc, proID, itemName, shopElement)
	...
	if checkingItem then
		...
		if addToShop then
			...
			storeKeeperSay(source, "Labi..labi..", pedName)
			setElementData(source, "shop:NoAccess", false, true )
			triggerClientEvent(source, "hideGeneralshopUI", source)
		end
	else
		outputChatBox(" "..itemName.." vairs neeksistē veikalā.", source, 255,0, 0)
		setElementData(source, "shop:NoAccess", false, true )
		triggerClientEvent(source, "hideGeneralshopUI", source)
	end
end
...
function takeOffProductFromShop(source, proID, itemI, itemV, itemName, shopElement, metadata)
	...
	if itemI == 115 or itemI == 116 then --weapons and ammo
		if getElementData( source, "license.gun" ) ~= 1 then
			storeKeeperSay(source, "Nē, piedod. Es nevaru tev iedod šo, kamēr neparādi tiesības..", pedName)
			setElementData(source, "shop:NoAccess", false, true )
			triggerClientEvent(source, "hideGeneralshopUI", source)
			return false
		end
	...
	if checkingItem then
		...
		if success then
			...
			local playerGender = getElementData(source,"gender")
			if playerGender == 0 then
				storeKeeperSay(source, "Ludzu, kungs.", pedName)
			else
				storeKeeperSay(source, "Ludzu, kundze.", pedName)
			end
		else
...
function customShopBuy(proID, itemI, itemV, itemPrice, itemName, payByBank, shopElement, metadata)
	...
	if itemI == 115 or itemI == 116 then --weapons and ammo
		if getElementData( client, "license.gun" ) ~= 1 then
			storeKeeperSay(client, "Nē, piedod. Es nevaru tev šo pārdot, kamēr neparādi man savas tiesības..", pedName)
	...
	if checkingItem and checkingPrice and (tonumber(checkingPrice) == tonumber(itemPrice) ) then
		...
		if success then
			...
			if not payByBank then
				...
				if playerGender == 0 then
					triggerEvent('sendAme', client, "izņem pāris banknotes no sava maka un pasniedz tās "..pedName)
				else
					triggerEvent('sendAme', client, "izņem pāris banknotes no sava maka un pasniedz tās "..pedName)
				end
				...
				if exports.global:takeMoney(client, itemPrice) then
					...
					storeKeeperSay(client, "Atvainojos, bet šeit nepietiek..", pedName)
					exports.global:takeItem( client, itemI, itemV )
					setElementData(client, "shop:NoAccess", false, true )
					triggerClientEvent(client, "hideGeneralshopUI", client)
					return false
				end
				ownerNoti = "Klients iegādājās "..itemName.." par $"..exports.global:formatMoney(itemPrice).."."
				logString = "- "..timeString.." : A customer bought a "..itemName.." for $"..exports.global:formatMoney(itemPrice)..".\n"
			else
				playBuySound(shopElement)
				if playerGender == 0 then
					triggerEvent('sendAme', client, "izņem kredītkarti no sava maka un pasniedz to "..pedName)
				else
					triggerEvent('sendAme', client, "izņem kredītkarti no sava maka un pasniedz to "..pedName)
				end
				...
				ownerNoti = getPlayerName(client):gsub("_", " ").."(Debit Karte) nopirka "..itemName.." pa $"..exports.global:formatMoney(itemPrice).."."
			...
			if payByBank then
				additionalText = " un atdod karti."
			end

			triggerEvent("sendAme", shopElement, "gave "..playerName.." a "..itemName..additionalText..".")

			storeKeeperSay(client, "Ludzu. un..", pedName)
			if playerGender == 0 then
				storeKeeperSay(client, "Paldies, kungs. Lai Jums izdevusies diena!", pedName)
			else
				storeKeeperSay(client, "Paldies, kundze. Lai Jums izdevusies diena!", pedName)
			end
		...
		outputChatBox(" "..itemName.." vairs neeksistē veikalā.", client, 255,0, 0)
		setElementData(client, "shop:NoAccess", false, true )
		triggerClientEvent(client, "hideGeneralshopUI", client)
	end
end
		...
		for _, owner in ipairs(possibleOwners) do
			...
			if isBizOwner then
				if exports.global:hasItem(owner, 2) then
					...
					exports.global:sendLocalText(owner, "*"..ownerName.." saņem īsziņu.", 255, 51, 102, 30, {}, true)
