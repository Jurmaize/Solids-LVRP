...
function giveProfit(interior, ped, player, item, price)
	if isProfitable(player, interior) then
		...
		if playerGender == 0 then
			triggerEvent('sendAme', player, "izņem pāris banknotes un padod tās "..pedName)
		else
			triggerEvent('sendAme', player, "izņem pāris banknotes un padod tās "..pedName)
		end
		local r = getRealTime()
		local timeString = ("%02d/%02d/%04d %02d:%02d"):format(r.monthday, r.month + 1, r.year+1900, r.hour, r.minute)
		local ownerNoti = "Kāds klients nopirka "..item.name.." pa $"..exports.global:formatMoney(price).."."
		...
		storeKeeperSay(player, "Ludzu. Un..", pedName)
		if playerGender == 0 then
			storeKeeperSay(player, "Paldies, lai Jums jauka diena!", pedName)
		else
			storeKeeperSay(player, "Paldies, lai Jums jauka diena!", pedName)
		end
...
function canPlayerCollectProfit(player, ped, int)
	...
	if not is then -- if not a business then noone can collect.
		return false, "Tu nevari saņemt peļņu no interjera, kas nav bizness."
	end
	...
	if faction_id > 0 then
		...
		if exports.factions:isPlayerInFaction(player, faction_id) then
			...
			return false, "Tu nēesi "..getTeamName(faction).." biedrs."
		end
	...
	-- exceptions
	return false, "Tev nav pieeju, lai veiktu šo darbību."
end
