...
function createlicenseBikeTestIntroWindow()
	...
	guiIntroWindowB = guiCreateWindow ( X , Y , Width , Height , "A Kategorijas Teorijas Eksāmens" , false )
	
	guiCreateStaticImage (0.35, 0.1, 0.3, 0.2, "banner.png", true, guiIntroWindowB)
	
	guiIntroLabel1B = guiCreateLabel(0, 0.3,1, 0.5, [[Jūs uzsāksiet A Kategorijas Teorijas Eksāmenu. Jums tiks uzdoti jautājumi no kuriem ir jaatbild vismaz 80% pareizi. Veiksmi!]], true, guiIntroWindowB)
	
	guiLabelSetHorizontalAlign ( guiIntroLabel1B, "center", true )
	guiSetFont ( guiIntroLabel1B,"default-bold-small")
	
	guiIntroProceedButtonB = guiCreateButton ( 0.4 , 0.75 , 0.2, 0.1 , "Sākt" , true ,guiIntroWindowB)
...
function createBikeLicenseQuestionWindow(number)
	...
	-- create the window
	guiQuestionWindowB = guiCreateWindow ( X , Y , Width , Height , "Jautājums Nr."..number.." no "..NoQuestionToAnswer , false )
	...
	if(number < NoQuestionToAnswer) then
		guiQuestionNextButtonB = guiCreateButton ( 0.4 , 0.75 , 0.2, 0.1 , "Nākošais" , true ,guiQuestionWindowB)
		...
		guiQuestionSumbitButtonB = guiCreateButton ( 0.4 , 0.75 , 0.3, 0.1 , "Iesniegt jautājumus" , true ,guiQuestionWindowB)
...
function createBikeTestFinishWindow()
	...
	-- create the window
	guiFinishWindowB = guiCreateWindow ( X , Y , Width , Height , "Eksāmena beigas.", false )
	
	if(score >= passPercent) then
	
		guiCreateStaticImage (0.35, 0.1, 0.3, 0.2, "pass.png", true, guiFinishWindowB)
	
		guiFinalPassLabelB = guiCreateLabel(0, 0.3, 1, 0.1, "Apsveicam! Jūs esat nokārtojis šo eksāmena daļu.", true, guiFinishWindowB)
		...
		guiFinalPassTextLabelB = guiCreateLabel(0, 0.4, 1, 0.4, "Jūs ieguvāt "..score.."%, nepieciešamais daudzums ir "..passPercent.."%. Labi padarīts!" ,true, guiFinishWindowB)
		guiLabelSetHorizontalAlign ( guiFinalPassTextLabelB, "center", true)
		
		guiFinalRegisterButtonB = guiCreateButton ( 0.35 , 0.8 , 0.3, 0.1 , "Turpināt" , true ,guiFinishWindowB)
		...
		guiFinalFailLabelB = guiCreateLabel(0, 0.3, 1, 0.1, "Diemžēl, jūs nenokārtojāt teorijas eksāmenu.", true, guiFinishWindowB)
		...
		guiFinalFailTextLabelB = guiCreateLabel(0, 0.4, 1, 0.4, "Jūs ieguvāt "..math.ceil(score).."%, bet nepieciešamais daudzums ir "..passPercent.."%." ,true, guiFinishWindowB)
		guiLabelSetHorizontalAlign ( guiFinalFailTextLabelB, "center", true)
		
		guiFinalCloseButtonB = guiCreateButton ( 0.2 , 0.8 , 0.25, 0.1 , "Aizvērt" , true ,guiFinishWindowB)
...
function initiateBikeTest()
	...
	outputChatBox("#FF9933Jūs tagad esat gatavs uzsākt braukšanas eksāmenu. Uzkāpiet uz CSDD eksāmena motociklu, lai uzsāktu braukšanas eksāmenu.", 255, 194, 14, true)
...
function startBikeTest(element)
	if element == getLocalPlayer() then
		...
		if not vehicle or not testBike[getElementModel(vehicle)] then
			outputChatBox("#FF9933Jums ir jāizmanto CSDD motocikls, braukšans eksāmenā!", 255, 0, 0, true ) -- Wrong  type.
		else
			destroyElement(blip)
			...
			outputChatBox("#FF9933Jums ir nepieciešams izbraukt maršrutu nesabojājot motociklu. Veiksmi un braucat droši.", 255, 194, 14, true)
		end
	end
end
...
function UpdateBikeCheckpoints(element)
	if (element == localPlayer) then
		local vehicle = getPedOccupiedVehicle(getLocalPlayer())
		if not vehicle or not testBike[getElementModel(vehicle)] then
			outputChatBox("Jums ir jāizmanto CSDD motocikls, braukšans eksāmenā!", 255, 0, 0) -- Wrong car type.
		elseif getElementData(vehicle, "dbid") ~= vehicleIdUsedToStartTest then
			outputChatBox("Jūs nelietojat to motociklu ar ko uzsākāt eksāmenu.", 255, 194, 14)
			outputChatBox("Jūs nenokārtojāt braukšans eksāmenu.", 255, 0, 0)
			...
			if (tonumber(max_number-1) == tonumber(m_number)) then -- if the next checkpoint is the final checkpoint.
				outputChatBox("#FF9933Noparkojiet motociklu #FF66CCCSDD stāvlaukumā, #FF9933lai pabeigtu testu.", 255, 194, 14, true)
...
function EndBikeTest(element)
	if (element == localPlayer) then
		local vehicle = getPedOccupiedVehicle(getLocalPlayer())
		if not vehicle or not testBike[getElementModel(vehicle)] then
			outputChatBox("Jums ir jāizmanto CSDD motocikls, braukšans eksāmenā!", 255, 0, 0)
		else
			...
			elseif (vehicleHealth >= 800) then
				...
				-- PASS --
				----------
				outputChatBox("Pēc motocikla pārskatīšanas, bojājumi netika manīti.", 255, 194, 14)
				...
				-- Fail --
				----------
				outputChatBox("Pēc motocikla pārskatīšanas, tika manīti vairāki bojājumi.", 255, 194, 14)
				outputChatBox("Jūs nenokārtojāt CSDD braukšanas eksāmenu.", 255, 0, 0)
			end
