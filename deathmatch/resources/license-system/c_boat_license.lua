...
function createlicenseBoatTestIntroWindow()
	...
	guiIntroWindowBo = guiCreateWindow ( X , Y , Width , Height , "Kuģu teorijas eksāmens" , false )
	
	guiCreateStaticImage (0.35, 0.1, 0.3, 0.2, "banner.png", true, guiIntroWindowBo)
	
	guiIntroLabel1Bo = guiCreateLabel(0, 0.3,1, 0.5, [[Jūs tagad uzsāksiet Kuģu teorijas eksāmenu. Veiksmi!]], true, guiIntroWindowBo)
	
	guiLabelSetHorizontalAlign ( guiIntroLabel1Bo, "center", true )
	guiSetFont ( guiIntroLabel1Bo,"default-bold-small")
	
	guiIntroProceedButtonBo = guiCreateButton ( 0.4 , 0.75 , 0.2, 0.1 , "Uzsākt eksāmenu" , true ,guiIntroWindowBo)
...
function createBoatLicenseQuestionWindow(number)
	...
	-- create the window
	guiQuestionWindowBo = guiCreateWindow ( X , Y , Width , Height , "Jautājums Nr."..number.." no "..NoQuestionToAnswer , false )
	...
	if(number < NoQuestionToAnswer) then
		guiQuestionNextButtonBo = guiCreateButton ( 0.4 , 0.75 , 0.2, 0.1 , "Nākošais jautājums" , true ,guiQuestionWindowBo)
		...
		guiQuestionSumbitButtonBo = guiCreateButton ( 0.4 , 0.75 , 0.3, 0.1 , "Iesniegt atbildes" , true ,guiQuestionWindowBo)
...
function createBoatTestFinishWindow()
	...
	-- create the window
	guiFinishWindowBo = guiCreateWindow ( X , Y , Width , Height , "Eksāmena beigas.", false )
	
	if(score >= passPercent) then
	
		guiCreateStaticImage (0.35, 0.1, 0.3, 0.2, "pass.png", true, guiFinishWindowBo)
	
		guiFinalPassLabelBo = guiCreateLabel(0, 0.3, 1, 0.1, "Apsveicam! Jūs esat veiksmīgi nokārtojis kuģošanas teorijas eksāmenu.", true, guiFinishWindowBo)
		...
		guiFinalPassTextLabelBo = guiCreateLabel(0, 0.4, 1, 0.4, "Jūs ieguvāt "..score.."%, no nepieciešamajiem "..passPercent.."%. Labi padarīts!" ,true, guiFinishWindowBo)
		guiLabelSetHorizontalAlign ( guiFinalPassTextLabelBo, "center", true)
		
		guiFinalRegisterButtonBo = guiCreateButton ( 0.35 , 0.8 , 0.3, 0.1 , "Turpināt" , true ,guiFinishWindowBo)
		...
		guiFinalFailLabelBo = guiCreateLabel(0, 0.3, 1, 0.1, "Diemžel, Jūs nenokārtojāt kuģošanas eksāmenu", true, guiFinishWindowBo)
		...
		guiFinalFailTextLabelBo = guiCreateLabel(0, 0.4, 1, 0.4, "Jūs ieguvāt "..math.ceil(score).."%, bet nepieciešams bija "..passPercent.."%." ,true, guiFinishWindowBo)
		guiLabelSetHorizontalAlign ( guiFinalFailTextLabelBo, "center", true)
		
		guiFinalCloseButtonBo = guiCreateButton ( 0.2 , 0.8 , 0.25, 0.1 , "Aizvērt" , true ,guiFinishWindowBo)
