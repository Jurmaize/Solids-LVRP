...
function applyPhone(string, popOutOnPhoneCall, itemName)
	...
	if string == "phone_in" then
		...
		triggerEvent('sendAme', source, "izvelk ārā " .. (getElementData(source, "gender") == 0 and "" or "") .. " " .. itemName .. ".")
	...
	elseif string == "phone_out" then
		...
		if getElementData(source, "cellphoneGUIStateSynced") then
			if not popOutOnPhoneCall then
				triggerEvent('sendAme', source, "noliek "..(getElementData(source, "gender") == 0 and "" or "").." " .. itemName .. ".")
			end
			if getElementData(source, "phone_anim") ~= "0" then
				setPedAnimation(source, "ped", string, 1, false)
			end
...
function callSomeone(thePlayer, commandName, phoneNumber, withNumber)
		...
		if isElementWithinColShape( thePlayer, v ) then
			for kx, vx in pairs( getElementsByType( "player" ) ) do
				if getElementData( vx, "call.col" ) == v then
					outputChatBox( "Kāds jau lieto šo telefonu.", thePlayer, 255, 0, 0 )
					return
				end
			end
			publicphone = v
	...
	if not outboundPhoneNumber or outboundPhoneNumber == -1 then
		outputChatBox("Tici vai nē, ir grūti rakstīt uz telefona, kurš tev nav.", thePlayer, 255, 0, 0)
		return
	end

	if not (phoneNumber) then
		outputChatBox("Nospied 'i' un uzspied uz telefona, kuru vēlies lietot.", thePlayer)
		return
	...
	if not tonumber(phoneNumber) then
		...
		if result then
			...
			outputChatBox("Nevarēja atrast nummuru, vai nummuru, kas saistīts ar šo kontaktu.", thePlayer, 255, 0, 0)
			return
		end
	end

	if not tonumber(phoneNumber) then
		outputChatBox("Nederīgs telefona nummurs.", thePlayer)
		return
	end
	...
	if callerphoneIsTurnedOn == 0 then
		outputChatBox("Tavs telefons ir izslēgts.", thePlayer, 255, 0, 0)
	else
		local calling = getElementData(thePlayer, "calling")

		if (calling) then -- Using phone already
			outputChatBox("Tu jau lieto telefonu.", thePlayer, 255, 0, 0)
		elseif getElementData(thePlayer, "injuriedanimation") then
			outputChatBox("Tu nevari lietot telefonu, kamēr esi bezsamaņā.", thePlayer, 255, 0, 0)
		else
			-- /me it
			if publicphone then
				triggerEvent('sendAme', thePlayer, "izstiep roku pēc publiskā telefona.")
			end
			...
			-- Otherwise find a fool to answer it
				...
				if bankMoney >= 1 then
					if not exports.donators:hasPlayerPerk(thePlayer, 6) and not exports.anticheat:changeProtectedElementDataEx(thePlayer, "bankmoney", tonumber(bankMoney) - 1, false) then
						outputChatBox("Tev nepietiek naudas zvanam.", thePlayer, 255, 0, 0)
						return
					end
				else
					outputChatBox("Tu nevari atļauties zvanu.", thePlayer, 255, 0, 0)
					return
				end

				-- Yes, Is the target phone online or found at all?
				if not found then
					outputChatBox("Nav atbildes...", thePlayer, 255, 194, 14)
					return
				end
					...
					if not foundInGame then
						...
						if t_ringtone > 1 and tone_volume > 0 then
							...
							if t_ringtone > 2 then
								triggerEvent('sendAme', targetPlayer, " telefons sāk zvanīt.")
							end
						end
	...
	if (logged==1) then
		if (exports.global:hasItem(thePlayer, 7)) then
			if not (partialNick) then
				...
				triggerEvent('sendAme', thePlayer, "ieskatās telefona grāmatā.")
				...
				if (mysql:num_rows(result)>10) then
					outputChatBox("Pārāk daudz rezultātu.", thePlayer, 255, 194, 14)
				elseif (mysql:num_rows(result)>0) then
					...
					outputChatBox("Tu nevienu neatrodi ar to vārdu.", thePlayer, 255, 194, 14)
				end
				mysql:free_result(result)
			end
		else
			outputChatBox("Tici vai nē, bet ir grūti lietot telefona grāmatu, kuras tev nav.", thePlayer, 255, 0, 0)
		end
	end
end
	...
	if logged == 1 then
		...
		elseif tonumber(phoneNumber) < 10 then
			...
			if not (exports.global:hasItem(thePlayer, 2, tonumber(phoneNumber))) then
				outputChatBox("Tev nepieder šis telefona nummurs", thePlayer, 255, 0, 0)
				return
			end
		end
		...
		if getElementData( thePlayer, "calling" ) then
			outputChatBox("Tu lieto savu telefonu!", thePlayer, 255, 0, 0)
		else
			if calledphoneIsTurnedOn == 0 then
				outputChatBox("Tu ieslēdzi savu telefonu ar nummuru '"..tostring(phoneNumber).."'.", thePlayer, 0, 255, 0)
			else
				outputChatBox("Tu izslēdzi savu telefonu ar nummuru '"..tostring(phoneNumber).."' off.", thePlayer, 255, 0, 0)
			end
...
function setPhoneBook(thePlayer, commandName, phoneNumber, ...)
	...
	if logged == 1 then
		...
		if name then
			...
			outputChatBox("Tu ievieto savu vārdu telefona grāmatā '" .. name .. "'.", thePlayer, 0, 255, 0)
		else
			success = mysql:query_free( "UPDATE `phones` SET `phonebook`=NULL WHERE `phonenumber`='".. mysql:escape_string(tostring(phoneNumber)) .."'")
			outputChatBox("Tu izņēmi savu vārdu no telefona grāmatas.", thePlayer, 0, 255, 0)
		end
	end
end
