local easyHotlines = {
	[911] = {
		name = "Ārkārtas tālrunis",
		order = 5,
		factions = { 1, 2, 164 },
		require_radio = true,
		operator = "911 Operators",
		dialogue = {
			{
				q = "911 ārkārtas tālrunis, kā palīdzība Jums ir nepieciešama? ((Visas, LSPD, LSGD))",
				as = "service",
				check = function(service)
					-- return which factions should receive the call
					local found, factions = checkService(service)
					return factions, found, "Atvainojos, nesaprotu ko Jūs teicāt."
				end
			},
			{ q = "Kāds ir Jūsu vārds?", as = "name" },
			{ q = "Kāds ir zvana iemesls?", as = "emergency" },
			done = "Paldies par zvanu, mēs esam paziņojuši atbildīgajiem departamentiem."
		},
			...
			if query then
				...
				log911("[911 Call] Persona: " .. getPlayerName(caller.element) .. " || Situācija: " .. callstate.emergency .. ", beidzu.")
				players:send({
					"[RADIO] Šis ir dispičers, mums ir ārkārtas paziņojums no " .. callstate.name .. " (#" .. caller.phone .. "), beidzu.",
					"[RADIO] Situācija: '" .. callstate.emergency .. "', beidzu.",
					streetlocation and ("[RADIO] Lokācija: '" .. streetlocation .. " iekš " .. zonelocation .. "', beidzu.") or ("[RADIO] Lokācija: '" .. zonelocation .. "', beidzu.")
				}, 0, 183, 239)
				players:beep()
			else
				caller:respond("Bija problēma apstrādājot Jūsu pieprasījumu, meiģinat vēlreiz vēlāk.")
			end
		end
	},
	--[[[311] = {
	...
	[511] = {
		name = "Los Santos Valdība",
		order = 30,
		factions = { 3 },
		require_radio = true,
		operator = "Valdības darbinieks",
		dialogue = {
			{ q = "Los Santos Valdība. Kā varam palīdzēt?", as = "reason" },
			done = "Paldies par zvanu."
		},
		done = function(caller, callstate, players)
			players:send({
				"[RADIO] Šis ir Valdības dispičers, mums ir ziņa no #" .. caller.phone .. ".",
				"[RADIO] Iemesls: '" .. callstate.reason .. "', beidzu."
			}, 245, 40, 135)
		end
	},
	--[[[711] = {
	...
	[9021] = {
		name = "Los Santos Auto Kofnsicētuve",
		order = 50,
		factions = { 4 },
		require_radio = true,
		operator = "Operator",
		dialogue = {
			{ q = "Jūs esat sazvanījis Los Santos Auto Konfiscētuvi. Lūdzu pasakiet savu vārdu.", as = "name" },
			{ q = "Vai Jūs lūdzu varētu paskaidrot situāciju?", as = "reason" },
			done = "Paldies par zvanu, mēs esam paziņojuši informāciju darbiniekiem.",
		},
		done = function(caller, callstate, players)
			...
			players:send({
				"[RADIO] Šis ir dispičeris, mums ir ziņa no " .. callstate.name .. " (#" .. caller.phone .. "), caur #9021 nummuru, beidzu.",
				"[RADIO] Situācija: '" .. callstate.reason .. "', beidzu.",
				streetlocation and ("[RADIO] Lokācija: '" .. streetlocation .. " iekš " .. zonelocation .. "', beidzu.") or ("[RADIO] Lokācija: '" .. zonelocation .. "', beidzu.")
			}, 0, 183, 239)
			players:beep()
		end,
		no_players = "Atvainojos, bet nav neviena, kas spētu palīdzēt. Zvaniet atpakaļ vēlāk."
	},
	[8294] = {
		name = "SIA Dzeltenie taksīši.",
		order = 40,
		operator = "Taksīšu operators",
		job = { id = 2, vehicle_models = { 438, 420 } },
		dialogue = {
			{ q = "Dzeltenie taksīšu dispičere šeit, no kurienes Jums nepieciešams aizvest?", as = "location" },
			done = "Labi tad, aizsūtīsim taksi tūliņ."
		},
		done = function(caller, callstate, players)
			players:send("[RADIO] Taxi Operator Saka: Taksīši, mums ir pieprasījums no #" .. caller.phone .. ". Personai vajag aizvest no " .. callstate.location .. ".", 0, 183, 239)
			players:beep()
		end,
		no_players = "Emm', mums nav neviens taksīts pieejams. Lūdzu zvanat vēlāk."
	},
	--[[[2552] = {
		name = "RS Haul",
	...
	[2600] = {
		name = "Los Santos Glābšanas Dienests",
		operator = "Reģistratūra",
		factions = { 164 },
		require_radio = true,
		dialogue = {
			{ q = "Jūs esat sasniedzis Los Santos Glābšanas Dienests. Vai es varētu lūdzu saņemt Jūsu vārdu?", as = "name" },
			{ q = "Kā es varētu palīdzēt?", as = "reason" },
			done = "Paldies par sazināšanos, padošu ziņu darbiniekiem."
		},
		done = function(caller, callstate, players)
			players:send({
				"[RADIO] Šis ir dispičers, mums ir zvans no #" .. caller.phone .. " caur #2600 nummuru, beidzu.",
				"[RADIO] Vārds: '" .. callstate.name .. "', beidzu.",
				"[RADIO] Informācija: '" .. callstate.reason .. "', beidzu."
			}, 245, 40, 135)
			players:beep()
		end
	},
...
function handleEasyHotlines(caller, callingPhoneNumber, startingCall, message)
	...
	caller = setmetatable(caller, {
		__index = {
			...
			respond = function(t, message)
				outputChatBox(hotline.operator .. " [Telefons]: " .. message, t.element, 200, 255, 200)
			end
		}
	})
	...
	if not startingCall then
		...
		if dialogue.check then
			...
			if not okay then
				caller:respond(err or "Diemžēl, tas nav iespējams.")
