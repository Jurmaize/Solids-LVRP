...
function startDialing(to, from)
	...
	elseif not exports.global:hasItem(source, 2, tonumber(from)) then
		outputDebugString("[Telefons] "..getPlayerName(source).." sāka zvanīt no "..from.." uz "..to.." bez telefona.")
		outputChatBox("Kļūdas kods x0EDSCVF23. Lūdzu reportot kļūdu iekš https://discord.gg/S5PDPdDeac", source, 255,0,0)
	...
	if powerOn == 0 then
		outputChatBox("Tavs telefons ir izslēgts.", source, 255,0,0)
		triggerClientEvent(source, "phone:slidePhoneOut", source)
		return false
	end

	if not canPlayerCall(source) then
		outputChatBox("Tu nevari šobrīd lietot telefonu.", source, 255,0,0)
		triggerClientEvent(source, "phone:slidePhoneOut", source)
		return false
	end
	...
	elseif t_powerOn ~= 1 then --turned off
		...
		if not foundInGame then
			...
			if t_ringtone > 1 and tone_volume > 0 then
				...
				if isSecret == 1 then
					displayedNumber = "Privāts nummurs"
				else
				...
				if t_ringtone > 2 then
					triggerEvent('sendAme', targetPlayer, " mobilais sāk zvanīt.")
				end
			end
...
function makeCall(thePlayer, commandName, phoneNumber)
	if not (phoneNumber) then
		...
		if not canPlayerCall(thePlayer) then
			outputChatBox("Tu šobrīd nevari veikt zvanu.", thePlayer, 255,0,0)
			return false
		end
		...
		if itemValue then
			...
			outputChatBox("Tev nav mobilais telefons.", thePlayer, 255, 0, 0)
		end
	end
end
...
function cancelPhoneCall(reason)
	...
	outputDebugString("[Telefons] "..getPlayerName(source).." triggered cancelPhoneCall / "..(reason and reason or "").." / "..phonestate)
	...
	if caller2col then
		outputChatBox("Persona nometa zvanu.", caller2, 155, 155, 255)
...
function acceptPhoneCall()
	if not canPlayerAnswerCall(source) then
		outputChatBox("Tu šobrīd nevari atbildēt uz zvanu.", source, 255,0,0)
		triggerClientEvent("stopRinging", source)
		return false
	end
	local phonestate = getElementData(source, "phonestate") or 0
	if phonestate ~= 3 then
		outputChatBox("Tavs mobilais nezvana.", source, 255,0,0)
		triggerClientEvent("stopRinging", source)
		return false
	end
	...
	if not getElementData(caller, "call.col") then
		...
		outputChatBox("Persona pacēla tālruni.", caller, 155, 155, 255)
	end
	...
	if cost > 0 then
		if exports.bank:takeBankMoney(source, cost) then
			...
			if exports.global:giveMoney(foundFaction, cost) then
				return exports.bank:addBankTransactionLog(getElementData(source, "dbid"), -20, cost, 2, "Telefona zvana izmaksas", "Zvans no #"..fromNumber..", ilgums: "..duration, nil, nil)
			end
		end
	end
	return false
		...
		if (exports.global:hasItem(thePlayer, 2)) or getElementData(thePlayer, "call.col") then -- 2 = Cell phone item
			...
			if phonestate == 4 or phonestate == 5 then
				...
				if (not loudspeaker) then
					triggerEvent('sendAme', thePlayer, "ieslēdz telefona skaļruni.")
					outputChatBox("Tu ieslēdz savu telefonu uz skaļruni.", thePlayer)
					exports.anticheat:changeProtectedElementDataEx(thePlayer, "call.loudspeaker", true, false)
				else
					triggerEvent('sendAme', thePlayer, "izslēdz telefona skaļruni.")
					outputChatBox("Tu izslēdz savu telefona skaļruni.", thePlayer)
...
function talkPhone(thePlayer, commandName, ...)
	...
	if not exports.global:hasItem(thePlayer, 2) and not getElementData(thePlayer, "call.col") then
		outputChatBox("Tici vai nē, bet ir grūti lietot telefonu, kurš tev nav!", thePlayer, 255, 0, 0)
		return
	end
	...
	if getElementData(thePlayer, "injuriedanimation")  then
		outputChatBox("Tu nevari izmantot telefonu, kamēr esi parubijies!", thePlayer, 255, 0, 0)
		return
	end
		
	local phoneState = getElementData(thePlayer, "phonestate")
	if not (phoneState == 4 or phoneState == 5) then
		outputChatBox("Tu neesi zvanā!", thePlayer, 255, 0, 0)
		return
	end
	...
	if (callprogress) then
		...
		if commandName == "plow" then
			...
			if not theVehicle or not exports.vehicle:isVehicleWindowUp(theVehicle) then
				exports.global:sendLocalText(thePlayer, "[" .. languagename .. "] " .. username .. " pačukst [Telefons]: " .. message, nil, nil, nil, 3, {[thePlayer] = true})
			end	
			outputChatBox("[" .. languagename .. "] " .. username .. " pačukst [Telefons]: " .. message, thePlayer, 200, 255, 200)
			triggerEvent("sendAme", thePlayer, "iečukst telefonā.")
		else
			if not theVehicle or not exports.vehicle:isVehicleWindowUp(theVehicle) then
				exports.global:sendLocalText(thePlayer, username .. " [Telefons]: " .. message, nil, nil, nil, 10, {[thePlayer] = true})
			end	
			outputChatBox("[" .. languagename .. "] " .. username .. " [Telefons]: " ..message, thePlayer, 200, 255, 200)
		end
	...
	if commandName == "plow" then
		outputChatBox("[" .. languagename .. "] " .. username .. " pačukst [Telefons]: " .. translatedMessage, target)
		outputChatBox("[" .. languagename .. "] " .. username .. " pačukst [Telefons]: " ..message, thePlayer)
		if not theVehicle or not exports.vehicle:isVehicleWindowUp(theVehicle) then
			sendLocalText(thePlayer, "[" .. languagename .. "] " .. username .. " pačukst [Telefons]: " .. translatedMessage, nil, nil, nil, 3, {[thePlayer] = true})
		end	
		triggerEvent("sendAme", thePlayer, "iečukst telefonā.")
	else
		outputChatBox("[" .. languagename .. "] " .. username .. " [Telefons]: " .. translatedMessage, target, 200, 255, 200)
		outputChatBox("[" .. languagename .. "] " .. username .. " [Telefons]: " ..message, thePlayer, 200, 255, 200)
		if not theVehicle or not exports.vehicle:isVehicleWindowUp(theVehicle) then
			sendLocalText(thePlayer, "[" .. languagename .. "] " .. username .. " [Telefons]: " .. translatedMessage, nil, nil, nil, 10, {[thePlayer] = true})
		end	
	end
					...
					if (exports.vehicle:isVehicleWindowUp(pveh)) then
						...
						for i = 0, getVehicleMaxPassengers(pveh) do
							...
							if (lp) and (lp~=source) then
								outputChatBox(" [" .. languagename .. "] " .. username .. " ((Iekš auto)) saka: " .. translatedMessage, lp)
						...
						outputChatBox("[" .. languagename .. "] " .. username .. " telefonā caur skaļruni: " .. translatedMessage, nearbyPlayer)
		...
		for i, timer in pairs(dialingTimers[phone]) do
			if isTimer(timer) then
				if killTimer(timer) then
					timer = nil
					outputDebugString("[Telefons] killDialingTimers killed.")
				end
			end
		end
	else
...
function outputChange(dataName,oldValue)
	if getElementType(source) == "player" and dataName == "phonestate"  then -- check if the element is a player
		...
		outputDebugString("[Telefons] "..getPlayerName(source).."'s "..tostring(dataName).."' has changed from '"..tostring(oldValue).."' to '"..tostring(newValue).."'") -- output the change for the affected player
	end
end
