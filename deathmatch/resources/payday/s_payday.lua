	...
	for k,v in pairs(taxVehicles) do
		if k<0 and v>0 then -- Must be a faction
			...
			if team then
				...
				exports.bank:addBankTransactionLog(k, -3, v, 11, "Auto nodoklis valdībai" )
			end
		end
	end
	...
	for k,v in pairs(taxHouses) do
		if k<0 and v>0 then -- Must be a faction
			...
			if team then
				...
				exports.global:takeMoney(team, v, true)
				exports.bank:addBankTransactionLog(k, -3, v, 11, "Interjera nodoklis valdībai")
			end
		end
	end
	...
	for k,v in pairs(rentalHouses) do
		if k<0 and v>0 then -- Must be a faction
			...
			if team then
				...
				totalTaxPaid[-k] = totalTaxPaid[-k] + v
				exports.global:takeMoney(team, v, true)
				exports.bank:addBankTransactionLog(k, -3, v, 11, "Interjera īres nodoklis valdībai")
			end
		end
	end
end
...
function payAllWages(isForcePayday)
	...
	-- Get some data
	govAmount = 1000000 --exports.global:getMoney(getTeamFromName("Los Santos Valdība"))
...
function doPayDayPlayer(value, isForcePayday)
	...
	if ((logged==1) and (timeinserver>=58) and (getPlayerIdleTime(value) < 600000)) or isForcePayday then
        ...
        if duty > 0 then
            ...
            if not foundPackage then
                ...
                outputChatBox("Tev vairs nav pieejas pie tā Duty, ko lieto - tāpēc, tas tiek noņemts.", value, 255, 0, 0)
            end
	...
	elseif (logged==1) and (timeinserver) and (timeinserver<60) then
		outputChatBox("Tu nēesi nospēlējis pietiekami ilgi serverī, lai saņemtu algu (Tev vajag vēl " .. 60-timeinserver .. " minūtes spēlēt.)", value, 255, 0, 0)
	end
end
...
function timeSaved(thePlayer)
	...
	if (logged==1) then
		...
		outputChatBox("Tev šobrīd ir " .. timeinserver .. " minūtes nospēlētas.", thePlayer, 255, 195, 14)
		outputChatBox("Tev vajag vēl " .. 60-timeinserver .. " minūtes nospēlēt, lai saņemtu algu.", thePlayer, 255, 195, 14)
	end
end
