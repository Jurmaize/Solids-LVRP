		...
		if not id or not (tonumber(id)) then
			...
			local qWeimy = mysql:query("SELECT `faction_id` FROM `characters` WHERE `id` ='"..issuerCharID.."'")
				...
				elseif issuerFaction == 1 then
					issuerFaction = "Los Santos Policijas Departaments"
				else
...
function issueTicket(thePlayer, theCommand, theVehicle, amount, ...)
	...
	if (logged==1) then
		...
		if (getTeamName(team) == "Los Sa326 Enterprises(getTeamName(team) == "Los Santos Policijas Departaments") or (getTeamName(team) == "San Andreas State Police") then
...
function showAllTickets(thePlayer, theCommand)
	...
	if (logged==1) then
		local team = getPlayerTeam(thePlayer)
		if (getTeamName(team) == "326 Enterprises") or (getTeamName(team) == "Los Santos Policijas Departaments") or (getTeamName(team) == "San Andreas State Police") then
