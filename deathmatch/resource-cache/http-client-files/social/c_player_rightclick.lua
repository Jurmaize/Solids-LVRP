...
function showPlayerMenu(targetPlayer, friend)
	...
	if getElementData(localPlayer, "cards:host") then
		local bAddToGame = exports['rightclick']:addRow("Uzaicināt uz kāršu spēli")
	...
	if (blindfold) and (blindfold == 1) then
		bBlindfold = exports['rightclick']:addRow("Noņemt acu apsēju")
		addEventHandler("onClientGUIClick", bBlindfold, cremoveBlindfold, false)
	else
		bBlindfold = exports['rightclick']:addRow("Uzlikt acu apsēju")
	...
	if exports.global:hasItem(getLocalPlayer(), 70) and getElementData(player, "injuriedanimation") then
		bStabilize = exports['rightclick']:addRow("Stabilizēt")
	...
	if (button=="left") then
		if (exports.global:hasItem(getLocalPlayer(), 66)) then -- Has blindfold?
			...
			if (blindfolded==1) then
				outputChatBox("Šī persona jau neko neredz.", 255, 0, 0)
				hidePlayerMenu()
			elseif (restrained==0) then
				outputChatBox("Šim spēlētājam ir jābūt sasietam/saslēgtam, lai uzliktu acu apsēju.", 255, 0, 0)
				hidePlayerMenu()
			else
			...
			outputChatBox("Jums nav acu apsējs.", 255, 0, 0)
		end
	end
end
...
function cremoveBlindfold(button, state, x, y)
	if (button=="left") then
		...
		if (blindfolded==1) then
			...
			outputChatBox("Šim spēlētājam nav uzlikts acu apsējs.", 255, 0, 0)
			hidePlayerMenu()
		end
	end
end
	...
	if (button=="left") then
		if (exports.global:hasItem(getLocalPlayer(), 45) or exports.global:hasItem(getLocalPlayer(), 46)) then
			...
			if (restrained==1) then
				outputChatBox("Šī persona ir sasieta/saslēgta rokudzelžos.", 255, 0, 0)
				hidePlayerMenu()
			else
				local restrainedObj
			...
			outputChatBox("Tev nav nekas ar ko sasiet/saslēgt personu.", 255, 0, 0)
			hidePlayerMenu()
		end
	end
end
...
function cunrestrainPlayer(button, state, x, y)
	if (button=="left") then
		...
		if (restrained==0) then
			outputChatBox("Šī persona nav sasieta/saslēgta.", 255, 0, 0)
			hidePlayerMenu()
		else
			...
			if (exports.global:hasItem(getLocalPlayer(), 47, dbid)) or (restrainedObj==46) then -- has the keys, or its a rope
				...
				outputChatBox("Jums nav atslēgas rokudzelžiem.", 255, 0, 0)
			end
		end
	end
end
...
function cfriskPlayer(button, state, x, y)
	if (button=="left") then
		...
		if restrained ~= 1 and not injured then
			outputChatBox("Šī persona nav sasieta/saslēgta vai ievainota.", 255, 0, 0)
			hidePlayerMenu()
