...
function startTalkToPed ()
	...
	if (isPedInVehicle(thePlayer)) then
		...
		if (exports.vehicle:isVehicleWindowUp(theVehicle)) then
			outputChatBox("Tev vajadzētu attaisīt logu pirms tu runā ar kādu, kas ir ārpusē.", thePlayer, 255,0,0)
			return
		end
	end
	...
	processMessage(thePed, "Sveiki! Vēlaties tikt garām? Tas būs $6.")
	setConvoState(thePlayer, 1)
	local responseArray = { "Jā, lūdzu.", "Nē, paldies." }
	...
	if not tollElementsLocked[tollKey] or exports.global:hasItem(thePlayer, 64) or exports.global:hasItem(thePlayer, 65) or exports.global:hasItem(thePlayer, 112) then
		if payByBank then
			...
			if money >= 0 then
				...
				outputChatBox("[Toll Pass] Tavai bankai atņēma $5.", thePlayer, 0, 255, 0)
			else
				return "Jums nepietiek naudas bankā."
			end
		else
			if not exports.global:takeMoney(thePlayer, 6) then
				return "Piedod, bet es nevaru Jūs laist caur, ja Jums nepietiek naudas."
			end
		end

		triggerGate(thePed)
		exports.global:giveMoney( getTeamFromName("Los Santos Valdība"), 3 )
		exports.global:giveMoney( getTeamFromName("Los Santos Policijas Departaments"), 2 )
		return "Paldies."
	else
		return "Atvainojamies, bet mums ir pavēle nevienu nelaist cauri."
	end
end
...
function talkToPed(answer, answerStr)
	...
	if (convState == 1) then --  "Hey, want to pass? That'll be six dollar please."
		...
		if (languageSkill < 60) or (currLang ~= 1) then
			processMessage(thePed, "Uh, piedod. Es runāju tikai angliski.")
		...
		elseif (answer == 2) then -- "No thanks."
			processMessage(thePed, "Okay, labi...")
			setConvoState(thePlayer, 0)
		end
	end
end
...
function testToggle(thePlayer, commandName, gateID, silent)
	...
	if not gateID or not tonumber(gateID) then
		...
		for tollID, tollName in ipairs(tollElementsName) do
			...
			outputChatBox(" "..tostring(tollID).. " - "..tollName .." - Štats: "..state, thePlayer)
		end
		return
	end
	...
	if not tollElementsName[gateID] then
		outputChatBox("Neeksistē.", thePlayer)
	end

	tollElementsLocked[gateID] = not tollElementsLocked[gateID]
	if tollElementsLocked[gateID] then
		local first = true
		exports['chat-system']:departmentradio(thePlayer, "d", "Lūdzu aizslēdziet "..tollElementsName[gateID].." pārbrauktuvi, sapratāt?, Beidzu.")
		for _, thePed in ipairs(getElementsByType('ped')) do
			local tollKey = getElementData(thePed, "toll:key")
			if tollKey and tollKey == gateID then
				if first then
					exports['chat-system']:departmentradio(thePed, "d", "10-4, Slēdzam ciet "..tollElementsName[gateID]..", beidzu.")
					first = false
				end
				processRadio(thePed, "Dispičers teiktu "..tollElementsName[gateID]..", nevienu nelaist cauri, beidzu!", thePed)
			end
		end
	else
		local first = true
		exports['chat-system']:departmentradio(thePlayer, "d", "Lūdzu atvērt "..tollElementsName[gateID].." pārbrauktuvi, sapratāt?, beidzu.")
		for _, thePed in ipairs(getElementsByType('ped')) do
			local tollKey = getElementData(thePed, "toll:key")
			if tollKey and tollKey == gateID then
				if first then
					exports['chat-system']:departmentradio(thePed, "d", "10-4, Veram vaļā "..tollElementsName[gateID]..", beidzu.")
					first = false
				end
				processRadio(thePed, "Dispičers pārbrauktuvēm "..tollElementsName[gateID]..", taisat vēlreiz vaļā, beidzu!", thePed)
			end
		end
	end
end
...
function tollCommand(thePlayer)
	...
	if duty > 0 and getElementDimension( thePlayer ) == 0 then
		...
		for key, value in ipairs( tollElements ) do
			...
			if name then
				outputChatBox( "Atver " .. name .. " pābrauktuvi.", thePlayer, 0, 255, 0 )
				for k, v in ipairs(value) do
					if v.ped then
		...
		if not any then
			outputChatBox("Tu nēesi tuvu nevienai pārbrauktuvei.", thePlayer, 255, 0, 0)
		end
	end
end
