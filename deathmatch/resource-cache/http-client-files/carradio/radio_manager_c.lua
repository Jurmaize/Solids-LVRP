...
function openRadioManager(defaultStations1, donorStations1)
	if true then--canAccessManager() then
		...
		if gui.main and isElement(gui.main) then
			guiSetText(gui.main, "Radio Staciju Pārskats")
			...
			gui.main = guiCreateWindow(x,y,w,h,"Radio Staciju Pārskats | Lādējas..",false)
			guiWindowSetSizable(gui.main, false)

			gui.tabpanel = guiCreateTabPanel(0.0122,0.0401,0.9757,0.87,true,gui.main)
			gui.defaultStations = guiCreateTab("Servera stacijas",gui.tabpanel)
			gui.donorStations = guiCreateTab("Ziedotāju stacijas",gui.tabpanel)
			...
			col.name = guiGridListAddColumn(grid.defaultStations,"Stacijas nos",0.18)
			col.ip = guiGridListAddColumn(grid.defaultStations,"Stacijas IP",0.3)
			col.status = guiGridListAddColumn(grid.defaultStations,"Statuss",0.1)
			col.owner = guiGridListAddColumn(grid.defaultStations,"Stacijas Īpašnieks",0.1)
			col.expireDate = guiGridListAddColumn(grid.defaultStations,"Derīguma termiņš",0.16)
			col.order = guiGridListAddColumn(grid.defaultStations,"Vieta",0.08)
			...
			col2.name = guiGridListAddColumn(grid.donorStations,"Stacijas nos",0.18)
			col2.ip = guiGridListAddColumn(grid.donorStations,"Stacijas IP",0.3)
			col2.status = guiGridListAddColumn(grid.donorStations,"Statuss",0.1)
			col2.owner = guiGridListAddColumn(grid.donorStations,"Stacijas Īpašnieks",0.1)
			col2.expireDate = guiGridListAddColumn(grid.donorStations,"Derīguma termiņš",0.16)
			col2.order = guiGridListAddColumn(grid.donorStations,"Vieta",0.08)

			gui.addNew = guiCreateButton(0.0135,0.9135,0.32476,0.0675,"Izveidot jaunu staciju",true,gui.main)
			...
			gui.buyNew = guiCreateButton(0.0135,0.9135,0.32476,0.0675,"Iegādāties jaunu staciju ("..perk[2].." GC)",true,gui.main)
			...
			gui.renew = guiCreateButton(0.0135,0.9135,0.32476,0.0675,"Atjaunot staciju",true,gui.main)
			...
			gui.refresh = guiCreateButton(0.0135+0.32476,0.9135,0.32476,0.0675,"Izsūtīt staciju visiem spēlētājiem",true,gui.main)
			...
			gui.bClose = guiCreateButton(0.0135+0.32476*2,0.9135,0.32476,0.0675,"Aizvērt",true,gui.main)
...
function updateDefaultStations()
	...
	for i = 1, maxRow do
		...
		guiGridListSetItemText(grid.defaultStations, row, col.status, (defaultStations[i]["enabled"] == "1" and "Aktivizēts" or "Deaktivizēts"), false, false)
		guiGridListSetItemText(grid.defaultStations, row, col.owner, accountNameBuilder(defaultStations[i]["owner"]) or "Neviens" , false, false)
		guiGridListSetItemText(grid.defaultStations, row, col.expireDate, defaultStations[i]["expire_date"] or "Nekad" , false, false)
...
function updateDonorStations()
	...
	for i = 1, maxRow do
		...
		guiGridListSetItemText(grid.donorStations, row, col2.status, (donorStations[i]["enabled"] == "1" and "Aktivizēts" or "Deaktivizēts"), false, false)
		guiGridListSetItemText(grid.donorStations, row, col2.owner, accountNameBuilder(donorStations[i]["owner"]) or "Neviens" , false, false)
		guiGridListSetItemText(grid.donorStations, row, col2.expireDate, donorStations[i]["expire_date"] or "Nekad" , false, false)
...
function openNewStation(id, name, ip, state, order, buyNew)
	...
	local lineH = 25
	local col1 = 85
	gui.l1 = guiCreateLabel(margin, margin, w-margin*2, lineH, "JAUNA STACIJA", false, gui.wNewStation)
	...
	gui.l2 = guiCreateLabel(margin, margin+lineH, col1, lineH, "Stacijas Nos:", false, gui.wNewStation)
	...
	gui.l3 = guiCreateLabel(margin, margin+lineH*2, col1, lineH, "Stacijas IP:", false, gui.wNewStation)
	...
	gui.bMoveUp = guiCreateButton(margin, margin+lineH/2+lineH*3, buttonW , lineH, buyNew and "Uz augšu\n("..moveCost.." GC)" or "Uz Augšu",false,gui.wNewStation)
	...
	gui.bMoveDown = guiCreateButton(margin+buttonW, margin+lineH/2+lineH*3, buttonW , lineH, buyNew and "Uz Leju\n("..moveCost.." GC)" or "Uz Leju",false,gui.wNewStation)
	...
	gui.bSubmit = guiCreateButton(margin+buttonW*2, margin+lineH/2+lineH*3, buttonW , lineH, "Izveidot",false,gui.wNewStation)
	...
	addEventHandler("onClientGUIClick", gui.bSubmit, function()
		if source == gui.bSubmit then
			...
			if string.len(sName) < 1 or string.len(sIP) < 1 then
				...
				if guiGetText(gui.bSubmit) == "Saglabāt" then
					triggerServerEvent("editStation", localPlayer, id, sName, sIP)
					exports.global:playSoundCreate()
				elseif guiGetText(gui.bSubmit) == "Izveidot" then
					triggerServerEvent("createNewStation", localPlayer, sName, sIP, false)
					exports.global:playSoundCreate()
				elseif guiGetText(gui.bSubmit) == "Pirkt" then
	...
	gui.bDelete = guiCreateButton(margin+buttonW*3, margin+lineH/2+lineH*3, buttonW , lineH, "Izdzēst",false,gui.wNewStation)
	...
	gui.bClose1 = guiCreateButton(margin+buttonW*4, margin+lineH/2+lineH*3, buttonW , lineH, "Aizvērt",false,gui.wNewStation)
	...
	if id then
		...
		guiSetText(gui.l1, "REDIĢĒT STACIJU #"..id)
	else
		guiSetEnabled(gui.bMoveDown, false)
		guiSetEnabled(gui.bMoveUp, false)
		guiSetEnabled(gui.bDelete, false)
		if buyNew then
			guiSetText(gui.l1, "PIRKT JAUNU STACIJU")
		else
			guiSetText(gui.l1, "JAUNA STACIJA")
		end
	end

	if state == "Aktivizēts" then
		guiSetText(gui.bSubmit, "Deaktivizēts")
	elseif state == "Deaktivizēts" then
		guiSetText(gui.bSubmit, "Aktivizēts")
	else
		if buyNew then
			guiSetText(gui.bSubmit, "Pirkt")
		else
			guiSetText(gui.bSubmit, "Izveidot")
		end
	end

	local changeSubmitBtn = function ()
		if id and (source == gui.eName or source == gui.eIP) then
			guiSetText(gui.bSubmit, "Saglabāt")
		end
	end
...
function renewStation(station)
	...
	gui.l1 = guiCreateLabel(margin, margin, w-margin*2, lineH, "ATJAUNOT STACIJU", false, gui.wRenewStation)
	guiSetFont(gui.l1, "default-bold-small")
	guiLabelSetHorizontalAlign(gui.l1, "center", true)
	guiLabelSetVerticalAlign(gui.l1, "center", true)

	gui.l2 = guiCreateLabel(margin, margin+lineH, col1, lineH, "Tu gatavojies atjaunot staciju ID#"..station[1]..":", false, gui.wRenewStation)
	--guiSetFont(gui.l2, "default-bold-small")
	guiLabelSetVerticalAlign(gui.l2, "center", true)

	gui.l3 = guiCreateLabel(margin, margin+lineH*2, col1, lineH, "Stacijas nos: "..station[2], false, gui.wRenewStation)
	guiSetFont(gui.l3, "default-bold-small")
	guiLabelSetVerticalAlign(gui.l3, "center", true)

	gui.l4 = guiCreateLabel(margin, margin+lineH*3, col1, lineH, "Tiešraides URL: "..station[3], false, gui.wRenewStation)
	guiSetFont(gui.l4, "default-bold-small")
	guiLabelSetVerticalAlign(gui.l3, "center", true)

	gui.l5 = guiCreateLabel(margin, margin+lineH*4, col1, lineH, "Īpašnieks: "..station[4], false, gui.wRenewStation)
	guiSetFont(gui.l5, "default-bold-small")
	guiLabelSetVerticalAlign(gui.l3, "center", true)

	gui.l6 = guiCreateLabel(margin, margin+lineH*5, col1, lineH, "Derīguma termiņš: "..station[5], false, gui.wRenewStation)
	...
	gui.b7days = guiCreateButton(margin, margin+lineH/2+lineH*6, buttonW , bH, "7 dienas\n("..cost7.." GC)",false,gui.wRenewStation)
	...
	gui.b30days = guiCreateButton(margin+buttonW, margin+lineH/2+lineH*6, buttonW , bH, "30 dienas\n("..cost30.." GC)",false,gui.wRenewStation)
	...
	gui.b90days = guiCreateButton(margin+buttonW*2, margin+lineH/2+lineH*6, buttonW , bH, "90 dienas\n("..cost90.." GC)",false,gui.wRenewStation)
	...
	gui.bClose1 = guiCreateButton(margin+buttonW*3, margin+lineH/2+lineH*6, buttonW , bH, "Aizvērt",false,gui.wRenewStation)
	--guiSetFont(gui.bClose1, "default-bold-small")
	addEventHandler("onClientGUIClick", gui.bClose1, function()
		if source == gui.bClose1 then
			closeRenewStation()
