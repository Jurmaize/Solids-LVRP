...
local function updateGUI(data)
	if show then	
		...
		if job == 1 and jobVeh == 1 then -- RS Haul
			...
			jobLevel = "Sertifikāts: Prasmju līmenis "..tempJobLevel
			if truckrunsTilNextLevel then
				...
				jobProgress = "Progress: "..tempProgress.." (Tu esi meistars 8) )"
			end
			jobCurrentProgress = "Aizvestas kravas: "..tempTruckRuns
			...
			if veh and jobVeh == 1 then
				...
				truckCap = "Tilpums: "..getVehicleNameFromModel(model).." - "..exports.global:formatWeight(tempTruckCap)
				local tempLoadedSupplies = data and data.carried or manualSupplies
				loadedSupplies = "Ielādēts: "..exports.global:formatWeight(tempLoadedSupplies).." ("..math.floor(tempLoadedSupplies/tempTruckCap*100).."%)"
				local currentRoute = getElementData(localPlayer, "job-system-trucker:currentRoute") or false
				if currentRoute then
					height = 175
					nextLocation = "Mērķis: "..currentRoute[6] or "Nezināms"
					if currentRoute[4] then
						...
						if tonumber(requiredSupplies) >= tempLoadedSupplies then
							nextDropStopRequires = "Nepieciešamais daudzums: "..exports.global:formatWeight(requiredSupplies).." - NEPIETIEKAMI"
							display_distance = currentRoute[9]
							r,b,g = 255, 0, 0
						else
							nextDropStopRequires = "Nepieciešamais daudzums: "..exports.global:formatWeight(requiredSupplies).." - PIETIEKAMI"
							display_distance = currentRoute[9]
							r,b,g = 0, 255, 0
						end
					else
		...
		elseif job == 2 and jobVeh == 2 then -- Taxi
			...
			if truckrunsTilNextLevel then
				...
				jobProgress = "Progress: "..tempProgress.." (Tu esi mesitars 8) )"
			end
			jobCurrentProgress = "Aizvestas kravas: "..tempTruckRuns
