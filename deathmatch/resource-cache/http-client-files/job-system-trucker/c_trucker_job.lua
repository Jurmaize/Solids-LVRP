...
local staticBlips = {}
local staticPoints = {
	["loadupBlip"] = { -74.158203125, -1111.7626953125, 1.078125, 0 , 0},
...
function waitAtStationLoadup(thePlayer)
	...
	if thePlayer == localPlayer and vehicle and getVehicleController(vehicle) == localPlayer then
		if getElementHealth(vehicle) < 900 then
			exports.hud:sendBottomNotification(localPlayer, "RS Haul Operators:", "Tev vispirms jāsalabo autiņš..")
		else
			if not timerLoadUp then
				exports.hud:sendBottomNotification(thePlayer, "RS Haul Operators:", "Uzgaidi momentu, kamēr tavs businš tiek piekravāts")
...
function spawnFinishMarkerTruckJob()
	if not endmarker then
		...
		--endmarker2 = createMarker(-74.5908203125 , -1109.603515625, 1.078125, "checkpoint", 4, 255, 0, 0, 150)
		--setMarkerIcon(endmarker2, "finish")
		
		endmarker = createMarker(-74.5908203125 , -1109.603515625, 1.078125, "checkpoint", 4, 255, 0, 0, 150)
...
function endDelivery(thePlayer)
	if thePlayer == localPlayer then
		...
		if not vehicle or getVehicleController(vehicle) ~= localPlayer then
			exports.hud:sendBottomNotification(thePlayer, "RS Haul Operators:", "Tev jābūt RS Haul busiņā, lai pievadātu preces!")
		else
			local health = getElementHealth(vehicle)
			if health <= 900 then
				exports.hud:sendBottomNotification(thePlayer, "RS Haul Operators:", "Šis busiņš ir saists, salabo to.")
			else
				if countInkeepableObjects(exports['item-system']:getItems(vehicle)) > 0 then
					outputChatBox('Lūdzu izņem jebkādas mantas no busiņa pirms pabeidz maršrutu.', 255, 0, 0)
					return
				end
...
function waitAtDelivery(thePlayer)
	...
	if thePlayer == localPlayer and vehicle and getVehicleController(vehicle) == localPlayer then
		if getElementHealth(vehicle) < 900 then
			exports.hud:sendBottomNotification(thePlayer, "RS Haul Operators:", "Tev nepieciešams salabot busiņu.")
		else
			...
			exports.hud:sendBottomNotification(thePlayer, "RS Haul Operators:", "Uzgaidi momentu, kamēr tavs busiņš tiek izkrauts.")
			addEventHandler("onClientMarkerLeave", marker, checkWaitAtDelivery)
		end 
	end
end
...
function checkIfPlayerTruckHasEnoughtShit()
	...
	if vehicle and getVehicleController(vehicle) == localPlayer  then
		if getElementData(vehicle, "job") ~= 1 then
			exports.hud:sendBottomNotification(localPlayer, "RS Haul Operators:", "Čalīt tev jālieto RS Haul transports.")
			return false
		end
		--spawnFinishMarkerTruckJob()
		triggerLatentServerEvent("truckerjob:checkIfPlayerTruckHasEnoughtShit", localPlayer)	
	else
		exports.hud:sendBottomNotification(localPlayer, "RS Haul Operators:", "Tev jābūt busiņā, lai pabeigtu sūtījumus.")
	end
end