...
function showFactionMenu(motd, memberUsernames, memberRanks, memberPerks, memberLeaders, memberOnline, memberLastLogin, --[[memberLocation,]] factionRanks, factionWages, factionTheTeam, note, fnote, vehicleIDs, vehicleModels, vehiclePlates, vehicleLocations, memberOnDuty, towstats, phone, membersPhone, fromShowF, factionID, properties, factionRankID, rankOrder)
	if (gFactionWindow==nil) then
		...
		gFactionWindow = guiCreateWindow(0.1, 0.25, 0.85, 0.525, "F3 panelis", true)
		...
		tabOverview = guiCreateTab("Pārskats", tabs)

		-- Make members list
		gMemberGrid = guiCreateGridList(0.01, 0.015, 0.8, 0.905, true, tabOverview)

		colName = guiGridListAddColumn(gMemberGrid, "Vārds", 0.20)
		colRank = guiGridListAddColumn(gMemberGrid, "Pakāpe", 0.20)
		colOnline = guiGridListAddColumn(gMemberGrid, "Statuss", 0.115)
		colLastLogin = guiGridListAddColumn(gMemberGrid, "Pēdējo reizi pieslēdzies", 0.13)
		...
		if (factionType==2) or (factionType==3) or (factionType==4) or (factionType==5) or (factionType==6) or (factionType==7) then -- Added Mechanic type \ Adams
			...
			colWage = guiGridListAddColumn(gMemberGrid, "Alga ($)", 0.06)
		--else
			--colLocation = guiGridListAddColumn(gMemberGrid, "Location", 0.1)
		end

		if phone then
			colPhone = guiGridListAddColumn(gMemberGrid, "Telefona Nr.", 0.08)
		end
				...
				if rID == y then
					...
					if (not memberLastLogin[x]) then
						...
						elseif (memberLastLogin[x]==1) then
							login = tostring(memberLastLogin[x]) .. " pirms dienas"
						else
							login = tostring(memberLastLogin[x]) .. " dienas atpakaļ"
						end
					end
					...
					if (memberOnline[x]) then
						guiGridListSetItemText(gMemberGrid, row, colOnline, "Pieslēdzies", false, false)
						guiGridListSetItemColor(gMemberGrid, row, colOnline, 0, 255, 0)
						counterOnline = counterOnline + 1
					else
						guiGridListSetItemText(gMemberGrid, row, colOnline, "Atslēdzies", false, false)
					...
					if colDuty then
						if memberOnDuty[x] then
							guiGridListSetItemText(gMemberGrid, row, colDuty, "Strādā", false, false)
							guiGridListSetItemColor(gMemberGrid, row, colDuty, 0, 255, 0)
						else
							guiGridListSetItemText(gMemberGrid, row, colDuty, "Nestrādā", false, false)
		...
		guiSetText(ftab[factionID], tostring(teamName) .. " (" .. counterOnline .. " NO " .. (counterOnline+counterOffline) .. " lietotājiem peslēgušies.)")

		-- Make the buttons
		if (hasMemberPermissionTo(localPlayer, factionID, "del_member")) then
			gButtonKick = guiCreateButton(0.825, 0.076, 0.16, 0.06, "Izmest spēlētāju", true, tabOverview)
		...
		if (hasMemberPermissionTo(localPlayer, factionID, "change_member_rank")) then
			gButtonPromote = guiCreateButton(0.825, 0.1526, 0.16, 0.06, "Paaugstināt/Pazemināt lietotāju", true, tabOverview)
		...
		if (factionType==2) or (factionType==3) or (factionType==4) or (factionType==5) or (factionType==6) or (factionType==7) then -- Added Mechanic type \ Adams
			if (hasMemberPermissionTo(localPlayer, factionID, "modify_ranks")) then
				gButtonEditRanks = guiCreateButton(0.825, 0.2292, 0.16, 0.06, "Rediģēt pakāpes un algas", true, tabOverview)
				addEventHandler("onClientGUIClick", gButtonEditRanks, btEditRanks, false)
			end	
		else
			if (hasMemberPermissionTo(localPlayer, factionID, "modify_ranks")) then
				gButtonEditRanks = guiCreateButton(0.825, 0.2292, 0.16, 0.06, "Rediģēt pakāpes", true, tabOverview)
				addEventHandler("onClientGUIClick", gButtonEditRanks, btEditRanks, false)
			end	
		end
		if (hasMemberPermissionTo(localPlayer, factionID, "edit_motd")) then
			gButtonEditMOTD = guiCreateButton(0.825, 0.3058, 0.16, 0.06, "Rediģēt MOTD", true, tabOverview)
		...
		if (hasMemberPermissionTo(localPlayer, factionID, "add_member")) then
			gButtonInvite = guiCreateButton(0.825, 0.3824, 0.16, 0.06, "Uzaicināt lietotājus", true, tabOverview)
		...
		if (hasMemberPermissionTo(localPlayer, factionID, "respawn_vehs")) then
			gButtonRespawnui = guiCreateButton(0.825, 0.459, 0.16, 0.06, "Atjaunot transportlīdzekļus", true, tabOverview)
			addEventHandler("onClientGUIClick", gButtonRespawnui, showrespawn, false)
		end

			local _y = 0.5356
			if phone then
				gAssignPhone = guiCreateButton(0.825, _y, 0.16, 0.06, "Telefona Nr.", true, tabOverview)
			...
			if factionType >= 2 then 
				if (hasMemberPermissionTo(localPlayer, factionID, "set_member_duty")) then
					gButtonPerk = guiCreateButton(0.825, _y, 0.16, 0.06, "Rediģēt darba aprīkojumu", true, tabOverview)
			...
			if (hasMemberPermissionTo(localPlayer, factionID, "respawn_vehs")) then
				tabVehicles = guiCreateTab("(Līderis) Auto", tabs)
				...
				colVehModel = guiGridListAddColumn(gVehicleGrid, "Modelis", 0.30)
				colVehPlates = guiGridListAddColumn(gVehicleGrid, "Nummurs", 0.1)
				colVehLocation = guiGridListAddColumn(gVehicleGrid, "Atrašanās vieta", 0.4)
				gButtonVehRespawn = guiCreateButton(0.825, 0.076, 0.16, 0.06, "Atjaunot transportu", true, tabVehicles)
				gButtonAllVehRespawn = guiCreateButton(0.825, 0.1526, 0.16, 0.06, "Atjaunot visus transportlīdzekļus", true, tabVehicles)
			...
			if (hasMemberPermissionTo(localPlayer, factionID, "manage_interiors")) then
				tabProperties = guiCreateTab("(Līderis) Īpašumi", tabs)
				...
				colName = guiGridListAddColumn(gPropertyGrid, "Vārds", 0.30)
				colProLocation = guiGridListAddColumn(gPropertyGrid, "Atrašanās vieta", 0.4)
			...
			if (hasMemberPermissionTo(localPlayer, factionID, "modify_factionl_note")) then
				tabNote = guiCreateTab("(Līderis) Piezīmes", tabs)
				eNote = guiCreateMemo(0.01, 0.02, 0.98, 0.87, note or "", true, tabNote)
				gButtonSaveNote = guiCreateButton(0.79, 0.90, 0.2, 0.08, "Saglabāt", true, tabNote)
			...
			if towstats then
				if (hasMemberPermissionTo(localPlayer, factionID, "see_towstats")) then
					tabTowstats = guiCreateTab("(Līderis) tow stati", tabs)
					gTowGrid = guiCreateGridList(0.01, 0.015, 0.8, 0.905, true, tabTowstats)
					local totals = {[0] = 0, [-1] = 0, [-2] = 0, [-3] = 0, [-4] = 0}
					local colName = guiGridListAddColumn(gTowGrid, 'Vārds', 0.2)
					local colRank = guiGridListAddColumn(gTowGrid, 'Pakāpe', 0.2)
					local cols = {
						[0] = guiGridListAddColumn(gTowGrid, 'Šonedēļ', 0.1),
						[-1] = guiGridListAddColumn(gTowGrid, 'Pagaišo nedēļ', 0.1),
						[-2] = guiGridListAddColumn(gTowGrid, '2 nedēļas atpakaļ', 0.1),
						[-3] = guiGridListAddColumn(gTowGrid, '3 nedēļas atpakaļ', 0.1),
						[-4] = guiGridListAddColumn(gTowGrid, '4 nedēļas atpakaļ', 0.1)
					}
					...
					local row = guiGridListAddRow(gTowGrid)
					guiGridListSetItemText(gTowGrid, row, colName, "Kopsummas", true, false)
					for week, col in pairs(cols) do
			...
			if hasMemberPermissionTo(localPlayer, factionID, "modify_faction_note") then
				tabFNote = guiCreateTab("Piezīmes", tabs)
				fNote = guiCreateMemo(0.01, 0.02, 0.98, 0.87, fnote or "", true, tabFNote)
				guiMemoSetReadOnly(fNote, false)
	
				gButtonSaveFNote = guiCreateButton(0.79, 0.90, 0.2, 0.08, "Saglabāt", true, tabFNote)
			...
			if hasMemberPermissionTo(localPlayer, factionID, "manage_finance") then
				tabFinance = guiCreateTab("(Līderis) Finanses", tabs)
			...
			if factionType >= 2 then
				if (hasMemberPermissionTo(localPlayer, factionID, "modify_duty_settings")) then	
					tabDuty = guiCreateTab("(Līderis) Duty iestatījumi", tabs)
			...
			gButtonQuit = guiCreateButton(0.825, 0.7834, 0.16, 0.06, "Pamest frakciju", true, tabOverview)
			gButtonExit = guiCreateButton(0.825, 0.86, 0.16, 0.06, "Iziet no izvēlnes", true, tabOverview)
...
function showrespawn()
	...
	showrespawnUI = guiCreateWindow(sx/2 - 150,sy/2 - 50,300,100,"Auto atjaunošana", false)
	local lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3,"Vai tu esi pārliecināts, ka vēlies atjaunot frakcijas transportlīdzekļus?",true,showrespawnUI)
	guiLabelSetHorizontalAlign (lQuestion,"center",true)
	gButtonRespawn = guiCreateButton(0.1,0.65,0.37,0.23,"Jā",true,showrespawnUI)
	gButtonNo = guiCreateButton(0.53,0.65,0.37,0.23,"Nē",true,showrespawnUI)
...
function saveRanks(button, state)
	if (source==bRanksSave) and (button=="left") and (state=="up") then
		...
		if (found) then
			outputChatBox("Tavos rangos ir neatļauti simboli! Pārliecinies, ka nosaukumā nav simboli, kā '@.;", 255, 0, 0)
		elseif not (isNumber) then
			outputChatBox("Tavas uzliktās algas nav cipari! Pārliecinies, ka visi simboli ir cipari un nav valūtas simbolu klāt!", 255, 0, 0)
		else
			local sendRanks = { }
			local sendWages = { }
...
function btEditMOTD(button, state)
	if (source==gButtonEditMOTD) and (button=="left") and (state=="up") then
		if not (wMOTD) then
			...
			wMOTD = guiCreateWindow(x, y, width, height, "Dienas ziņa", false)
			tMOTD = guiCreateEdit(0.1, 0.2, 0.85, 0.1, tostring(theMotd), true, wMOTD)

			guiSetInputEnabled(true)

			bUpdate = guiCreateButton(0.1, 0.6, 0.85, 0.15, "Atjaunot!", true, wMOTD)
			addEventHandler("onClientGUIClick", bUpdate, sendMOTD, false)

			bMOTDClose= guiCreateButton(0.1, 0.775, 0.85, 0.15, "Aizvērt logu", true, wMOTD)
...
function sendMOTD(button, state)
	if (source==bUpdate) and (button=="left") and (state=="up") then
		...
		if (found1) or (found2) then
			outputChatBox("Tavā ziņā ir neatļauti simboli.", 255, 0, 0)
		else
...
function btInvitePlayer(button, state)
	if (source==gButtonInvite) and (button=="left") and (state=="up") then
		if not (wInvite) then
			...
			wInvite = guiCreateWindow(x, y, width, height, "Uzaicināt spēlētāju", false)
			tInvite = guiCreateEdit(0.1, 0.2, 0.85, 0.1, "Daļējs spēlētāja vārds", true, wInvite)
			addEventHandler("onClientGUIChanged", tInvite, checkNameExists)

			lNameCheck = guiCreateLabel(0.1, 0.325, 0.8, 0.3, "Spēlētājs netika atrasts vai tika atrasti vairāki ar tādu nosaukumu.", true, wInvite)
			...
			bInvite = guiCreateButton(0.1, 0.6, 0.85, 0.15, "Uzaicināt!", true, wInvite)
			guiSetEnabled(bInvite, false)
			addEventHandler("onClientGUIClick", bInvite, sendInvite, false)

			bCloseInvite = guiCreateButton(0.1, 0.775, 0.85, 0.15, "Aizvērt logu", true, wInvite)
...
function checkNameExists(theEditBox)
	...
	if (count>1) then
		guiSetText(lNameCheck, "Vairāki atrasti.")
		guiLabelSetColor(lNameCheck, 255, 255, 0)
		guiSetEnabled(bInvite, false)
	elseif (count==1) then
		guiSetText(lNameCheck, "Spēlētājs atrasts. ("..foundstr..")")
		guiLabelSetColor(lNameCheck, 0, 255, 0)
		invitedPlayer = found
		guiSetEnabled(bInvite, true)
	elseif (count==0) then
		guiSetText(lNameCheck, "Spēlētājs netika atrasts vai tika atrasti vairāki!")
		guiLabelSetColor(lNameCheck, 255, 0, 0)
		guiSetEnabled(bInvite, false)
	end
	guiLabelSetHorizontalAlign(lNameCheck, "center")
...
function btQuitFaction(button, state)
	if (button=="left") and (state=="up") and (source==gButtonQuit) then
		...
		--else
			local sx, sy = guiGetScreenSize()
			wConfirmQuit = guiCreateWindow(sx/2 - 125,sy/2 - 50,250,100,"Pamešanas apstiprinājums", false)
			local lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3,"Vai jūs patiešām vēlaties pamest " .. getTeamName(theTeam) .. "?",true,wConfirmQuit)
			guiLabelSetHorizontalAlign (lQuestion,"center",true)
			local bYes = guiCreateButton(0.1,0.65,0.37,0.23,"Jā",true,wConfirmQuit)
			local bNo = guiCreateButton(0.53,0.65,0.37,0.23,"Nē",true,wConfirmQuit)
...
function btKickPlayer(button, state)
	if (button=="left") and (state=="up") and (source==gButtonKick) then
		...
		--[[else]]if (playerName~="") then
			...
			outputChatBox("Tu atlaidi " .. playerName:gsub("_", " ") .. " no frakcijas '" .. tostring(theTeamName) .. "'.", 0, 255, 0)
			triggerServerEvent("cguiKickPlayer", getLocalPlayer(), playerName, faction_tab)
		else
			outputChatBox("Lūdzu izvēlaties spēlētaju, kuru atlaist.")
		end
	end
end
...
function btButtonPerk(button, state)
	if (button=="left") and (state=="up") and (source==gButtonPerk) then
		...
		if (playerName == "") then
			outputChatBox("Lūdzu izvēlaties darbinieku, kuru rediģēt!")
			return
		end
...
function gotPackages(factionPackages)
	...
	wPerkWindow = guiCreateWindow(x, y, width, height, "Frakcijas bonusi priekš "..playerName, false)
	...
	if not factionPerks then
		outputChatBox("Neizdevās ielādēt "..playerName.. " frakcijas bonusus")
		factionPerks = { }
	end
	...
	bPerkSave = guiCreateButton(0.05, 0.900, 0.9, 0.045, "Saglabāt", true, wPerkWindow)
	bPerkClose = guiCreateButton(0.05, 0.950, 0.9, 0.045, "Aizvērt", true, wPerkWindow)
...
function btRespawnOneVehicle(button, state)
	if button == "left" and state == "up" then
		...
		if vehID then
			...
			outputChatBox("Lūdzu izvēlaties auto, kuru respawnot.", 255, 0, 0)
		end
	end
end
	...
	if (button=="left") and (state=="up") and (source==gAssignPhone) then
		...
		if playerName ~= "" then
			...
			if not (wPhone) then
				...
				wPhone = guiCreateWindow(x, y, width, height, "Telefona Nr.", false)
				...
				guiCreateLabel(0.1, 0.2, 0.8, 0.08, "Telefona Nr. priekš " .. playerName .. ":", true, wPhone)

				guiSetInputEnabled(true)

				bSet = guiCreateButton(0.1, 0.6, 0.85, 0.15, "Piešķirt nummuru.", true, wPhone)
				addEventHandler("onClientGUIClick", bSet, setPhoneNumber, false)

				bClosePhone = guiCreateButton(0.1, 0.775, 0.85, 0.15, "Aizvērt logu", true, wPhone)
			...
			outputChatBox("Lūdzu izvēlaties spēlētāju, kam dot līderi.")
		end
	end
end
...
function btPromotePlayer(button, state)
	if (button=="left") and (state=="up") and (source==gButtonPromote) then
		...
		if (playerName == "") then
			outputChatBox("Lūdzu izvēlaties spēlētāju, kam mainīt pakāpes.", 255, 125, 0)
			return
		end
...
function setPromotionRanks(rankTbl, rankName, playerName)
	...
	if (playerName~="") then
		...
		wPromotions = guiCreateWindow(sX, sY, wX, wY, "Izmainīt frakcijas pakāpi", false)
		guiWindowSetSizable(wPromotions, false)
		-- Labels
		lPromotions1 = guiCreateLabel(14, 26, 150, 15, "Izvēlēties darbinieku:", false, wPromotions)
		...
		lPromotions3 = guiCreateLabel(14, 65, 89, 15, "Šobrīdējā pakāpe:", false, wPromotions)
		...
		guiGridListAddColumn(promotionsGridlist, "Pakāpju sarkasts", 0.9)
		-- Button
		bPromotionsUpdate = guiCreateButton(9, 280, 93, 27, "Atjaunot pakāpi", false, wPromotions)
		bPromotionsCancel = guiCreateButton(112, 280, 89, 27, "Atcelt", false, wPromotions)
		...
		outputChatBox("Izvēlaties darbinieku, kuru paaugstināt / pazemināt.", 255, 0, 0)
	end
end
...
function saveNewRank()
	...
	if (not row or row == -1) then
		outputChatBox("Izvēlaties pakāpi, kuru likt šai personai.", 255, 125, 0)
		return
	end
...
function loadFaction(tab)
	...
	colRank = guiGridListAddColumn(gMemberGrid, "Pakāpe", 0.20)
	colOnline = guiGridListAddColumn(gMemberGrid, "Statuss", 0.115)
	colLastLogin = guiGridListAddColumn(gMemberGrid, "Pēdējo reiz iekšā", 0.13)
...
function fillFactionMenu(motd, memberUsernames, memberRanks, memberPerks, memberLeaders, memberOnline, memberLastLogin, factionRanks, factionWages, factionTheTeam, note, fnote, vehicleIDs, vehicleModels, vehiclePlates, vehicleLocations, memberOnDuty, towstats, phone, membersPhone, fromShowF, factionID, properties, factionRankID, rankOrder)
	...
	local factionPackages = exports.duty:getFactionPackages(factionID)
	if factionPackages and factionType >= 2 then
		colDuty = guiGridListAddColumn(gMemberGrid, "Darbā", 0.06)
	end
			...
			if rID == y then
				...
				if (not memberLastLogin[x]) then
					...
					elseif (memberLastLogin[x]==1) then
						...
						login = tostring(memberLastLogin[x]) .. " pirms dienām"
					end
				end
				guiGridListSetItemText(gMemberGrid, row, colLastLogin, login, false, false)
				--guiGridListSetItemText(gMemberGrid, row, colLocation, memberLocation[x], false, false)
				...
				if (memberOnline[x]) then
					...
					guiGridListSetItemText(gMemberGrid, row, colOnline, "Izgājis", false, false)
					guiGridListSetItemColor(gMemberGrid, row, colOnline, 255, 0, 0)
					counterOffline = counterOffline + 1
				end
	...
	-- Update the window title
	guiSetText(ftab[factionID], tostring(teamName) .. " (" .. counterOnline .. " no " .. (counterOnline+counterOffline) .. " lietotājiem pieslēgušies.)")

	-- Make the buttons

		-- Make the buttons
		if (hasMemberPermissionTo(localPlayer, factionID, "del_member")) then
			gButtonKick = guiCreateButton(0.825, 0.076, 0.16, 0.06, "Izmest lietotāju", true, tabOverview)
			addEventHandler("onClientGUIClick", gButtonKick, btKickPlayer, false)
		end	

		if (hasMemberPermissionTo(localPlayer, factionID, "change_member_rank")) then
			gButtonPromote = guiCreateButton(0.825, 0.1526, 0.16, 0.06, "Paaugstināt/Pazemināt darbinieku", true, tabOverview)
			addEventHandler("onClientGUIClick", gButtonPromote, btPromotePlayer, false)	
		end
		...
		if factionType >= 2 then 
			if (hasMemberPermissionTo(localPlayer, factionID, "set_member_duty")) then
				gButtonPerk = guiCreateButton(0.825, _y, 0.16, 0.06, "Rediģēt duty", true, tabOverview)
				addEventHandler("onClientGUIClick", gButtonPerk, btButtonPerk, false)
			end	
		end
		if (hasMemberPermissionTo(localPlayer, factionID, "respawn_vehs")) then
			gButtonRespawnui = guiCreateButton(0.825, 0.459, 0.16, 0.06, "Atjaunot transportus", true, tabOverview)
			addEventHandler("onClientGUIClick", gButtonRespawnui, showrespawn, false)

			tabVehicles = guiCreateTab("(Līderis) Transportlīdzekļi", tabs)
			...
			colVehPlates = guiGridListAddColumn(gVehicleGrid, "Nummurs", 0.1)
			colVehLocation = guiGridListAddColumn(gVehicleGrid, "Atrašanās vieta", 0.4)
			gButtonVehRespawn = guiCreateButton(0.825, 0.076, 0.16, 0.06, "Respawnot transportlīdzekli", true, tabVehicles)
			gButtonAllVehRespawn = guiCreateButton(0.825, 0.1526, 0.16, 0.06, "Respawnot visus transportlīdzekļus", true, tabVehicles)
		...
		if (hasMemberPermissionTo(localPlayer, factionID, "manage_interiors")) then
			...
			colProID = guiGridListAddColumn(gPropertyGrid, "ID", 0.1)
			colName = guiGridListAddColumn(gPropertyGrid, "Vārds", 0.30)
			colProLocation = guiGridListAddColumn(gPropertyGrid, "Atrašanās", 0.4)
		...
		if towstats then
			if (hasMemberPermissionTo(localPlayer, factionID, "see_towstats")) then
				...
				local cols = {
					[0] = guiGridListAddColumn(gTowGrid, 'šonedēļ', 0.1),
					[-1] = guiGridListAddColumn(gTowGrid, 'pagājušo nedēļu', 0.1),
					[-2] = guiGridListAddColumn(gTowGrid, '2 nedēļas atpakaļ', 0.1),
					[-3] = guiGridListAddColumn(gTowGrid, '3 nedēļas atpakaļ', 0.1),
					[-4] = guiGridListAddColumn(gTowGrid, '4 nedēļas atpakaļ', 0.1)
				}
...
function loadFinance()
	if source == tabFinance then
		if not financeLoaded then
			local label = guiCreateLabel(0,0,1,1,"Lādējas...",true,tabFinance)
...
function fillFinance(factionID, bankThisWeek, bankPrevWeek, bankmoney, vehiclesvalue, propertiesvalue)
	...
	tabWeeklyStatement = guiCreateTab("Nedēļas Finanses", financeTabs)
		...
		statementColLast = guiGridListAddColumn(weeklyStatementGridlist, "Pagaišo Nedeļu", 0.25)
		statementColThis = guiGridListAddColumn(weeklyStatementGridlist, "Šonedēļ", 0.25)

		assetsGridlist = guiCreateGridList(639, 11, 395, 298, false, tabWeeklyStatement)
		guiGridListAddColumn(assetsGridlist, "Resursi", 0.65)
		guiGridListAddColumn(assetsGridlist, "Vērtība", 0.25)

		local row = guiGridListAddRow(assetsGridlist)
		guiGridListSetItemText(assetsGridlist, row, 1, "Bankas konts", false, false)
		...
		local row = guiGridListAddRow(assetsGridlist)
		guiGridListSetItemText(assetsGridlist, row, 1, "Transports", false, false)
		...
		local row = guiGridListAddRow(assetsGridlist)
		guiGridListSetItemText(assetsGridlist, row, 1, "Īpašumi", false, false)
		...
		local row = guiGridListAddRow(assetsGridlist)
		guiGridListSetItemText(assetsGridlist, row, 1, "KOPSUMMA", false, false)
        ...
        local label1 = guiCreateLabel(413, 10, 156, 30, "Frakcijas finanses rāda maksimāli divas nedēļas atpakaļ.", false, tabWeeklyStatement)
        	guiSetFont(label1, "default-small")
        	guiLabelSetHorizontalAlign(label1, "left", true)

        local label2 = guiCreateLabel(413, 292, 156, 15, "Uzspied divreiz, lai redzētu detalizētāk.", false, tabWeeklyStatement)
        	guiSetFont(label2, "default-small")

	tabTransactions = guiCreateTab("Transakcijas", financeTabs)
...
function createDutyMain()
    ...
    guiGridListAddColumn(Duty.gridlist[1], "Vārds", 0.2)
    guiGridListAddColumn(Duty.gridlist[1], "Rādiuss", 0.1)
    guiGridListAddColumn(Duty.gridlist[1], "Interjers", 0.1)
    guiGridListAddColumn(Duty.gridlist[1], "Dimensija", 0.12)
    ...
    Duty.button[1] = guiCreateButton(0.005, 0.939, 0.09, 0.0504, "Pievienot vietu", true, tabDuty)
    ...
    Duty.label[1] = guiCreateLabel(0.0059, 0.0076, 0.2625, 0.03, "Duty vietas", true, tabDuty)
    guiLabelSetHorizontalAlign(Duty.label[1], "center", false)
    Duty.button[2] = guiCreateButton(0.1, 0.939, 0.099, 0.0504, "Noņemt vietu", true, tabDuty)
    ...
    Duty.button[3] = guiCreateButton(0.205, 0.939, 0.099, 0.0504, "Rediģēt duty vietu", true, tabDuty)
    ...
    guiGridListAddColumn(Duty.gridlist[2], "Vārds", 0.3)
    guiGridListAddColumn(Duty.gridlist[2], "Atrašanās vieta", 0.4)

    Duty.label[2] = guiCreateLabel(0.68, 0.0076, 0.2636, 0.03, "Duty bonusi", true, tabDuty)
    guiLabelSetHorizontalAlign(Duty.label[2], "center", false)
    Duty.button[4] = guiCreateButton(0.66, 0.939, 0.09, 0.0504, "Pievienot jaunu duty", true, tabDuty)
    guiSetProperty(Duty.button[4], "NormalTextColour", "FFAAAAAA")
    addEventHandler("onClientGUIClick", Duty.button[4], createDuty, false)

    Duty.button[5] = guiCreateButton(0.765, 0.939, 0.09, 0.0504, "Noņemt Duty", true, tabDuty)
    guiSetProperty(Duty.button[5], "NormalTextColour", "FFAAAAAA")
    addEventHandler("onClientGUIClick", Duty.button[5], removeDuty, false)

    Duty.button[6] = guiCreateButton(0.869, 0.939, 0.09, 0.0504, "Rediģēt duty bonusus", true, tabDuty)
    ...
    guiGridListAddColumn(Duty.gridlist[3], "Auto ID", 0.4)
    guiGridListAddColumn(Duty.gridlist[3], "Auto", 0.5)

    Duty.label[3] = guiCreateLabel(0.325, 0.0076, 0.2886, 0.03, "Duty Auto vietas", true, tabDuty)
    guiLabelSetHorizontalAlign(Duty.label[3], "center", false)
    Duty.button[8] = guiCreateButton(0.3355, 0.5304, 0.1, 0.0504, "Pievienot duty auto", true, tabDuty)
    ...
    Duty.button[9] = guiCreateButton(0.5177, 0.5304, 0.1, 0.0504, "Noņemt duty auto", true, tabDuty)
		...
		for k,v in pairs(customg[customEditID][5]) do
			if tonumber(v[2]) >= 0 then -- Items
				...
				if tonumber(v[2]) == -100 then
					guiGridListSetItemText(DutyCreate.gridlist[3], row, 1, "Bruņas", false, false) -- Weapon Name
	...
	for k,v in pairs(allowList) do
		if tonumber(v[2]) >= 0 then -- Items
			...
			if customEditID == 0 or (customEditID ~= 0 and not customg[customEditID][5][tostring(v[1])]) then
				...
				if tonumber(v[2]) == -100 then
					guiGridListSetItemText(DutyCreate.gridlist[1], row, 1, "Bruņas", false, false)
...
function createDuty()
    ...
    DutyCreate.window[1] = guiCreateWindow(450, 310, 768, 566, "Duty Rediģēšanas logs - Galvenais", false)
    guiWindowSetSizable(DutyCreate.window[1], false)
    centerWindow(DutyCreate.window[1])

    DutyCreate.button[1] = guiCreateButton(600, 512, 158, 44, "Atcelt", false, DutyCreate.window[1])
    ...
    DutyCreate.button[2] = guiCreateButton(454, 512, 138, 44, "Saglabāt", false, DutyCreate.window[1])
    ...
    guiGridListAddColumn(DutyCreate.gridlist[1], "Ieroča nosaukums", 0.5)
    guiGridListAddColumn(DutyCreate.gridlist[1], "Maksimālais daudzums munīcijas", 0.5)
   ...
   --guiGridListAddColumn(DutyCreate.gridlist[2], "ID", 0.1)
    guiGridListAddColumn(DutyCreate.gridlist[2], "Mantas nosaukums", 0.3)
    guiGridListAddColumn(DutyCreate.gridlist[2], "Apraksts", 0.7)
    ...
    guiGridListAddColumn(DutyCreate.gridlist[3], "Ierocis", 0.5)
    guiGridListAddColumn(DutyCreate.gridlist[3], "Munīcija", 0.3)
    ...
    DutyCreate.edit[2] = guiCreateEdit(445, 81, 127, 27, "Munīcijas daudzums", false, DutyCreate.window[1])
    ...
    guiGridListAddColumn(DutyCreate.gridlist[4], "Manta", 0.5)
    ...
    DutyCreate.button[8] = guiCreateButton(160, 512, 138, 44, "Atrašanās vieta", false, DutyCreate.window[1])
    ...
    DutyCreate.label[3] = guiCreateLabel(57, 19, 319, 15, "Pieejamie ieroči", false, DutyCreate.window[1])
    guiLabelSetHorizontalAlign(DutyCreate.label[3], "center", false)
    DutyCreate.label[4] = guiCreateLabel(582, 17, 176, 17, "Duty ieroči", false, DutyCreate.window[1])
    guiLabelSetHorizontalAlign(DutyCreate.label[4], "center", false)
    DutyCreate.label[5] = guiCreateLabel(57, 228, 319, 15, "Pieejamās mantas", false, DutyCreate.window[1])
    guiLabelSetHorizontalAlign(DutyCreate.label[5], "center", false)
    DutyCreate.label[6] = guiCreateLabel(583, 227, 175, 21, "Duty mantas", false, DutyCreate.window[1])
    guiLabelSetHorizontalAlign(DutyCreate.label[6], "center", false)
    DutyCreate.label[7] = guiCreateLabel(14, 463, 88, 32, "Duty Nosaukums:", false, DutyCreate.window[1])
...
function createLocations()
    ...
    DutyLocations.window[1] = guiCreateWindow(573, 285, 520, 423, "Duty rediģēšanas logs - Vietas", false)
    ...
    guiGridListAddColumn(DutyLocations.gridlist[1], "Nosaukums", 0.9)
    ...
    guiGridListAddColumn(DutyLocations.gridlist[2], "Nosaukums", 0.9)
    ...
    DutyLocations.label[1] = guiCreateLabel(10, 19, 233, 17, "Visas vietas", false, DutyLocations.window[1])
    guiLabelSetHorizontalAlign(DutyLocations.label[1], "center", false)
    DutyLocations.label[2] = guiCreateLabel(270, 19, 233, 17, "Duty vietas", false, DutyLocations.window[1])
    guiLabelSetHorizontalAlign(DutyLocations.label[2], "center", false)
    DutyLocations.button[3] = guiCreateButton(270, 367, 146, 36, "Saglabāt", false, DutyLocations.window[1])
    ...
    DutyLocations.button[4] = guiCreateButton(103, 367, 146, 36, "Atcelt", false, DutyLocations.window[1])
	...
	if string.find(raw, ":") then
		...
		if tonumber(howAboutIt[1]) and tonumber(howAboutIt[2]) then
			if not skinAlreadyExists(tonumber(howAboutIt[1]), tonumber(howAboutIt[2])) then
				...
				outputChatBox("Nevar pievienot to pašu skinu divreiz.", 255, 0, 0)
			end
		else
			outputChatBox("Lūgums lietot tikai ciparus", 255, 0, 0)
		end
	else
...
function createSkins(customSkins)
	if DutySkins.window[1] and isElement(DutySkins.window[1]) and customSkins and type(customSkins)=='table' then
	    ...
	    DutySkins.label[1] = guiCreateLabel(10, 80, 132, 138, "Lādējas...", false, DutySkins.window[1])
	    ...
	    DutySkins.label[2] = guiCreateLabel(10, 27, 132, 20, "Skina ID: ", false, DutySkins.window[1])
	    ...
	    guiGridListAddColumn(DutySkins.gridlist[2], "Pievienoti", 0.85)
	    if customEditID == 0 then
	    ...
	    DutySkins.button[1] = guiCreateButton(10, 228, 87, 25, "Atcelt", false, DutySkins.window[1])
	    DutySkins.button[2] = guiCreateButton(103, 228, 87, 25, "Pievienot", false, DutySkins.window[1])
	    guiSetVisible(DutySkins.button[2], false)
	    DutySkins.button[3] = guiCreateButton(197, 228, 87, 25, "Saglabāt", false, DutySkins.window[1])
	    ...
	    addEventHandler('onClientGUIClick', DutySkins.window[1], function()
	    	...
	    	if source == DutySkins.gridlist[1] then
				...
				if r >= 0 then
					...
					guiSetText(DutySkins.button[2], "Pievienot")
					guiSetText(DutySkins.edit[1], "")
				end
	    	...
	    	elseif source == DutySkins.gridlist[2] then
				...
				if r >= 0 then
					guiSetVisible(DutySkins.button[2], true)
					guiSetText(DutySkins.button[2], "Noņemt")
				end
			...
			elseif source == DutySkins.button[2] then
				if guiGetText(DutySkins.button[2]) == "Noņemt" then
				...
				elseif guiGetText(DutySkins.button[2]) == 'Pievienot' then
					r, c = guiGridListGetSelectedItem ( DutySkins.gridlist[1] )
					if r >= 0 then
						...
						if not skinAlreadyExists(tonumber(howAboutIt[1]), tonumber(howAboutIt[2])) then
							...
							outputChatBox("Nedrīkst pievienot to pašu skinu divreiz!", 255, 0, 0)
						end
					else
						...
						if raw then
							...
							outputChatBox("Lūdzu lietot tikai ciparus!", 255, 0, 0)
						end
						guiSetText(DutySkins.edit[1], "")
					end
				end
...
function createVehicleAdd()
    ...
    DutyVehicleAdd.window[1] = guiCreateWindow(685, 338, 335, 85, "Pievienot jaunu duty transportu", false)
    guiWindowSetSizable(DutyVehicleAdd.window[1], false)
    centerWindow(DutyVehicleAdd.window[1])

    DutyVehicleAdd.edit[1] = guiCreateEdit(9, 26, 181, 40, "Auto ID", false, DutyVehicleAdd.window[1])
    DutyVehicleAdd.button[1] = guiCreateButton(192, 26, 62, 40, "Pievienot", false, DutyVehicleAdd.window[1])
    ...
    DutyVehicleAdd.button[2] = guiCreateButton(263, 26, 62, 40, "Aizvērt", false, DutyVehicleAdd.window[1])
...
function createDutyLocationMaker(x, y, z, r, i, d, name)
    ...
    DutyLocationMaker.window[1] = guiCreateWindow(638, 285, 488, 198, "Pievienot duty vietu", false)
    guiWindowSetSizable(DutyLocationMaker.window[1], false)
    centerWindow(DutyLocationMaker.window[1])

    DutyLocationMaker.label[1] = guiCreateLabel(8, 24, 44, 19, "X vērtība:", false, DutyLocationMaker.window[1])
    DutyLocationMaker.edit[1] = guiCreateEdit(56, 24, 135, 20, "", false, DutyLocationMaker.window[1])
    DutyLocationMaker.label[2] = guiCreateLabel(201, 24, 53, 19, "Y vērtība:", false, DutyLocationMaker.window[1])
    DutyLocationMaker.edit[2] = guiCreateEdit(253, 23, 88, 20, "", false, DutyLocationMaker.window[1])
    DutyLocationMaker.label[3] = guiCreateLabel(355, 25, 52, 18, "Z vērtība:", false, DutyLocationMaker.window[1])
    DutyLocationMaker.edit[3] = guiCreateEdit(406, 23, 71, 20, "", false, DutyLocationMaker.window[1])
    DutyLocationMaker.label[4] = guiCreateLabel(8, 60, 49, 18, "Rādiuss:", false, DutyLocationMaker.window[1])
    DutyLocationMaker.edit[4] = guiCreateEdit(53, 58, 82, 20, "1-10", false, DutyLocationMaker.window[1])
    DutyLocationMaker.label[5] = guiCreateLabel(162, 61, 72, 17, "Interjers:", false, DutyLocationMaker.window[1])
    DutyLocationMaker.edit[5] = guiCreateEdit(216, 58, 93, 20, "", false, DutyLocationMaker.window[1])
    DutyLocationMaker.label[6] = guiCreateLabel(336, 60, 60, 18, "Dimensija:", false, DutyLocationMaker.window[1])
    DutyLocationMaker.edit[6] = guiCreateEdit(402, 58, 75, 20, "", false, DutyLocationMaker.window[1])
    DutyLocationMaker.label[7] = guiCreateLabel(9, 92, 57, 21, "Vārds:", false, DutyLocationMaker.window[1])
    DutyLocationMaker.label[8] = guiCreateLabel(10, 119, 467, 28, "Duty nosaukums tiek tikai lietots identificēšanai.", false, DutyLocationMaker.window[1])
    ...
    DutyLocationMaker.button[1] = guiCreateButton(10, 149, 115, 37, "Ievietot šobrīdējo pozīciju.", false, DutyLocationMaker.window[1])
    ...
    DutyLocationMaker.button[2] = guiCreateButton(184, 149, 115, 37, "Aizvērt", false, DutyLocationMaker.window[1])
    ...
    DutyLocationMaker.button[3] = guiCreateButton(357, 149, 115, 37, "Saglabāt", false, DutyLocationMaker.window[1])
...
function saveGUI()
	if source == DutyCreate.button[2] then -- Main
		...
		if name ~= "" then
			...
			outputChatBox("Lūgums ievadīt nosaukumu šim duty.", 255, 0, 0)
			return
		end
		destroyElement(DutyCreate.window[1])
	...
	elseif source == DutyVehicleAdd.button[1] then -- Vehicle Add
		...
		if not duplicateVeh("location", id, factionIDg) then
			...
			outputChatBox("Šis transports jau ir pievienots!", 255, 0, 0)
		end
		...
		if (x and y and z and r and i and d and name) then
			if r >= 1 and r <=10 then
				if string.len(name) > 0 then
					...
					outputChatBox("Jums ir jāievada nosaukums.", 255, 0, 0)
					return
				end
			else
				outputChatBox("Rādiusam ir jābūt no 1 līdz 10.", 255, 0, 0)
				return
			end
		else
			outputChatBox("Lūgums ievadīt visu informāciju pareizi!", 255, 0, 0)
			return
		end
		locationEditID = 0
		destroyElement(DutyLocationMaker.window[1])
