...
local pdReleasePed = createPed(280, 246.4609375, 120.3935546875, 1003.2662963867 )
setPedRotation( pdReleasePed, 176.33 )
setElementDimension( pdReleasePed, 1)
...
local rtReleasePed = createPed(240, 2685.8100585938, -2532.3601074219, 13.536288261414)
setElementInterior(rtReleasePed, 0)
setElementDimension(rtReleasePed, 0)
setElementRotation(rtReleasePed, 0, 0, 356, "default", true)
...
function openImpGui(dep, vehid, first, last, vehName, plate, vin, laneData)
	...
	gui.label[1] = guiCreateLabel(14, 29, 365, 18, dep == "LSAK" and "Darbinieka informācija" or "LSPD darbinieka informācija", false, gui.window[1])
	guiSetFont(gui.label[1], "default-bold-small")
	gui.label[2] = guiCreateLabel(31, 58, 82, 25, "Vārds:", false, gui.window[1])
	...
	gui.label[3] = guiCreateLabel(31, 87, 82, 25, "Uzvārds:", false, gui.window[1])
	...
	gui.label[4] = guiCreateLabel(31, 117, 82, 25, "Žetons: ", false, gui.window[1])
	...
	gui.label[5] = guiCreateLabel(31, 147, 82, 25, "Pakāpe:", false, gui.window[1])
	...
	gui.label[7] = guiCreateLabel(31, 219, 82, 25, "Modelis:", false, gui.window[1])
	...
	gui.label[8] = guiCreateLabel(31, 249, 82, 25, "Nummurs:", false, gui.window[1])
	...
	gui.label[10] = guiCreateLabel(406, 29, 365, 18, "Konfiscēšanas informācija", false, gui.window[1])
	...
	gui.label[11] = guiCreateLabel(423, 57, 82, 25, "Pārkāpumi:", false, gui.window[1])
	...
	gui.label[12] = guiCreateLabel(423, 88, 82, 25, "Atrašanas vieta:", false, gui.window[1])
	guiLabelSetVerticalAlign(gui.label[12], "center")
	gui.label[1] = guiCreateLabel(423, 118, 119, 25, "Pozīcija placī:", false, gui.window[1])
	...
	guiLabelSetVerticalAlign(gui.label[2], "center")
	gui.label[13] = guiCreateLabel(423, 148, 119, 25, "Tiks atlaists:", false, gui.window[1])
	guiLabelSetVerticalAlign(gui.label[13], "center")
	gui.edit[11] = guiCreateEdit(542, 148, 142, 25, dep == "LSAK" and "N/A" or "1", false, gui.window[1])
	guiEditSetMaxLength(gui.edit[11], 200)
	gui.label[14] = guiCreateLabel(694, 148, 85, 25, "0 diena = konfiscēta šodien", false, gui.window[1])
	guiLabelSetVerticalAlign(gui.label[14], "center")
	gui.label[15] = guiCreateLabel(413, 217, 365, 18, "Papildus informācija", false, gui.window[1])
	guiSetFont(gui.label[15], "default-bold-small")
	gui.memo[1] = guiCreateMemo(423, 242, 348, 63, "", false, gui.window[1])
	gui.button[1] = guiCreateButton(21, 338, 368, 41, "Atcelt", false, gui.window[1])
	gui.button[2] = guiCreateButton(413, 338, 368, 41, "Konfiscēt", false, gui.window[1])
	gui.label[3] = guiCreateLabel(423, 178, 119, 25, "Sods:", false, gui.window[1])
	guiLabelSetVerticalAlign(gui.label[3], "center")
	gui.edit[12] = guiCreateEdit(542, 178, 142, 25, dep == "LSAK" and "N/A" or "0", false, gui.window[1])
	guiEditSetMaxLength(gui.edit[1], 200)

	guiSetEnabled(gui.label[3], dep ~= "LSAK")
	guiSetEnabled(gui.label[14], dep ~= "LSAK")
	guiSetEnabled(gui.label[13], dep ~= "LSAK")
	guiSetEnabled(gui.edit[11], dep ~= "LSAK")
	guiSetEnabled(gui.edit[12], dep ~= "LSAK")
	...
	addEventHandler("onClientGUIClick", gui.window[1], function ()
		...
		elseif source == gui.button[2] then
			...
			if dep ~= "LSAK" and (not days or not tonumber(days) or tonumber(days) < 0 or tonumber(days) > 500) then
			...
			if dep ~= "LSAK" and (not fine or not tonumber(fine) or tonumber(fine) < 0 or tonumber(fine) > 50000) then
...
function openReleaseGUI(factionId, vehs)
	...
	rgui.window[1] = guiCreateWindow(587, 335, 749, 326, factionName.." konfiscēšanas placis | Reģistratūra", false)
	guiWindowSetSizable(rgui.window[1], false)
	exports.global:centerWindow(rgui.window[1])

	rgui.label[1] = guiCreateLabel(27, 40, 696, 35, "Lai izpirktu auto būs jasamaksā par vilkšanas cenu, kā arī ikdienas uzturēšanas izmaksas, kas ir $"..exports.global:formatMoney(costPerDay).." dienā. Iespējams nāksies arī samaksāt kādus sodus, kas ir saistīti ar Jūsu auto.", false, rgui.window[1])
	...
	rgui.col_mod = guiGridListAddColumn(rgui.gridlist[1], "Modelis", 0.4)
	rgui.col_days = guiGridListAddColumn(rgui.gridlist[1], "Konfiscēts", 0.15)
	rgui.col_fine = guiGridListAddColumn(rgui.gridlist[1], "Sods ($)", 0.1)
	rgui.col_date = guiGridListAddColumn(rgui.gridlist[1], "Izlaišanas datums", 0.2)
	...
	for vin, veh in pairs(vehs) do
		...
		local releasteDate = (veh.lane and veh.lane.release_date) and veh.lane.release_date or "Konfiscēts"
		if factionId == 4 then
			releasteDate = "Jebkad"
		end
	...
	rgui.label[2] = guiCreateLabel(37, 271, 188, 34, "Kopā: --", false, rgui.window[1])
	...
	rgui.button[1] = guiCreateButton(621, 275, 102, 31, "Samaksāt un izlaist", false, rgui.window[1])
	rgui.button[2] = guiCreateButton(514, 275, 102, 31, "Aizvērt", false, rgui.window[1])
	...
	addEventHandler("onClientGUIClick", rgui.window[1], function()
		...
		if source == rgui.button[1] then
			if selectedRow == -1 or selectedCol == -1 then
				...
				if exports.global:hasMoney(localPlayer, cost) then
					...
					triggerServerEvent("tow:pedSay", localPlayer, pedName, "Vai es varētu saņemt $"..exports.global:formatMoney(cost).." lūdzu?")
				end
			end
		...
		elseif source == rgui.gridlist[1] then
			if selectedvin then
				...
				guiSetText(rgui.label[2], "Kopā: $"..exports.global:formatMoney(cost))
