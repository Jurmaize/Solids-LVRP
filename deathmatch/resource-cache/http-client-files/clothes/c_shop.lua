...
function updateGrid(dopont_npc)
	...
	if not dopont_npc then
		...
		guiGridListSetItemText(grid, row, 2, v.description.." dizainēja "..v.creator_charname, false, false)
...
function listClothes(item, list)
	...
	if window and isElement(window) then
		...
		local margin = 30
		window = guiCreateWindow(screen_width - width - 45, screen_height - height-110, width, height, "Kolekcija #"..(item.itemValue or ""), false)
...
function listCreateGuiElements(dopont_npc)
	...
	guiGridListAddColumn(grid, 'Apraksts', 0.5)
	guiGridListAddColumn(grid, 'Kolekcija', 0.07)
	guiGridListAddColumn(grid, 'Cena', 0.1)
	if dopont_npc then
