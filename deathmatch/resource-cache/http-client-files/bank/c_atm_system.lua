...
function requestATMInterfacePIN(theATM, atmLocationName)
	...
	if not (wPIN) then
		...
		wPIN = guiCreateWindow(x, y, width, height, "ATM bankomāts #"..getElementData(theATM, "dbid")..", kas atrodas "..(atmLocationName or "Unknown Area"), false)
			guiWindowSetSizable(wPIN, false)

		local tabPanel = guiCreateTabPanel(0.05, 0.05, 0.9, 0.85, true, wPIN)
		local tabPersonal = guiCreateTab("Lūdzu ievadiet PIN kodu", tabPanel)
		...
		Label_Error = guiCreateLabel(20,90,220,30,"Lūdzu ievadiet 4 ciparu PIN kodu",false,tabPersonal)
		...
		bEnter = guiCreateButton(20,268+posYOffset,225,66,"Ievadīt",false,tabPersonal)
        ...
        local bClose = guiCreateButton(20,344+posYOffset,225,66,"Iziet",false,tabPersonal)
...
function respondToATMInterfacePIN(Label_Error_Msg, r,g,b, action, otherInfo)
	...
	if action == "cardRemoved" then
		exports.hud:sendBottomNotification(localPlayer, "ATM bankomāts", "Bankomāts pareizi nestrādā! Karte atgriezta.")
		setTimer(closeATMInterfacePIN, 2000, 1)
	elseif action == "failedLessThan3" then
		exports.hud:sendBottomNotification(localPlayer, "ATM bankomāts", "Atļauja noliegta! Ievadot PIN kodu nepareizi vairāk pa 3 reizēm liks bankomātam neatdot karti! ("..otherInfo.."/3)")
		guiSetEnabled(bEnter, true)
	elseif action == "failedMoreThan3" then
		exports.hud:sendBottomNotification(source, "ATM bankomāts", "Atļauja noliegta! ("..otherInfo.."/3). Bankomāts apēda karti. Lūdzu sazinieties ar banku, lai atgūtu karti.")
		setTimer(closeATMInterfacePIN, 2000, 1)
	elseif action == "locked" then
		exports.hud:sendBottomNotification(source, "ATM bankomāts", "Šis bankas konts ir aizslēgts '"..otherInfo.."' Lūdzu sazinieties ar banku!")
		guiSetEnabled(bEnter, true)
	elseif action == "success" then
		exports.hud:sendBottomNotification(source, "ATM bankomāts", "Piekļuve piešķirta!")
...
function openATMGrantedGUI(cardInfo, depositable, limit, withdrawable, theATM, atmLocationName, isPinCodeDefault)
	if not (guiATMGranted) then
		...
		guiATMGranted = guiCreateWindow(x, y, width, height, "ATM bankomāts #"..getElementData(theATM, "dbid").." atrodas "..(atmLocationName or "Unknown Area"), false)
		guiWindowSetSizable(guiATMGranted, false)

		tabPanel = guiCreateTabPanel(0.05, 0.05, 0.9, 0.85, true, guiATMGranted)

		tabPersonal = guiCreateTab("Personīgā banka", tabPanel)
		tabPersonalTransactions = guiCreateTab("Transakciju vēsture", tabPanel)
		tabChangePin = guiCreateTab("Mainīt PIN", tabPanel)
		...
		local lBankName = guiCreateLabel(0.1, 0.05, 0.9, 0.05, "Kartes īpašnieks:                           "..(cardInfo[1] or "Unknown"):gsub("_", " "), true, tabPersonal)
		guiSetFont(lBankName, "default-bold-small")

		local lBankNumber = guiCreateLabel(0.1, 0.1, 0.9, 0.05, "Bankas konta nummurs:       "..(cardInfo[2] or "Unknown"), true, tabPersonal)
		guiSetFont(lBankNumber, "default-bold-small")

		local posYOffset = 0.2
		bClose = guiCreateButton(0.75, 0.91, 0.2, 0.1, "Aizvērt", true, guiATMGranted)
		addEventHandler("onClientGUIClick", bClose, function()
			hideATMGrantedGUI()
			triggerServerEvent("bank:ejectATMCard", localPlayer, theATM)
		end, false)
		...
		lBalance = guiCreateLabel(0.1, 0.05+posYOffset, 0.9, 0.05, "Bilance: $" .. exports.global:formatMoney(balance), true, tabPersonal)
		guiSetFont(lBalance, "default-bold-small")

		if withdrawable then
			-- WITHDRAWAL PERSONAL
			lWithdrawP = guiCreateLabel(0.1, 0.15+posYOffset, 0.2, 0.05, "Izņemt:", true, tabPersonal)
			...
			bWithdrawP = guiCreateButton(0.44, 0.13+posYOffset, 0.2, 0.075, "Izņemt", true, tabPersonal)
			addEventHandler("onClientGUIClick", bWithdrawP, function()
				...
				if not amount or amount <= 0 or math.ceil( amount ) ~= amount then
					exports.hud:sendBottomNotification(localPlayer, "ATM bankomāts", "Lūdzu ievadiet pozitīvu daudzumu!")
				elseif (amount>money) then
					exports.hud:sendBottomNotification(localPlayer, "ATM bankomāts", "Jums nav tik daudz naudas! LOL")
				elseif not _depositable and limitedwithdraw ~= 0 and oldamount + amount > limitedwithdraw then
					exports.hud:sendBottomNotification(localPlayer, "ATM bankomāts", "Šis bankomāts ļauj izņemt tikai $" .. exports.global:formatMoney( limitedwithdraw - oldamount ) .. ".")
				else
			...
			lWithdrawP = guiCreateLabel(0.1, 0.15+posYOffset, 0.5, 0.05, "Šajā bankomātā nav iespējams izņemt naudu!", true, tabPersonal)
			guiSetFont(lWithdrawP, "default-bold-small")
		end

		if (depositable) then
			-- DEPOSIT PERSONAL
			lDepositP = guiCreateLabel(0.1, 0.25+posYOffset, 0.2, 0.05, "Iemaksāt:", true, tabPersonal)
			...
			bDepositP = guiCreateButton(0.44, 0.23+posYOffset, 0.2, 0.075, "Iemaksāt", true, tabPersonal)
			...
			lDepositP = guiCreateLabel(0.1, 0.25+posYOffset, 0.5, 0.05, "Šajā bankomātā nav iespējams iemaksāt naudu!", true, tabPersonal)
			lCharge = guiCreateLabel(0.1, 0.29+posYOffset, 0.5, 0.05, "Šim bankomātām ir $2 darba maksa", true, tabPersonal)
			guiSetFont(lDepositP, "default-bold-small")
			guiSetFont(lCharge, "default-bold-small")

			if limitedwithdraw > 0 and withdrawable then
				tDepositP = guiCreateLabel(0.67, 0.15+posYOffset, 0.2, 0.05, "Maksimums: $" .. ( limitedwithdraw - ( getElementData( source, "withdrawn" ) or 0 ) ) .. ".", true, tabPersonal)
				guiSetFont(tDepositP, "default-bold-small")
			end
		end
		...
		if hoursplayed >= 12 then
			-- TRANSFER PERSONAL
			lTransferP = guiCreateLabel(0.1, 0.45+posYOffset, 0.2, 0.05, "Pārskaitīt uz:", true, tabPersonal)
			...
			bTransferP = guiCreateButton(0.44, 0.43+posYOffset, 0.2, 0.075, "Pārskaitīt uz", true, tabPersonal)
			addEventHandler("onClientGUIClick", bTransferP, function()
				...
				if not amount or amount <= 0 or math.ceil( amount ) ~= amount then
					exports.hud:sendBottomNotification(localPlayer, "ATM bankomāts", "Lūdzu ievadiet pozitīvu daudzumu!")
				elseif (amount>money) then
					exports.hud:sendBottomNotification(localPlayer, "ATM bankomāts", "Jums nav pietiekami daudz naudas! LOL")
				elseif reason == "" then
					exports.hud:sendBottomNotification(localPlayer, "ATM bankomāts", "Lūdzu ievadiet transakcijas iemeslu!")
				elseif bankAccNo == "" then
					exports.hud:sendBottomNotification(localPlayer, "ATM bankomāts", "Lūdzu ievadiet otras personas bankas konta nummuru!")
				else
			...
			eTransferP = guiCreateEdit(0.66, 0.43+posYOffset, 0.3, 0.075, "<Konta nummurs / Frakcijas nosaukums>", true, tabPersonal)
			...
			lTransferPReason = guiCreateLabel(0.1, 0.55+posYOffset, 0.2, 0.05, "Iemesls:", true, tabPersonal)
			guiSetFont(lTransferPReason, "default-bold-small")

			tTransferPReason = guiCreateEdit(0.22, 0.54+posYOffset, 0.74, 0.075, "<Transakcijas iemesls>", true, tabPersonal)
			addEventHandler("onClientGUIClick", tTransferPReason, function()
				if guiGetText(tTransferPReason) == "<Transakcijas iemesls>" then
...
function respondToPendingTransfer(msg, closeATMGUI)
	...
	exports.hud:sendBottomNotification(localPlayer, "ATM Bankomāts", msg)
end
...
function createTheRestOf(parent, isPinCodeDefault)
	...
	Label_Keypad_Number = guiCreateLabel(20,30,220,80,enteredCode,false,parent)
		...
		guiLabelSetVerticalAlign(Label_Keypad_Number,"center")
		guiLabelSetHorizontalAlign(Label_Keypad_Number,"center",false)
	Label_Error = guiCreateLabel(20,100,220,115,"Lūdzu ievadiet 4 ciparu jauno PIN kodu",false,parent)
	...
	bEnter = guiCreateButton(20,344+posYOffset,225,66,"Ievadīt",false,parent)
	addEventHandler( "onClientGUIClick", bEnter, function ()
		guiSetEnabled(bEnter, false)
		...
		if not cooldown and element and getElementType(element) =="object" and state=="up" and getElementParent(getElementParent(element)) == getResourceRootElement() then
			...
			if getDistanceBetweenPoints3D( px, py, pz, ax, ay, az ) < 1.3 then
				...
				if isAnyOneElseAround(element) then
					exports.hud:sendBottomNotification(localPlayer,"ATM Bankomāts jau tiek lietots!", "Pisies rindā, jo bankomātu var lietot tikai viens cilvēks.")
				else
...
function atmCardExisted()
	...
	if not (wGui) then
		showCursor(true)
		--NEW CARD
		wGui = guiCreateWindow(X, Y, Width, Height, "'Mums jau ir viens sistēmā, ko man darīt?'", false )
		option[1] = guiCreateButton( 0.05, verticalPos, 0.9, 1/numberOfButtons, "Izdzēs iepriekšējo karti un izveido jaunu.", true, wGui )
		...
		--CANCEL CARD
		option[6] = guiCreateButton( 0.05, verticalPos, 0.9, 1/numberOfButtons, "Ah, vienalga.", true, wGui )
...
function chooseAtm()
	...
	if not (wGui) then
		showCursor(true)
		--NEW CARD
		wGui = guiCreateWindow(X, Y, Width, Height, "'Kādu bankomāta karti vēlaties?'", false )
		option[1] = guiCreateButton( 0.05, verticalPos, 0.9, 1/numberOfButtons, "ATM Karte - Parastā ($50)\nVar veikt transakcijas ar ļoti limitētu daudzumu. (($10,000, 5 stundās))", true, wGui )
		...
		option[2] = guiCreateButton( 0.05, verticalPos, 0.9, 1/numberOfButtons, "ATM Karte - Premium ($200)\nVar veikt transakcijas ar godīgu daudzumu. (($50,000, 5 stundās))", true, wGui )
		...
		option[3] = guiCreateButton( 0.05, verticalPos, 0.9, 1/numberOfButtons, "ATM Karte - Sekseris ($500)\nVar veikt bezlimita daudzuma transakcijas dienā.", true, wGui )
		...
		--CANCEL CARD
		option[6] = guiCreateButton( 0.05, verticalPos, 0.9, 1/numberOfButtons, "Ah, neko.", true, wGui )
		addEventHandler( "onClientGUIClick", option[6], function()
			closeChooseAtm()
		end, false )
		verticalPos = verticalPos + 1/numberOfButtons
...
function hardenATM()
	...
	outputDebugString(count.." Bankomāti ir sacietināti.")
end
