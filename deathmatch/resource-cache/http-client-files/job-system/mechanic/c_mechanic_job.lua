...
function displayMechanicJob()
	outputChatBox("#FF9933Lieto #FF0000labā klikšķa izvēlni#FF9933 , lai apskatītu pakalpojumus, kurus vari veikt!.", 255, 194, 15, true)
end
...
function mechanicWindow(vehicle)
	...
	if factionType == 7 then -- Mechanic Faction / Adams]]
		if not vehicle then
			outputChatBox("Tev jāizvēlas auto.", 255, 0, 0)
		else
			...
			if not (wMechanic) then
				-- Create the window
				wMechanic = guiCreateWindow(X, Y, Width, Height, "Mehāniķa opcijas", false )
				...
				if not isPedInVehicle( localPlayer ) and getPedSimplestTask(localPlayer) ~= "TASK_SIMPLE_CAR_DRIVE" then
					...
					if getElementHealth(vehicle) > 950 then
						bMechanicTwo = guiCreateButton( 0.05, y, 0.9, 0.1, "Salabot - $110", true, wMechanic )
					...
					if not noTyres[getVehicleType(vehicle)] then
						bMechanicThree = guiCreateButton( 0.05, y, 0.9, 0.1, "Riepu maiņa - $10", true, wMechanic )
					...
					if faction[4] or ( tonumber( getElementData(vehicle, "job") or 0 ) == 0 and ( getElementData(vehicle, "faction") == -1 or faction[getElementData(vehicle, "faction")] ) ) then
						bMechanicFour = guiCreateButton( 0.05, y, 0.9, 0.1, "Pārkrāsot auto - $100", true, wMechanic )
					...
					if vehicleWithPaintjob[getElementModel(vehicle)] then
						bMechanicSix = guiCreateButton( 0.05, y, 0.9, 0.1, "Pašveidots krāsojums - $7,500", true, wMechanic )
					...
					-- Replace lights
					--if vehicleWithPaintjob[getElementModel(vehicle)] then
						bMechanicNine = guiCreateButton( 0.05, y, 0.9, 0.1, "Pārmainīt lukturus - $1,250", true, wMechanic )
					...
					if getVehicleUpgradeOnSlot(vehicle, 8) ~= 0 and faction[4] then
						bMechanicSeven = guiCreateButton( 0.05, y, 0.9, 0.1, "Noņemt NOS", true, wMechanic )
				...
				-- Add/Remove Tint
				bMechanicEight = guiCreateButton( 0.05, y, 0.9, 0.1, "Rediģēt tonējumu", true, wMechanic )
				y = y + 0.1
				...
				-- Close
				bMechanicClose = guiCreateButton( 0.05, 0.85, 0.9, 0.1, "Aizvērt", true, wMechanic )
...
function tyreWindow()
	...
	if not (wTyre) then
		-- Create the window
		wTyre = guiCreateWindow(X+100, Y, Width, Height, "Izvēlies riepu, kuru mainīt.", false )

		if getVehicleType(currentVehicle) ~= "Bike" then
			-- Front left
			bTyreOne = guiCreateButton( 0.05, 0.1, 0.45, 0.35, "Priekšējā kreisā", true, wTyre )
			...
			-- Back left
			bTyreTwo = guiCreateButton( 0.05, 0.5, 0.45, 0.35, "Aizmugurējā kreisā", true, wTyre )
			...
			-- front right
			bTyreThree = guiCreateButton( 0.5, 0.1, 0.45, 0.35, "Priekšējā labā", true, wTyre )
			...
			-- back right
			bTyreFour = guiCreateButton( 0.5, 0.5, 0.45, 0.35, "Aizmugurējā labā", true, wTyre )
			...
			-- Front
			bTyreOne = guiCreateButton( 0.05, 0.1, 0.9, 0.35, "Priekša", true, wTyre )
			addEventHandler( "onClientGUIClick", bTyreOne, function(button, state)
				if(button == "left" and state == "up") then
			...
			-- back right
			bTyreThree = guiCreateButton( 0.05, 0.5, 0.9, 0.35, "Aizmugure", true, wTyre )
			addEventHandler( "onClientGUIClick", bTyreThree, function(button, state)
				if(button == "left" and state == "up") then
		...
		-- Close
		bTyreClose = guiCreateButton( 0.05, 0.9, 0.9, 0.1, "Aizvērt", true, wTyre )
...
function paintColor( colorIndex )
	if exports.colorblender:isPickerOpened("paintjob") then
		outputChatBox("Tu vēlprojām rediģē citu krāsu pamuļķi.", 255, 0 ,0)
		return
	end
	local r, g, b = 0,0 ,0
	...
	exports.colorblender:openPicker("paintjob", string.format("#%02X%02X%02X", r, g, b) , "Izvēlies krāsu savam auto:")
end
...
function paintWindow()
	...
	if not (wPaint) then
		wPaint = guiCreateWindow(windowX, windowY, windowWidth, windowHeight, "Auto krāsa", false )
		bPaintColor1 = guiCreateButton( 0.05, 0.1, 0.45, 0.2, "Krāsa 1", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor1,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 1 )
			end
		end, false)
		bPaintColor2 = guiCreateButton( 0.55, 0.1, 0.45, 0.2, "Krāsa 2", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor2,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 2 )
			end
		end, false)
		bPaintColor3 = guiCreateButton( 0.05, 0.4, 0.45, 0.2, "Krāsa 3", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor3,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 3 )
			end
		end, false)
		bPaintColor4 = guiCreateButton( 0.55, 0.4, 0.45, 0.2, "Krāsa 4", true, wPaint )
		...
		bPaintSubmit = guiCreateButton( 0.05, 0.75, 0.90, 0.1, "Saglabāt", true, wPaint )
		...
		bPaintClose = guiCreateButton( 0.05, 0.85, 0.90, 0.1, "Aizvērt", true, wPaint )
...
function replaceLightsWindow()
	if exports.colorblender:isPickerOpened("paintjob") then
		outputChatBox("Tu vel rediģē citu krāsu!", 255, 0 ,0)
		return
	end

	local r, g, b = getVehicleHeadLightColor( currentVehicle )
	exports.colorblender:openPicker("headlights", string.format("#%02X%02X%02X", r, g, b) , "Izvēlies lukturu krāsu: ")
end
...
function paintjobWindow()
	...
	if not (wPaintjob) then
		...
		-- Create the window
		wPaintjob = guiCreateWindow(X+100, Y, Width, Height, "Izvēlēties jaunu paintjobu.", false )

		-- Paintjob 1
		bPaintjob1 = guiCreateButton( 0.05, 0.1, 0.9, 0.17, "Paintjobs 1", true, wPaintjob )
		...
		-- Paintjob 2
		bPaintjob2 = guiCreateButton( 0.05, 0.3, 0.9, 0.17, "Paintjobs 2", true, wPaintjob )
		...
		-- Paintjob 3
		bPaintjob3 = guiCreateButton( 0.05, 0.5, 0.9, 0.17, "Paintjobs 3", true, wPaintjob )
		...
		-- Paintjob 4
		bPaintjob4 = guiCreateButton( 0.05, 0.7, 0.9, 0.17, "Paintjobs 4", true, wPaintjob )
		...
		-- Close
		bPaintjobClose = guiCreateButton( 0.05, 0.9, 0.9, 0.1, "Aizvērt", true, wPaintjob )
