...
local fishermanJohn = createPed( 209, 154.21, -1942.65, 3.77 )
setPedRotation(fishermanJohn, 1.97)
setElementDimension(fishermanJohn, 0)
setElementInterior(fishermanJohn , 0)
setElementData(fishermanJohn, "nametag", true)
setElementData(fishermanJohn, "name", "Zvejnieks Oltis")
...
function startFishingSession()
    if exports['item-system']:hasItem(localPlayer, 49, 1) then
        if getElementData(localPlayer, "isfishing") then
            outputChatBox("Tu jau makšķerē...", 255, 0, 0)
        else 
            if isPedInVehicle(localPlayer) then
                return outputChatBox("Tu nevari makšķerēt kamēr vadi laivu.", 255, 0, 0)
            end
            if onBoat() then
                triggerServerEvent("sendAmeClient", localPlayer, "sāk makšķerēt, kā kaislīgs copmanis.")
                ...
                mgTimer = setTimer(
                    function()
                        if not onBoat() then
                            endFishingSession()
                            return outputChatBox("Tev nesanāca palikt pie ūdens.", 255, 0, 0)
                        end
                        pFish = guiCreateProgressBar(0.425, 0.75, 0.2, 0.035, true)
                        exports.hud:sendBottomNotification(localPlayer, "Zvejošana", "Tev pieķērās zivs! Lieto [ un ], lai to ievilktu iekšā!")
                        bindKey("[", "down", beginFishingGame)
                        -- Start the timer which determins if the fish would get away.
                        escapeTimer = setTimer(
                            function() 
                                endFishingSession()
                                outputChatBox("Velkot iekšā zivi, pēkšņi pārtrūktu aukla!", 255, 0, 0)
                                triggerServerEvent("fishing:takeRod", localPlayer, localPlayer)
                            end, 
                        math.random(8000, 10000), 1)
                    end
                , math.random(180000, 600000), 1)  
                outputChatBox("Tu jebkad vari beigt zvejot lietojot /stopfishing.", 0, 255, 0)
            else 
                outputChatBox("Tu nevari zvejot šeit.", 255, 0, 0)
            end
        end
    else
        outputChatBox("Tev nav makšķere un ar rokām čist tu neķersi.", 255, 0, 0)
    end
end
...
function endFishingSession()
    if (getElementData(localPlayer, "isfishing")) then
        ...
        triggerServerEvent("sendAmeClient", localPlayer, "ievelk iekšā auklu.")
...
function fishermanJohnRightClick(button, state, absX, absY, wx, wy, wz, element)
    if (element) and (getElementType(element)=="ped") and (button=="right") and (state=="down") then
        ...
        local rcMenu
        if(pedName == "Zvejnieks Oltis") then
...
function sellFish(fish, price)
    ...
    sellfishGUI = guiCreateWindow(744, 301, 303, 117, "Pārdot zivis", false)
    exports.global:centerWindow(sellfishGUI)
    showCursor(true)
    guiWindowSetSizable(sellfishGUI, false)

    sellfishLabel = guiCreateLabel(4, 18, 295, 44, "Tev šobrīd ir " .. fish .. " zivju un tu nopelnīsi $" .. price .. ".", false, sellfishGUI)
    guiLabelSetHorizontalAlign(sellfishLabel, "center", true)
    guiLabelSetVerticalAlign(sellfishLabel, "center")
    sellfishbutton = guiCreateButton(10, 68, 140, 34, "Pārdot zivis", false, sellfishGUI)
    cancelfishbutton = guiCreateButton(153, 68, 140, 34, "Atcelt", false, sellfishGUI)
