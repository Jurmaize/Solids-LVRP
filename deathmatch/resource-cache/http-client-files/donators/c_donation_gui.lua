...
function openDonationGUI(obtained1, available1, credits1, history1, purchased1, globalPurchaseHistory1)
	...
	if wDonation and isElement(wDonation) then
		...
		wDonation = guiCreateWindow(x,y,w,h,"Ziedotāju bonusi",false)
		guiWindowSetSizable(wDonation, false)

		GUIEditor_TabPanel = guiCreateTabPanel(0.0122,0.0401,0.9757,0.87,true,wDonation)
		tab.availableItems = guiCreateTab("Pieejamie bonusi",GUIEditor_TabPanel)
		tab.activatedPerks = guiCreateTab("Aktivizētie bonusi",GUIEditor_TabPanel)
		tab.purchased = guiCreateTab("Pirkumu vēsture",GUIEditor_TabPanel)
		tab.history = guiCreateTab("Mani ziedojumi",GUIEditor_TabPanel)
		...
		if exports.integration:isPlayerLeadAdmin(localPlayer) then
			tab.recent = guiCreateTab("Citu pirkumi",GUIEditor_TabPanel)
		end
		...
		col.name = guiGridListAddColumn(grid.availableItems,"Bonuss",0.78)
		col.duration = guiGridListAddColumn(grid.availableItems,"Ilgums",0.1)
		col.price = guiGridListAddColumn(grid.availableItems,"Cena",0.07)
		...
		col.a_name = guiGridListAddColumn(grid.activatedPerks,"Bonuss",0.7)
		col.a_expire = guiGridListAddColumn(grid.activatedPerks,"Beigsies",0.2)
		--col.a_id = guiGridListAddColumn(grid.activatedPerks,"ID",0.1)

		grid.purchased = guiCreateGridList(0,0,1,1,true,tab.purchased)
		col.b_name = guiGridListAddColumn(grid.purchased,"Bonuss",0.6)
		col.b_GC = guiGridListAddColumn(grid.purchased,"Cena",0.1)
		col.b_purchaseDate = guiGridListAddColumn(grid.purchased,"Pirkšanas datums",0.2)
		--[[
		...
		col.h_txn_id = guiGridListAddColumn(grid.history,"Transakcijas ID",0.2)
		col.h_email = guiGridListAddColumn(grid.history,"Info",0.45)
		col.h_amount = guiGridListAddColumn(grid.history,"Daudzums",0.1)
		col.h_date = guiGridListAddColumn(grid.history,"Datums",0.2)

		if exports.integration:isPlayerLeadAdmin(localPlayer) then
			grid.recent = guiCreateGridList(0,0,1,1,true,tab.recent)
			col.r_account = guiGridListAddColumn(grid.recent,"Konts",0.2)
			col.r_details = guiGridListAddColumn(grid.recent,"Info",0.4)
			col.r_amount = guiGridListAddColumn(grid.recent,"Daudzums",0.1)
			col.r_date = guiGridListAddColumn(grid.recent,"Datums",0.2)
		end

		gui.donate = guiCreateButton(0.0135,0.9135,0.48715,0.0675,"Pirkt GC!",true,wDonation)
		...
		bClose = guiCreateButton(0.0135+0.48715,0.9135,0.48715,0.0675,"Aizvērt",true,wDonation)
		...
		lSpendText = guiCreateLabel(0.82, 0.05, 0.3, 0.05, "GC:     "..exports.global:formatMoney(credits), true, wDonation)
	...
	updateMyHistory()
	updateRecents()
	guiSetText(lSpendText ,"GC:     "..exports.global:formatMoney(credits))
end
...
function updateAvailablePerks()
	...
	for perkID, perkArr in ipairs(available) do
		if (perkArr[1] ~= nil) and (perkArr[2] ~= 0) then
			...
			guiGridListSetItemText(grid.availableItems, row, col.duration, ( perkArr[3] > 1 and (perkArr[3] .." dienas") or "Bezgalīgs") , false, false)

			if perkArr[4] == 13 then--GCs Transfer
				guiGridListSetItemText(grid.availableItems, row, col.price, "Maksa "..perkArr[2].."%", false, false)
			...
			elseif perkArr[4] == 14 then--max ints
				...
				if credits >= perkArr[2]*(nextIntCap-10)*2 then
					...
					guiGridListSetItemText(grid.availableItems, row, col.price, perkArr[2]*(nextIntCap-10)*2 .." GC (nepietiekami)", false, false)
				end
			elseif perkArr[4] == 15 then--max veh
				...
				if credits >= perkArr[2]*(currentMaxVehicles-5)*2 then
					...
					guiGridListSetItemText(grid.availableItems, row, col.price, perkArr[2]*(currentMaxVehicles-5)*2 .." GC (nepietiekami)", false, false)
				end
			else
				if credits >= perkArr[2] then
					guiGridListSetItemText(grid.availableItems, row, col.price, perkArr[2] .." GC" , false, false)
				else
					guiGridListSetItemText(grid.availableItems, row, col.price, perkArr[2] .." GC (nepietiekami)", false, false)
				end
			end
...
function updatePurchaseHistory()
	...
	for i = 1, #purchased do
		...
		guiGridListSetItemText(grid.purchased, row, col.b_GC, ((tonumber(purchased[i][2]) > 0) and ("+"..purchased[i][2]) or (purchased[i][2])).." GC", false, true)
...
function updateRecents()
	...
	if exports.integration:isPlayerLeadAdmin(localPlayer) then
		...
		for i = 1, #globalPurchaseHistory do
			...
			guiGridListSetItemText(grid.recent, row, col.r_amount, ( tonumber(globalPurchaseHistory[i][2]) > 0 and ("+"..globalPurchaseHistory[i][2]) or (globalPurchaseHistory[i][2])).." GC" , false, true)
...
function showInfoPanel(state, cost)
	...
	local links = {
		[1] = "https://discord.gg/S5PDPdDeac",
		[20] = "https://discord.gg/S5PDPdDeac",
		[21] = "https://discord.gg/S5PDPdDeac",
	}
	if state == 1 then -- Donation intro
		length = 120
		content = "Sveiki, "..getElementData(localPlayer, "account:username").."! Mūsu serverim vajag tavu palīdzību! Pērciet GC, lai atbalstītu mūsu servera izmaksas!\n\nPalīdzot mūsu serveri, tu tiksi apbalvots ar GameCoins. Šis ir vēlviens valūtas veids ar ko var iegādāties ziedotāju bonusus.\n\nJebkāda naudiņa, kas ienāks no ziedotājiem tiks 100% ieinvestēta atpakaļ serverī un tā izmaksās.\n\nLūdzu apskatiet #ziedošana sadaļu mūsu diskordā, ja vēlaties ziedot. https://discord.gg/S5PDPdDeac"
		confirmBtnText = "Nokopēt URL"
	elseif state == 20 then-- Assest transfer
		length = 65
		content = "Tev šobrīd ir "..exports.global:formatMoney(credits).." GC, tas ļaus tev veikt "..math.floor(credits/cost).." pārsūtījumus.\n\nTu vari iegādāties vēl GC iegādājoties tos mūsu discordā!\n\nKatrs pārsūtījums izmaksā "..cost.." GC. Pats varat izvēlēties vai pārsūtat vienu mantu vai arī vairākas. (Auto, naudu, īpašumus)\n\nLūdzu apmeklējiet '"..links[state].."', lai uzzinātu vairāk par pārsūtīšanu."
		confirmBtnText = "Nokopēt URL"
	...
	elseif state == 23 then-- Personalized vehicle licence plates
		length = 0
		content = "Tu vari iztērēt "..cost.." GC, lai iegādātos sev pilnībā unikālu automašīnas nummuru, tādejādi laižot vaļā savu iekšējo radošo gariņu.\n\nLūdzu apmeklējiet CSDD, lai izmantotu bonusu."
		confirmBtnText = "Activate"
	...
	elseif state == 28 then -- radio
		length = 230
		content = "Tu vari iegādāties un uzturēt bezgalīgu daudzumu ar radiostacijām pa ("..cost.." GC katra), lai raidītu ēterā savu balsi, radio vai mūzikas kanālu!\n\nKad bonuss būs iegādāts, tu varēsi uzstādīt savu staciju, atjaunot vai iegādāties vēl, pārsaukt vai pat pārmainīt tiešraides URL jebkurā laikā, spiežot F10 -> Radio Staciju pārskats.\n\nLai iegādātos radio staciju, lūdzu ejat F10 izvēlne -> Radio Staciju pārskats -> Ziedotāju stacijas -> Iegādāties jaunu staciju."
...
function showPhonePicker(perkID)
	...
	gui["lblText2"] = guiCreateLabel(20, 45+15*2, windowWidth-40, 70, "Nospiežot 'Pirkt', Jūs piekrītat, ka atgriezt GC nevarēs. Paldies par sapratni!", false, wPhone)
...
function showUsernameChange(perkID)
	...
	guiCreateLabel(20, 25, windowWidth-40, 16, "Pārsaukt manu lietotāju:", false, guiUsername.main)
	...
	guiUsername.noti = guiCreateLabel(20, 45+30 , windowWidth-40, 16, "Pārliecinies, ka tas ir pareizs!", false, guiUsername.main)
	--guiLabelSetColor(guiUsername.noti, 255, 255, 255)

	guiUsername["lblText2"] = guiCreateLabel(20, 45+15*2, windowWidth-40, 70, "Nospiežot 'Pirkt', Jūs piekrītat, ka atgriezt GC nevarēs. Paldies par sapratni!", false, guiUsername.main)
	guiLabelSetHorizontalAlign(guiUsername["lblText2"], "left", true)
	guiLabelSetVerticalAlign(guiUsername["lblText2"], "center", true)

	guiUsername.purchase = guiCreateButton(20, 140, 150, 30, "Pirkt", false, guiUsername.main)
	...
	guiUsername.cancel = guiCreateButton(180, 140, 150, 30, "Atcelt", false, guiUsername.main)
...
function showGcTransfer(perkID, fee1)
	...
	guiGC["lblText2"] = guiCreateLabel(20, 45+15*6, windowWidth-40, 70, "Nospiežot 'Pirkt', Jūs piekrītat, ka atgriezt GC nevarēs. Paldies par sapratni!", false, guiGC.main)
...
function showKeypadDoorLock(perkID)
	...
	keypadDoor.purchase = guiCreateButton(20, 140+offSet, 150, 30, "Pirkt", false, keypadDoor.main)
	guiSetEnabled(keypadDoor.purchase, false)

	local ints = getInteriorsOwnedByPlayer()
	if #ints <= 0 then
		guiSetEnabled(keypadDoor.purchase, false)
		local t1 = guiCreateLabel(20, 25, windowWidth-40, 16*4, "Tev ir jāpieder vismaz vienam interjeram, lai iegādātos šo bonusu.", false, keypadDoor.main)
		guiLabelSetHorizontalAlign(t1, "left", true)
		--guiLabelSetVerticalAlign(t1, "center", true)
	else
		guiSetEnabled(keypadDoor.purchase, true)
		guiCreateLabel(20, 25, windowWidth-40, 16, "Digitālās slēdzene priekš interjera durvīm:", false, keypadDoor.main)
		...
		keypadDoor["lblText2"] = guiCreateLabel(20, 45+15*2, windowWidth-40, 70+offSet, "Šis bonuss nāk ar pāri digitālām slēdzenēm.\n\nŠī augstā gala drošības sistēma ir daudz labāka par parastām slēdzenēm, jo to nevar atlauzt.\n\nNospiežot 'Pirkt', Jūs piekrītat, ka atgriezt GC nevarēs. Paldies par sapratni!", false, keypadDoor.main)
	...
	keypadDoor.cancel = guiCreateButton(180, 140+offSet, 150, 30, "Atcelt", false, keypadDoor.main)
...
function checkCodeLength()
	if tonumber(string.len(guiGetText(source))) == 40 then
		guiSetText(lValid, "Kods izskatas derīgs.")
		guiLabelSetColor(lValid, 0, 255, 0)
		guiSetEnabled(bValidate, true)
	else
		guiSetText(lValid, "Kods nav derīgs .")
...
function showConfirmSpend(perkName, perkDur, perkCost, perkID)
	...
	local btmText = "Nospiežot 'Iegādāties', jūs saprotat, ka atgriezt GC nav iespējams. Paldies par izpalīdzēšanu!"
	if perkID == 24 or perkID == 25 or perkID == 26 then
		length = 5
		shiftDown = previewHeight+10
		btmText = "Ja tev pieder vairāk pa vienu ekrānu jau, tu vienmēr vari izmainīt aktīvo zem 'Aktivizetie bonusi' cilnes.\n\n"..btmText
	elseif perkID == 28 then
		length = 8
		previewHeight = 0
		shiftDown = previewHeight+10
		btmText = "Tu vari iegādāties un uzturēt bezgalīgu daudzumu ar radiostacijām pa ("..perkCost.." GC katra), lai raidītu ēterā savu balsi, radio vai mūzikas kanālu!\n\nKad bonuss būs iegādāts, tu varēsi uzstādīt savu staciju, atjaunot vai iegādāties vēl, pārsaukt vai pat pārmainīt tiešraides URL jebkurā laikā, spiežot F10 -> Radio Staciju pārskats.\n\nLai iegādātos radio staciju, lūdzu ejat F10 izvēlne -> Radio Staciju pārskats -> Ziedotāju stacijas -> Iegādāties jaunu staciju."..btmText
	end
	...
	gui["lblText1"] = guiCreateLabel(20, 25, windowWidth-40, 16, "Tu taisies iegādāties šo bonusu:", false, wPhone)
	gui["lblVehicleName"] = guiCreateLabel(20, 45+5, windowWidth-40, 13, "Bonuss: "..perkName, false, wPhone)
	guiSetFont(gui["lblVehicleName"], "default-bold-small")
	gui["lblDurr"] = guiCreateLabel(20, 45+15+5, windowWidth-40, 13, "Ilgums: "..perkDur, false, wPhone)
	guiSetFont(gui["lblDurr"], "default-bold-small")
	gui["lblVehicleCost"] = guiCreateLabel(20, 45+15*2+5, windowWidth-40, 13, "Cena: "..exports.global:formatMoney(perkCost), false, wPhone)
	...
	gui["spend"] = guiCreateButton(20, 140+15*(length)+shiftDown, 150, 30, "Iegādāties", false, wPhone)
	...
	gui["btnCancel"] = guiCreateButton(180, 140+15*(length)+shiftDown, 150, 30, "Atcelt", false, wPhone)
	addEventHandler( "onClientGUIClick", gui["btnCancel"], function()
		if source == gui["btnCancel"] then
			closeConfirmSpend()
		end
...
function showConfirmRemovePerk(aName, aExpireDate, aID)
	if tonumber(aID) == 24 or tonumber(aID) == 25 or tonumber(aID) == 26 then
		...
		gui["lblText1"] = guiCreateLabel(20, 25, windowWidth-40, 16, "Unikāls personāža izvēles ekrāns:", false, wPhone)
		guiSetFont(gui["lblText1"], "default-bold-small")
		gui["lblVehicleName"] = guiCreateLabel(20, 45+5, windowWidth-40, 13, aName, false, wPhone)
		--guiSetFont(gui["lblVehicleName"], "default-bold-small")
		gui["lblVehicleCost"] = guiCreateLabel(20, 45+15+5, windowWidth-40, 13, "Derīguma termiņš: "..aExpireDate, false, wPhone)
		--guiSetFont(gui["lblVehicleCost"], "default-bold-small")
		...
		if hasThisPerk and tonumber(thisPerkValue) == 1 then
			gui["use"] = guiCreateButton(20, 45+15*4+150, windowWidth-40, 30, "Beigt lietot šo ekrānu", false, wPhone)
			...
			gui["use"] = guiCreateButton(20, 45+15*4+150, windowWidth-40, 30, "Lietot šo ekrānu", false, wPhone)
			addEventHandler( "onClientGUIClick", gui["use"], function()
				if source == gui["use"] then
					if wPhone and isElement(wPhone) then
						guiSetEnabled(wPhone, false)
		...
		gui["spend"] = guiCreateButton(20, 140+150, 150, 30, "Noņemt", false, wPhone)
		addEventHandler( "onClientGUIClick", gui["spend"], function()
			if source == gui["spend"] then
				if wPhone and isElement(wPhone) then
					guiSetEnabled(wPhone, false)
		...
		gui["btnCancel"] = guiCreateButton(180, 140+150, 150, 30, "Atcelt", false, wPhone)
		addEventHandler( "onClientGUIClick", gui["btnCancel"], function()
			if source == gui["btnCancel"] then
				closeConfirmRemovePerk()
			end
		...
		gui["lblText1"] = guiCreateLabel(20, 25, windowWidth-40, 16, "Tu taisies noņemt šo bonusu:", false, wPhone)
		gui["lblVehicleName"] = guiCreateLabel(20, 45+5, windowWidth-40, 13, aName, false, wPhone)
		guiSetFont(gui["lblVehicleName"], "default-bold-small")
		gui["lblVehicleCost"] = guiCreateLabel(20, 45+15+5, windowWidth-40, 13, "Derīguma termiņš: "..aExpireDate, false, wPhone)
		guiSetFont(gui["lblVehicleCost"], "default-bold-small")
		gui["lblText2"] = guiCreateLabel(20, 45+15*2, windowWidth-40, 70, "Šo darbību nevarēs atgriezt!", false, wPhone)
		guiLabelSetHorizontalAlign(gui["lblText2"], "left", true)
		guiLabelSetVerticalAlign(gui["lblText2"], "center", true)

		gui["spend"] = guiCreateButton(20, 140, 150, 30, "Noņemt", false, wPhone)
		addEventHandler( "onClientGUIClick", gui["spend"], function()
			if source == gui["spend"] then
				if wPhone and isElement(wPhone) then
					guiSetEnabled(wPhone, false)
		...
		gui["btnCancel"] = guiCreateButton(180, 140, 150, 30, "Atcelt", false, wPhone)
		addEventHandler( "onClientGUIClick", gui["btnCancel"], function()
			if source == gui["btnCancel"] then
				closeConfirmRemovePerk()
			end
...
function showCustomChatIconMenu(pID, pCost, removing)
	...
	local content = "Tu vari iztērēt "..pCost..", lai dabūtu sev unikālu runāšanas ikoniņu (ar savas valsts karogu) virs galvas, kad runāsi.\n\nKad būsi iegādājies un aktivizējis sev šo bonusu, vienmēr varēsi pārmainīt karodziņu zem 'Aktivētie Bonusi' cilnes."

	if removing then
		content = "Šis bonuss tev uzliek virs galvas unikālu runāšanas ikoniņu ar sev izvēlēto karogu, kad runā!\n\nLūdzu izvēlies uz kuru karodziņu mainīt, jo esi jau iegādājies šo bonusu!"
	end
	...
	chatIcon["confirm"] = guiCreateButton(20, 120+length, btnW, 30, (removing and "Mainīt" or "Pirkt"), false, chatIcon.wMain)
	...
	chatIcon["btnCancel"] = guiCreateButton(20+btnW, 120+length, btnW, 30, "Atcelt", false, chatIcon.wMain)
...
function showLearnLanguageMenu(pID, pCost)
	...
	local btnW = (windowWidth-40)/2
	chatIcon["confirm"] = guiCreateButton(20, 120+length, btnW, 30, "Pirkt ("..tostring(pCost).." GC)", false, chatIcon.wMain)
	addEventHandler( "onClientGUIClick", chatIcon["confirm"], function()
		if source == chatIcon["confirm"] then
			playSoundCreate()
	...
	chatIcon["select"] = guiCreateComboBox(20, 150, windowWidth-40, 100, "Izvēlies valodu", false, chatIcon.wMain)
