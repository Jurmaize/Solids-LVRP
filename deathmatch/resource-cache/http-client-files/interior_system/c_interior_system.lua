...
function showIntName(name, ownerName, inttype, cost, ID, bizMsg)
	...
	elseif name then
		...
		if (ownerName=="None") and (inttype==3) then -- Unowned type 3 (rentable)
			gBuyMessage = guiCreateLabel(0.0, 0.915, 1.0, 0.3, "Nospied F, lai īrētu pa $" .. tostring(exports.global:formatMoney(cost)) .. ".", true)
		...
		elseif (ownerName=="None") and (inttype<2) then -- Unowned any other type
			gBuyMessage = guiCreateLabel(0.0, 0.915, 1.0, 0.3, "Nospied F, lai pirktu pa $" .. tostring(exports.global:formatMoney(cost)) .. ".", true)
			guiSetFont(gBuyMessage, "default")
			guiLabelSetHorizontalAlign(gBuyMessage, "center", true)
			guiSetAlpha(gBuyMessage, 0.0)
		else
			local msg = "Nospied F, lai ieietu."
...
function showHouseMenu( absX, absY )
	...
	row.lock = rightclick:addRow("Aizslēgt/Atslēgt")
	addEventHandler("onClientGUIClick", row.lock, lockUnlockHouse, false)

	row.knock = rightclick:addRow("Pieklauvēt pie durvīm")
	...
	if getElementType(house) == "interior" then
		if hasKey(houseID, true) then
			row.note = rightclick:addRow("Izmainīt sveicienu ziņu")
			addEventHandler("onClientGUIClick", row.note, function()
				...
				wBizNote = guiCreateWindow(posX,posY,width,height,"Izmainīt biznesa sveiciena ziņu - "..(getElementData(house, "name") or ("Interior ID #"..tostring( houseID ))),false)
				local eBizNote = guiCreateEdit(9,22,488,40,"",false,wBizNote)
				local bRemove = guiCreateButton(9,68,163,28,"Noņemt",false,wBizNote)
				local bSave = guiCreateButton(172,68,163,28,"Saglabāt",false,wBizNote)
				local bCancel = guiCreateButton(335,68,163,28,"Atcelt",false,wBizNote)
		...
		if interiorType>=0 and interiorType<3 then
			row.mailbox = rightclick:addRow("Pastkastīte")
...
function knockHouse()
	...
	if tick - lastKnocked > 5000 then
		...
		outputChatBox("Lūdzu uzgaidi nedaudz prims klauvē vēlreiz.", 255, 0, 0)
	end
end
...
function purchasePropertyGUI(interior, cost, isHouse, isRentable, neighborhood)
	...
	purchaseProperty.window[1] = guiCreateWindow(607, 396, 499, 210, "Iegādāties īpašumu", false)
	...
	local btnTextSet = {"Samaksāt skaidrā", "Samaksāt caur banku"}
	if exports.global:hasItem(localPlayer, 262) and (cost <= 40000) and isHouse and not isRentable then
		btnTextSet = {"Samaksāt ar tokenu", "Samaksāt caur banku"}
		exports.hud:sendBottomNotification(localPlayer, "Tip", "Tu spēlē ar jaunu personāžu. Tavā somā ir tokens ar ko var iegādāties šo īpašumu!")
	end
	if exports.global:countTable(fTable) > 0 then
		btnTextSet = {"Iegādāties\n sev", "Iegadāties\n frakcijai"}
	end
	...
	btnPosX = btnPosX + btnW + margin/2
	purchaseProperty.button[4] = guiCreateButton(btnPosX, 156, btnW, 43, "Apskatīt interjeru", false, purchaseProperty.window[1])
	guiSetProperty(purchaseProperty.button[4], "NormalTextColour", "FFAAAAAA")
	btnPosX = btnPosX + btnW + margin/2
	purchaseProperty.button[3] = guiCreateButton(btnPosX, 156, btnW, 43, "Aizvērt", false, purchaseProperty.window[1])
	guiSetProperty(purchaseProperty.button[3], "NormalTextColour", "FFAAAAAA")

	purchaseProperty.label[2] = guiCreateLabel(110, 44, 315, 20, "Tu vari izvēlēties maksāšanas metodes.", false, purchaseProperty.window[1])
	purchaseProperty.label[3] = guiCreateLabel(20, 70, 88, 15, "Nosaukums:", false, purchaseProperty.window[1])
	purchaseProperty.label[6] = guiCreateLabel(20, 90, 93, 15, "Apkārtne:", false, purchaseProperty.window[1])
	purchaseProperty.label[4] = guiCreateLabel(20, 110, 100, 15, "Cena:", false, purchaseProperty.window[1])
	purchaseProperty.label[5] = guiCreateLabel(250, 110, 73, 15, "Nodokļi:", false, purchaseProperty.window[1])
	purchaseProperty.label[11] = guiCreateLabel(20, 130, 315, 15, "Vai Jūs vēlaties, lai ir ieslēgtas mēbeles?", false, purchaseProperty.window[1]) -- Furniture
    ...
    purchaseProperty.rad[1] = guiCreateRadioButton(245, 128, 50, 20, "Jā", false, purchaseProperty.window[1])
    purchaseProperty.rad[2] = guiCreateRadioButton(295, 128, 50, 20, "Nē", false, purchaseProperty.window[1])
	...
	function()
		...
		if btnText == "Samaksāt skaidrā" then
	...
	function()
		...
		if btnText == "Samaksāt caur banku" then
	...
	function()
		...
		if btnText == "Apskatīt interjeru" then
	...
	if isHouse then
		...
		purchaseProperty.label[1] = guiCreateLabel(50, 26, 419, 18, "Lūdzu apstipriniet sekojošo informāciju par šo īpašumu!", false, purchaseProperty.window[1])
	...
	elseif isRentable then
		...
		purchaseProperty.label[1] = guiCreateLabel(50, 26, 419, 18, "Lūdzu apstipriniet sekojošo informāciju par šo īrējamo īpašumu!", false, purchaseProperty.window[1])
		guiLabelSetHorizontalAlign(purchaseProperty.label[1], "center", false)
		...
		purchaseProperty.label[1] = guiCreateLabel(50, 26, 419, 18, "Lūdzu apstipriniet sekojošo informāciju par šo biznesa īpašumu!", false, purchaseProperty.window[1])
		guiLabelSetHorizontalAlign(purchaseProperty.label[1], "center", false)
		taxtax = cost * theTax
		guiSetText(purchaseProperty.label[10], "$"..exports.global:formatMoney(taxtax).."")
	end
...
function startBuyingForFaction(interior, cost, isHouse, furniture)
    ...
    factionBuyGUI.window[1] = guiCreateWindow(766, 385, 399, 121, "Izvēlaties frakciju priekš šī interjera", false)
    ...
    factionBuyGUI.button[1] = guiCreateButton(13, 64, 111, 42, "Atcelt", false, factionBuyGUI.window[1])
    guiSetProperty(factionBuyGUI.button[1], "NormalTextColour", "FFAAAAAA")
    factionBuyGUI.combobox[1] = guiCreateComboBox(13, 35, 366, 113, "Izvēlies, kurai frakcijai iegādāties", false, factionBuyGUI.window[1])
    factionBuyGUI.button[2] = guiCreateButton(268, 64, 111, 42, "Apstiprināt", false, factionBuyGUI.window[1])
    ...
    addEventHandler("onClientGUIClick", factionBuyGUI.button[2], function()
    	...
    	if name ~= "Izvēlies, kurai frakcijai iegādāties" then
