...
function renderInteriorName()
    if hitPickup and isElement(hitPickup) then
        ...
        makeFonts()
        local intInst = "Nospied F, lai ieietu"
        ...
        if isLastSourceInterior then
			...
			if intAddress and intAddress ~= "" then
				local intAddress = "Adrese: "..intAddress
            ...
            if canPlayerKnowInteriorOwner(theInterior) then -- House or Biz
                ...
                if intStatus.owner > 0 then
                    ...
                    if intType == 3 then
                        intOwner = "Izīrējis "..(ownerName or "..Lādējas..")
                        intInst = "Press F to enter"
                    elseif intType ~= 2 then
                        intOwner = "Pieder "..(ownerName or "..Lādējās..")
                        intInst = "Spied F, lai ieietu"
                    end
                elseif intStatus.faction > 0 then
                    local ownerName = exports.cache:getFactionNameFromId(intStatus.faction)
                    if intType ~= 2 then
                        intOwner = "Pieder "..(ownerName or "..Lādējas..")
                        intInst = "Spied F, lai ieietu"
                    end
                else
                    if intType == 2 then
                        intOwner = "Nepieder nevienam"
                        intInst = "Spied F, lai ieietu"
                    elseif intType == 3 then
                        local intPrice = exports.global:formatMoney(intStatus.cost)
                        intOwner = "Izīrējams pa: $"..intPrice
                        intInst = "Spied F, lai izīrētu"
                    else
                        local intPrice = exports.global:formatMoney(intStatus.cost)
                        intOwner = "Pārdodas pa: $"..intPrice
                        intInst = "Spied F, lai nopirktu"
