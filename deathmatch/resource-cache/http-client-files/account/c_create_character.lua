...
function newCharacter_init()
	...
	gui["lblCharName"] = guiCreateLabel(10, 25, 91, 16, "Vārds:", false, gui["_root"])
	...
	gui["lblCharNameExplanation"] = guiCreateLabel(10, 40, 240, 80,"Vārdam ir jābūt sekojošajā formātā: \n     Vārds Uzvārds\nPiemēram: Taylor Jackson.\nTu nedrīksti lietot īstu, populāru personību\n vārdus. Piemēram, Michael Jackson.", false, gui["_root"])
...
--[[	gui["lblCharDesc"] = guiCreateLabel(10, 125, 230, 100, "When you first spawn type /editlook and describe your character in more detail.", false, gui["_root"])
	...
	gui["lblGender"] = guiCreateLabel(10, 160, 46, 13, "Dzimums:", false, gui["_root"])
	guiLabelSetHorizontalAlign(gui["lblGender"], "left", false)
	guiLabelSetVerticalAlign(gui["lblGender"], "center")
	gui["rbMale"] = guiCreateRadioButton(90, 160, 51, 13, "Vīrietis", false, gui["_root"])
	gui["rbFemale"] = guiCreateRadioButton(150, 160, 82, 13, "Sieviete", false, gui["_root"])
	...
	gui["lblWeight"] = guiCreateLabel(10, 220, 111, 16, "Svars", false, gui["_root"])
	...
	gui["lblAge"] = guiCreateLabel(10, 240, 111, 16, "Vecums", false, gui["_root"])
	...
	gui["lblDay"] = guiCreateLabel(10, 282, 111, 16, "Dzimšanas diena", false, gui["_root"])
	...
	gui["lblMonth"] = guiCreateLabel(10, 260, 111, 16, "Mēnesis", false, gui["_root"])
	guiLabelSetHorizontalAlign(gui["lblDay"], "left", false)
	guiLabelSetVerticalAlign(gui["lblDay"], "center")


	gui["drpMonth"] =  guiCreateComboBox ( 110, 260, 130, 16, "Janvāris", false, gui["_root"])
	guiComboBoxAdjustHeight(gui["drpMonth"], 15)
	guiComboBoxAddItem(gui["drpMonth"], "Janvāris")
	guiComboBoxAddItem(gui["drpMonth"], "Februāris")
	guiComboBoxAddItem(gui["drpMonth"], "Marts")
	guiComboBoxAddItem(gui["drpMonth"], "Aprīlis")
	guiComboBoxAddItem(gui["drpMonth"], "Maijs")
	guiComboBoxAddItem(gui["drpMonth"], "Jūnijs")
	guiComboBoxAddItem(gui["drpMonth"], "Jūlijs")
	guiComboBoxAddItem(gui["drpMonth"], "Augusts")
	guiComboBoxAddItem(gui["drpMonth"], "Septembris")
	guiComboBoxAddItem(gui["drpMonth"], "Oktobris")
	guiComboBoxAddItem(gui["drpMonth"], "Novembris")
	guiComboBoxAddItem(gui["drpMonth"], "Decembris")
	...
	gui["lblLanguage"] = guiCreateLabel(10, 305, 111, 16, "Valoda:", false, gui["_root"])
	...
	gui["lblLanguageDisplay"] = guiCreateLabel(126, 305, 98, 16, "Angļu", false, gui["_root"])
	...
	gui["btnLangs"] = guiCreateButton(10, 330, 231, 41, "Visas valodas", false, gui["_root"])
	addEventHandler("onClientGUIClick", gui["btnLangs"] , newCharacter_viewAllLangs, false)

	gui["btnCancel"] = guiCreateButton(10, 370+5, 231, 41, "Atcelt", false, gui["_root"])
	addEventHandler("onClientGUIClick", gui["btnCancel"], newCharacter_cancel, false)

	gui["btnCreateChar"] = guiCreateButton(10,410+10, 231, 41, "Izveidot", false, gui["_root"])
...
function newCharacter_updateScrollBars()
	...
	guiSetText(gui["lblDay"], "Datums: "..(scrDay or "1"))
end
...
function newCharacters_getSkinArray()
	...
	if (guiCheckBoxGetSelected( gui["chkBlack"] )) then -- BLACK
		...
		elseif (guiRadioButtonGetSelected( gui["rbFemale"] )) then -- BLACK FEMALE
			...
			outputChatBox("Lūdzu izvēlaties dzimumu!", 0, 255, 0)
		end
...
function newCharacter_attemptCreateCharacter()
	...
	if not nameCheckPassed then
		LoginScreen_showWarningMessage( "Neizdevās apstrādāt Jūsu personāža vārdu:\n".. nameCheckError )
		return
	end
	--[[
	...
	elseif (guiCheckBoxGetSelected(gui["chkAsian"])) then
		...
		LoginScreen_showWarningMessage( "Neizdevās apstrādāt Jūsu personāža rasi:\nNone selected." )
		return
	end
	...
	elseif (guiRadioButtonGetSelected( gui["rbFemale"] )) then
		...
		LoginScreen_showWarningMessage( "Neizdevās apstrādāt Jūsu personāža dzimumu:\nNone selected." )
		return
	end

	local skin = getElementModel( dummyPed )
	if not skin then
		LoginScreen_showWarningMessage( "Neizdevās apstrādāt Jūsu personāža ādiņu:\nNone selected." )
		return
	end
	...
	if languageselected == nil then
		LoginScreen_showWarningMessage( "Neizdevās apstrādāt Jūsu personāža valodu:\nNone selected." )
		return
	end
...
function newCharacter_response(statusID, statusSubID)
	if (statusID == 1) then
		LoginScreen_showWarningMessage( "Whoops, kaut kas nesanāca, meiģini vēlreiz\nvai kontaktējies ar administrāciju.\nError ACC"..tostring(statusSubID) )
	elseif (statusID == 2) then
		if (statusSubID == 1) then
			LoginScreen_showWarningMessage( "Šis personāža vārds jau ir lietošanā\npiedod :(!" )
		else
...
function monthToNumber(monthName)
	if not monthName then
		...
		if monthName == "Janvāris" then
			return 1
		elseif monthName == "Februāris" then
			return 2
		elseif monthName == "Marts" then
			return 3
		elseif monthName == "Aprīlis" then
			return 4
		elseif monthName == "Maijs" then
			return 5
		elseif monthName == "Jūnijs" then
			return 6
		elseif monthName == "Jūlijs" then
			return 7
		elseif monthName == "Augusts" then
			return 8
		elseif monthName == "Septembris" then
			return 9
		elseif monthName == "Oktobris" then
			return 10
		elseif monthName == "Novembris" then
			return 11
		elseif monthName == "Decembris" then
			return 12
		else
			return 1
		end
