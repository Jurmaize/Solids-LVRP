...
function createComputerGUI(computerName, websiteToStartOn)
	...
	local toggleInput = guiCreateButton(0, computerHeight - 24, 90, 24, "Pārslēgt ievadi", false, desktopImage)
	...
	if key == "left" and state == "down" then
		if(source == chatButton) then
			if (guiGetInputEnabled( )) then
				...
				showCursor(false)
				outputChatBox("Čats aktīvs", 0, 255, 0, true)
				outputChatBox("Nospied M uz klavietūras, lai turpinātu lietot datoru.")
			else
				guiSetInputEnabled(true)
				showCursor(false)
				outputChatBox("Datora logs aktīvs", 0, 255, 0, true)
			end
		end
	end
end
