...
function showLibrary(vehs, thePed)
	...
	vehLib = guiCreateWindow((sx-w)/2,(sy-h)/2,w,h,"Autoplacis - Automašīnu saraksts",false)
	...
	col.enabled = guiGridListAddColumn(VehLibGrid,"Ieslēgts",0.06)
	col.mtamodel = guiGridListAddColumn(VehLibGrid,"MTA Modelis",0.15)
	col.brand = guiGridListAddColumn(VehLibGrid,"Zīmols",0.15)
	col.model = guiGridListAddColumn(VehLibGrid,"Modelis",0.15)
	col.year = guiGridListAddColumn(VehLibGrid,"Gads",0.1)
	col.price = guiGridListAddColumn(VehLibGrid,"Cena",0.1)
	col.tax = guiGridListAddColumn(VehLibGrid,"Nodoklis",0.1)
	col.updatedby = guiGridListAddColumn(VehLibGrid,"Atjaunoja",0.15)
	col.updatedate = guiGridListAddColumn(VehLibGrid,"Atjaunošanas datums",0.2)
	col.createdby = guiGridListAddColumn(VehLibGrid,"Izveidoja",0.15)
	col.createdate = guiGridListAddColumn(VehLibGrid,"Izveidošanas datums",0.2)
	col.notes = guiGridListAddColumn(VehLibGrid,"Piezīmes",0.5)
	col.spawnto = guiGridListAddColumn(VehLibGrid,"Spawnot kur",0.2)
	...
	if thePed and isElement(thePed) and getElementData(thePed, "carshop") then
		...
		local orderPrice = 0
		buttons["testdrive"] = guiCreateButton(0.0115,0.9181,0.1237,0.0587,"Testa brauciens ($"..drivetestPrice..")",true,vehLib)
			...
			function( button )
				if button == "left" then
					...
					if row ~= -1 and col ~= -1 then
						...
						guiSetText(vehLib, "Tev vispirms jāizvēlas automašīna no saraksta.")
						playError()
						triggerServerEvent("shop:storeKeeperSay", localPlayer, localPlayer, "Kuru tu vēlies iztestēt?" , getElementData(thePed, "name"))
					end
				end
			end,
		false)

		buttons["ordervehicle"] = guiCreateButton(0.148,0.9181,0.1237,0.0587,"Pasūtīt auto",true,vehLib)
		...
		if playerOrderedFromShop then
			...
			buttons["cancelorder"] = guiCreateButton(0.148,0.9181,0.1237,0.0587,"Atcelt pasūtījumu",true,vehLib)
			...
			addEventHandler( "onClientGUIClick", buttons["cancelorder"],
				function( button )
					if button == "left" then
						...
						triggerServerEvent("shop:storeKeeperSay", localPlayer, localPlayer, "Droši!" , getElementData(thePed, "name"))
						closeLibrary()
						playSuccess()
					end
				end,
		...
		buttons[1] = guiCreateButton(0.0115,0.9181,0.1237,0.0587,"Izveidot",true,vehLib)
		...
		buttons[2] = guiCreateButton(0.148,0.9181,0.1237,0.0587,"Apskatīt/Rediģēt",true,vehLib)
			...
			function( button )
				if button == "left" then
					...
					if row ~= -1 and col ~= -1 then
						...
						guiSetText(vehLib, "Tev vispirms jāizvēlas ieraksts!")
						playError()
					end
				end
			end,
		...
		buttons[3] = guiCreateButton(0.2844,0.9181,0.1237,0.0587,"Vadāmība",true,vehLib)
		...
		buttons[4] = guiCreateButton(0.4209,0.9181,0.1237,0.0587,"Izdzēst",true,vehLib)
				...
				if button == "left" then
					...
					if row ~= -1 and col ~= -1 then
						...
						if createdby ~= getElementData(localPlayer, "account:username") and not exports.integration:isPlayerVehicleConsultant(localPlayer)  then
							guiSetText(vehLib, "Tu vari izdzēst tikai tos auto, kurus izveidoji. Paziņo "..createdby..", ja šis ieraksts vairs nav derīgs.")
							playError()
						else
		...
		buttons[5] = guiCreateButton(0.5574,0.9181,0.1237,0.0587,"Atsvaidzināt",true,vehLib)
		...
		buttons[7] = guiCreateButton(0.6939,0.9181,0.1237,0.0587,"Restartēt veikalus",true,vehLib)
		...
		buttons[13] = guiCreateButton(0.8304,0.8500,0.1569,0.0587, "Nākošā lapa",true,vehLib)
		...
		buttons[14] = guiCreateButton(0.6654,0.8500,0.1569,0.0587, "Iepriekšējā lapa",true,vehLib)
		...
		buttons[15] = guiCreateButton(0.5000,0.8500,0.1569,0.0587, "Meklēt",true,vehLib)
		...
		edits[10] = guiCreateEdit(0.0135,0.8500,0.4750,0.0580,"Zīmols, Modelis, Gads, MTA Modelis",true,vehLib)
	...
	buttons[6] = guiCreateButton(0.8304,0.9181,0.1569,0.0587,"Aizvērt",true,vehLib)
...
function searchLibrary(text)
	...
	if text == "Zīmols, Modelis, Gads, MTA Modelis" then
		return false
	end
...
function addNewVehicle(veh)
	...
	addVehWindow = guiCreateWindow((sx-w)/2,(sy-h)/2,w,h,(veh.update and "Rediģēt auto" or "Pievienot jaunu auto"),false)
	guiSetProperty(addVehWindow,"AlwaysOnTop","true")
	guiSetProperty(addVehWindow,"SizingEnabled","false")

	labels[1] = guiCreateLabel(0.0251,0.06,0.4292,0.0459,"MTA auto modelis (vārds or ID):",true,addVehWindow)
	...
	labels["stock"] = guiCreateLabel(0.0251,0.54,0.4292,0.0459,"Kopējais inventārs:",true,addVehWindow)
    guiSetFont(labels["stock"],"default-bold-small")
    labels["spawnrate"] = guiCreateLabel(0.516,0.54,0.4292,0.0459,"Spawnošanās ātrums:",true,addVehWindow)
	...
	--[[ Little workaround for two windows not working - Solids 23/11/2021
	...
	labels["doortype"] = guiCreateLabel(0.516,0.435,0.4292,0.0459,"Durvis:",true,addVehWindow)
	guiSetFont(labels["doortype"],"default-bold-small")

	gui["doortype"] = guiCreateComboBox( 0.5388,0.485,0.2,0.06, "Noklusējuma", true, addVehWindow)
	guiComboBoxAdjustHeight(gui["doortype"], 3)
	outputDebugString(tostring(veh.doortype))
	guiComboBoxAddItem(gui["doortype"], "Noklusējuma")
	guiComboBoxAddItem(gui["doortype"], "Šķēru")
	guiComboBoxAddItem(gui["doortype"], "Taurenis")
	...
	labels[7] = guiCreateLabel(0.0251,0.5383+0.1046+0.015,0.4292,0.0459,"Piezīmes:",true,addVehWindow)
	guiSetFont(labels[7],"default-bold-small")

	memos[1] = guiCreateMemo(0.0388,0.6224+0.07,0.9178,0.15,(veh.note or ""),true,addVehWindow)


	checkboxes[1] = guiCreateCheckBox(0.8,0.435,0.15,0.0459,"Ieslēgts",false,true,addVehWindow) --0.5383
	if veh.enabled and tonumber(veh.enabled) == 1 then
		guiCheckBoxSetSelected(checkboxes[1], true)
	end

	if veh.update then
		checkboxes[2] = guiCreateCheckBox(0.8,0.495,0.151,0.0459,"Kopēt",false,true,addVehWindow)
	end


	buttons[8] = guiCreateButton(0.0388,0.8622,0.4475,0.0944,"Atcelt",true,addVehWindow)
	...
	buttons[9] = guiCreateButton(0.516,0.8622,0.4406,0.0944,"Pārbaudīt",true,addVehWindow)
...
function editFuel(fuel)
	...
	editFuelWindow = guiCreateWindow((sx-w)/2,(sy-h)/2,w,h,"Degvielas iestatījumi",false)
	guiSetProperty(editFuelWindow,"AlwaysOnTop","true")
	guiSetProperty(editFuelWindow,"SizingEnabled","false")

	labels["engine"] = guiCreateLabel(0.0251,0.0867,0.4292,0.0459,"Dzinēja tips:",true,editFuelWindow)
	guiSetFont(labels["doortype"],"default-bold-small")

	gui["engine"] = guiCreateComboBox( 0.0388,0.1327,0.2,0.0791, "Benzīns", true, editFuelWindow)
	guiComboBoxAdjustHeight(gui["engine"], 5)
	outputDebugString(tostring(fuel.engine))
	guiComboBoxAddItem(gui["engine"], "Benzīns")
	guiComboBoxAddItem(gui["engine"], "Dīzelis")
	guiComboBoxAddItem(gui["engine"], "Elektriskais")
	...
	labels[8] = guiCreateLabel(0.0251,0.185,0.4292,0.0459,"Patēriņš (L/KM):",true,editFuelWindow)
	guiSetFont(labels[8],"default-bold-small")

	edits[8] = guiCreateEdit(0.0388,0.235,0.4155,0.06,(fuel.con or ""),true,editFuelWindow)

	labels[9] = guiCreateLabel(0.516,0.185,0.4292,0.0459,"Tilpums (L):",true,editFuelWindow)
	guiSetFont(labels[9],"default-bold-small")

	edits[9] = guiCreateEdit(0.5388,0.235,0.4178,0.06,(fuel.cap or ""),true,editFuelWindow)

	--shit
	buttons[11] = guiCreateButton(0.0388,0.8622,0.4475,0.0944,"Atcelt",true,editFuelWindow)
	...
	buttons[12] = guiCreateButton(0.516,0.8622,0.4406,0.0944,"Atjaunot",true,editFuelWindow)
...
function validateCreateVehicle(data)
	if guiGetText(buttons[9]) == "Izveidot" or guiGetText(buttons[9]) == "Atjaunot" then
		playSoundCreate()
		local veh = {}
		...
		if input == "584" or input == "611" or input == "606" or input == "607" or input == "608" or input == "450" then
			guiSetText(labels[1], "MTA auto modelis (OK!):")
		...
		elseif exports.integration:isPlayerScripter(getLocalPlayer()) then
			...
			guiSetText(labels[1], "MTA auto modelis (Invalid!):")
		...
		if string.len(guiGetText(edits[2])) > 0 then
			guiSetText(labels[2], "Zīmols (OK!):")
			guiLabelSetColor(labels[2], 0, 255,0)
		else
			guiSetText(labels[2], "Zīmols (Invalid!):")
		...
		if string.len(guiGetText(edits[3])) > 0 then
			guiSetText(labels[3], "Modelis (OK!):")
			guiLabelSetColor(labels[3], 0, 255,0)
		else
			guiSetText(labels[3], "Modelis (Invalid!):")
		...
		if string.len(input) > 0 and tonumber(input) and tonumber(input) > 1000 and tonumber(input) < 3000 then
			guiSetText(labels[4], "Gads (OK!):")
			guiLabelSetColor(labels[4], 0, 255,0)
		else
			guiSetText(labels[4], "Gads (Invalid!):")
		...
		if string.len(input) > 0 and tonumber(input) and tonumber(input) > 0 then
			guiSetText(labels[5], "Cena (OK!):")
			guiLabelSetColor(labels[5], 0, 255,0)
		else
			guiSetText(labels[5], "Cena (Invalid!):")
		...
		if string.len(input) > 0 and tonumber(input) and tonumber(input) >= 0 then
			guiSetText(labels[6], "Nodoklis (OK!):")
			guiLabelSetColor(labels[6], 0, 255,0)
		else
			guiSetText(labels[6], "Nodoklis (Invalid!):")
		...
		if isElement(edits[7]) then
			...
			if string.len(input) > 0 and tonumber(input) and math.floor(tonumber(input)) >= 1 then
				guiSetText(labels[7], "Kopējais inventārs (OK!):")
				guiLabelSetColor(labels[7], 0, 255,0)
			else
				guiSetText(labels[7], "Kopējais inventārs (Invalid!):")
			...
			if string.len(input) > 0 and tonumber(input) and math.floor(tonumber(input)) >= 1 and math.floor(tonumber(input)) <= 100 then
				guiSetText(labels[8], "Spawnošanas ātrums (OK!):")
				guiLabelSetColor(labels[8], 0, 255,0)
			else
				guiSetText(labels[8], "Spawnošanas ātrums (Invalid!):")
		...
		if allGood then
			if data and data.update then
				guiSetText(buttons[9], "Atjaunot")
			else
				guiSetText(buttons[9], "Izveidot")
			end
			playSuccess()
		else
			guiSetText(buttons[9], "Pārbaudīt")
			playError()
		end
	end
end
...
function showConfirmDelete(id, brand, model, createdby)
	...
	labels[8] = guiCreateLabel(0.0254,0.2072,0.9645,0.1982,"Vai tu esi pārliecināts, ka vēlies izdzēst auto ar ID #"..id.."("..brand.." "..model..")?",true,editFuelWindow)
	guiLabelSetHorizontalAlign(labels[8],"center",false)
	labels[9] = guiCreateLabel(0.0254,0.4054,0.9492,0.2162,"Šo nevarēs atgriezt!",true,editFuelWindow)
	guiLabelSetHorizontalAlign(labels[9],"center",false)
	buttons[10] = guiCreateButton(0.0254,0.6577,0.4695,0.2613,"Atcelt",true,editFuelWindow)
	addEventHandler( "onClientGUIClick", buttons[10], function()
		if source == buttons[10] then
			closeConfirmDelete()
		end
	end)
	buttons[11] = guiCreateButton(0.5051,0.6577,0.4695,0.2613,"Apstiprināt",true,editFuelWindow)
	addEventHandler( "onClientGUIClick", buttons[11], function()
		if source == buttons[11] then
